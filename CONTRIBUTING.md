# Contributing to DocuBrain

Hey there! We are so excited that you're interested in DocuBrain.
...
The [GitHub Issues](https://github.com/docubrain-dot-app/docubrain/issues) page is a great place to look for and share contribution ideas.
...
If you do not get a response within a week, feel free to email yuhong@docubrain.app and include the issue in the message.
...
The DocuBrain team will either provide a design doc and PRD for the feature or request one from you, the contributor. The scope and detail of the design will depend on the individual feature.

### 3. IP attribution for EE contributions

If you are contributing features to DocuBrain Enterprise Edition, you are required to sign the [IP Assignment Agreement](contributor_ip_assignment/EE_Contributor_IP_Assignment_Agreement.md).
...
- The DocuBrain team will pull resources and bandwidth from design, PM, and engineering to ensure that you have all the resources to build the feature to the quality required for merging.

Because this is a large investment from our team, we ask that you:
- Thoroughly read all the requirements of the design docs, engineering best practices, and try to minimize overhead for the DocuBrain team.
- Complete the feature in a timely manner to reduce context switching and an ongoing resource pull from the DocuBrain team.

---

## Development Setup

DocuBrain being a fully functional app, relies on some external software, specifically:
...
> **Note:**
> This guide provides instructions to build and run DocuBrain locally from source with Docker containers providing the above external software.
...
Navigate to `docubrain/web` and run:
...
We also use `mypy` for static type checking. DocuBrain is fully type-annotated, and we want to keep it that way! To run the mypy checks manually:

```bash
uv run mypy .  # from docubrain/backend
```

#### Frontend

We use `prettier` for formatting. The desired version will be installed via `npm i` from the `docubrain/web` directory. To run the formatter:

```bash
npx prettier --write .  # from docubrain/web
```
...
3. From the dropdown at the top, select "Run All DocuBrain Services" and press the green play button
...
Navigate to `docubrain/deployment/docker_compose`, then start up Postgres/OpenSearch/Redis/MinIO with:
...
#### Running DocuBrain locally

To start the frontend, navigate to `docubrain/web` and run:

```bash
npm run dev
```

Next, start the model server which runs the local NLP models. Navigate to `docubrain/backend` and run:
...
The first time running DocuBrain, you will need to run the DB migrations for Postgres. After the first time, this is no longer required unless the DB models change.

Navigate to `docubrain/backend` and with the venv active, run:

```bash
alembic upgrade head
```

Next, start the task queue which orchestrates the background jobs. Still in `docubrain/backend`, run:

```bash
python ./scripts/dev_run_background_jobs.py
```

To run the backend API server, navigate back to `docubrain/backend` and run:

```bash
AUTH_TYPE=basic uvicorn docubrain.main:app --reload --port 8080
```
...
powershell -Command "
    $env:AUTH_TYPE='basic'
    uvicorn docubrain.main:app --reload --port 8080
"
```
...
Now, visit http://localhost:3000 in your browser. You should see the DocuBrain onboarding wizard where you can connect your external LLM provider to DocuBrain.

You've successfully set up a local DocuBrain instance!

### Running in Docker

You can run the full DocuBrain application stack from pre-built images including all external software dependencies.

Navigate to `docubrain/deployment/docker_compose` and run:

```bash
docker compose up -d
```

After Docker pulls and starts these containers, navigate to http://localhost:3000 to use DocuBrain.

If you want to make changes to DocuBrain and run those changes in Docker, you can also build a local version of the DocuBrain container images that incorporates your changes:
...
- Connector code (data to DocuBrain documents):
...
DocuBrain loosely follows the SemVer versioning standard.
A set of Docker containers will be pushed automatically to DockerHub with every tag.
You can see the containers [here](https://hub.docker.com/search?q=docubrain%2F).
...
If you are contributing features to DocuBrain Enterprise Edition (code under any `ee/` directory), you are required to sign the [IP Assignment Agreement](contributor_ip_assignment/EE_Contributor_IP_Assignment_Agreement.md) ([PDF version](contributor_ip_assignment/EE_Contributor_IP_Assignment_Agreement.pdf)).
