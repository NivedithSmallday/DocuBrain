...
# Remote (with params):
#   & ([scriptblock]::Create((irm https://raw.githubusercontent.com/docubrain-dot-app/docubrain/main/deployment/docker_compose/install.ps1))) -Lite -NoPrompt
# Remote (defaults only, configure via interaction during script):
#   irm https://raw.githubusercontent.com/docubrain-dot-app/docubrain/main/deployment/docker_compose/install.ps1 | iex
...
$script:InstallRoot = if ($env:INSTALL_PREFIX) { $env:INSTALL_PREFIX } else { "docubrain_data" }
$script:LiteComposeFile = "docker-compose.docubrain-lite.yml"
$script:GitHubRawUrl = "https://raw.githubusercontent.com/docubrain-dot-app/docubrain/main/deployment/docker_compose"
$script:NginxBaseUrl = "https://raw.githubusercontent.com/docubrain-dot-app/docubrain/main/deployment/data/nginx"
...
function Print-DocubrainError{ param([string]$Message) Write-Host "[X]  $Message" -ForegroundColor Red }
...
function Prompt-DeploymentMode {
    ...
    if ($modeChoice -eq "2") {
        ...
        if (-not (Ensure-DocubrainFile $LiteOverlayPath "$($script:GitHubRawUrl)/$($script:LiteComposeFile)" $script:LiteComposeFile)) { exit 1 }
    }
}
...
function Assert-NotCraftLite {
    ...
    Print-DocubrainError "Cannot use a craft image tag ($Tag) with Lite mode."
...
function Download-DocubrainFile {
...
function Test-DocubrainHealth {
    param([int]$Port)
    Print-Info "Checking DocuBrain service health..."
    ...
    for ($attempt = 1; $attempt -le 600; $attempt++) {
        ...
        Write-Host -NoNewline "`rChecking DocuBrain service${dots}${pad} (${m}m ${s}s elapsed)"
...
function Show-DocubrainHelp {
    $help = "DocuBrain Installation Script for Windows`n"
    $help += "`nUsage: .\install.ps1 [OPTIONS]`n"
    $help += "`nOptions:"
    $help += "`n  -IncludeCraft  Enable DocuBrain Craft (AI-powered web app building)"
    $help += "`n  -Lite          Deploy DocuBrain Lite (no Vespa, Redis, or model servers)"
    $help += "`n  -Local         Use existing config files instead of downloading from GitHub"
    $help += "`n  -Shutdown      Stop (pause) DocuBrain containers"
    $help += "`n  -DeleteData    Remove all DocuBrain data (containers, volumes, and files)"
    ...
    $help += "`n`nExamples:"
    $help += "`n  .\install.ps1                    # Install DocuBrain"
    $help += "`n  .\install.ps1 -Lite              # Install DocuBrain Lite"
    $help += "`n  .\install.ps1 -IncludeCraft      # Install with Craft enabled"
    $help += "`n  .\install.ps1 -Shutdown          # Pause DocuBrain services"
    $help += "`n  .\install.ps1 -DeleteData        # Completely remove DocuBrain"
    $help += "`n  .\install.ps1 -Local             # Re-run using existing config"
    $help += "`n  .\install.ps1 -NoPrompt          # Non-interactive install"
    Write-Host $help
}

function Invoke-DocubrainShutdown {
    Write-Host "`n=== Shutting down DocuBrain ===`n" -ForegroundColor Cyan
    $deployDir = Join-Path $script:InstallRoot "deployment"
    if (-not (Test-Path (Join-Path $deployDir "docker-compose.yml"))) {
        Print-Warning "DocuBrain deployment not found. Nothing to shutdown."
        return
    }
    if (-not (Initialize-ComposeCommand)) { Print-DocubrainError "Docker Compose not found."; exit 1 }
    $stopArgs = @("stop")
    $result = Invoke-Compose -AutoDetect @stopArgs
    if ($result -ne 0) { Print-DocubrainError "Failed to stop containers"; exit 1 }
    Print-Success "DocuBrain containers stopped (paused)"
}

function Invoke-DocubrainDeleteData {
    Write-Host "`n=== WARNING: This will permanently delete all DocuBrain data ===`n" -ForegroundColor Red
    Print-Warning "This action will remove all DocuBrain containers, volumes, files, and user data."
    if (Test-Interactive) {
        $confirm = Prompt-OrDefault "Type 'DELETE' to confirm" ""
        if ($confirm -ne "DELETE") { Print-Info "Operation cancelled."; return }
    } else {
        Print-DocubrainError "Cannot confirm destructive operation in non-interactive mode."
        exit 1
    }
    $deployDir = Join-Path $script:InstallRoot "deployment"
    if ((Test-Path (Join-Path $deployDir "docker-compose.yml")) -and (Initialize-ComposeCommand)) {
        ...
        else { Print-DocubrainError "Failed to remove containers" }
    }
    if (Test-Path $script:InstallRoot) {
        Remove-Item -Recurse -Force $script:InstallRoot
        Print-Success "Data directories removed"
    }
    Print-Success "All DocuBrain data has been permanently deleted!"
}
...
function Wait-ForDockerDaemon {
    ...
    while ($waited -lt $MaxWait) {
        ...
        if ($currentError) {
            ...
            if ($unchangedErrorCount -ge 5) {
                Write-Host ""; Print-DocubrainError "Docker daemon is not starting. Persistent error after ${waited}s:"
    ...
    Write-Host ""; Print-DocubrainError "Docker daemon did not respond within ${MaxWait} seconds."
...
function Register-DockerService {
    ...
    if (-not $dockerdPath) {
        Print-DocubrainError "Could not find dockerd.exe to register as a service."
        return $false
    }
    ...
    if (-not (Get-Service docker -ErrorAction SilentlyContinue)) {
        Print-DocubrainError "Failed to register Docker as a Windows service."
        return $false
    }
...
function Install-DockerEngine {
    ...
    if (-not $installed) {
        ...
        try {
            ...
            Download-DocubrainFile -Url "https://download.docker.com/win/static/stable/x86_64/$($latestZip.href)" -Output $zipPath
    ...
    if (-not $installed) {
        Print-DocubrainError "Could not install Docker Engine on Windows Server."
        Print-Info "Install manually: https://docs.docker.com/engine/install/binaries/#install-server-and-client-binaries-on-windows"
        exit 1
    }

    try { Start-Service docker -ErrorAction Stop; Print-Success "Docker service started" }
    catch { Print-DocubrainError "Failed to start Docker service: $_"; exit 1 }
    Install-ComposePlugin
    if (-not (Wait-ForDockerDaemon -MaxWait 30)) { Print-DocubrainError "Docker installed but daemon not responding."; exit 1 }
...
function Install-ComposePlugin {
    ...
    try {
        Download-DocubrainFile -Url "https://github.com/docker/compose/releases/latest/download/docker-compose-windows-x86_64.exe" -Output (Join-Path $dest "docker-compose.exe")
...
function Install-DockerDesktop {
    ...
    if (-not $installed) {
        ...
        try {
            Download-DocubrainFile -Url "https://desktop.docker.com/win/main/amd64/Docker%20Desktop%20Installer.exe" -Output $installerPath
            ...
            if ($proc.ExitCode -eq 0) {
                ...
                if (-not $wslReady) { Print-DocubrainError "WSL2 is required. Run: wsl --install --no-distribution, then reboot." }
    ...
    if (-not $installed) {
        Print-DocubrainError "Could not install Docker Desktop automatically."
    ...
    if (-not (Get-Command docker -ErrorAction SilentlyContinue)) {
        Print-DocubrainError "Docker installed but 'docker' command not available. Restart your terminal and re-run."
        exit 1
    }
    if (-not (Start-DockerDaemon)) {
        Print-DocubrainError "Docker Desktop installed but could not be started. Launch it from the Start Menu and re-run."
        exit 1
    }
...
function Invoke-WslInstall {
    ...
    Print-Info "DocuBrain will be installed via WSL2 (Windows Subsystem for Linux)."
    if (-not (Confirm-Action "DocuBrain via WSL2 (installs WSL2 + Ubuntu + Docker inside Linux)")) { exit 1 }
    if (-not (Test-IsAdmin)) { Invoke-ElevatedRelaunch }
    ...
    try {
        ...
        if ($totalGB -lt 4) {
            Print-DocubrainError "DocuBrain requires at least 4GB RAM (Lite mode) or 10GB RAM (Standard mode)."
    ...
    if ($LASTEXITCODE -ne 0) {
        ...
        try {
            ...
            if ($proc.ExitCode -ne 0) {
                Print-DocubrainError "WSL2 installation failed (code $($proc.ExitCode)). A reboot may be needed."
                Print-Info "After rebooting, re-run this script."
                exit 1
            }
        } catch {
            Print-DocubrainError "Could not install WSL2: $_"
            exit 1
        }
    }
    ...
    if ($distros -notmatch "Ubuntu") {
        ...
        if ($proc.ExitCode -ne 0) {
            Print-DocubrainError "Ubuntu installation failed. Try manually: wsl --install -d Ubuntu"
    ...
    if ($wslExit -eq 0) {
        Print-Success "DocuBrain installation complete (via WSL2)"
        # Determine the port DocuBrain is running on inside WSL
        Print-Info "DocuBrain should be accessible at http://localhost:3000"
        Print-Info "WSL2 automatically forwards ports to the Windows host."
    } else {
        Print-DocubrainError "Installation inside WSL2 exited with code $wslExit"
...
function Main {
    if ($Help) { Show-DocubrainHelp; return }
    if ($PSVersionTable.PSVersion.Major -lt 5) { Print-DocubrainError "PowerShell 5+ required (found $($PSVersionTable.PSVersion))"; exit 1 }
    if ($script:LiteMode -and $script:IncludeCraftMode) {
        Print-DocubrainError "-Lite and -IncludeCraft cannot be used together."
        exit 1
    }
    if ($script:LiteMode) { $script:ExpectedDockerRamGB = 4; $script:ExpectedDiskGB = 16 }
    if ($Shutdown)   { Invoke-DocubrainShutdown; return }
    if ($DeleteData) { Invoke-DocubrainDeleteData; return }
    ...
    Write-Host "Welcome to DocuBrain Installation Script (Windows)"
    Write-Host "=============================================="
    Print-Success "$edition detected"
    Write-Host "This script will:" -ForegroundColor Yellow
    Write-Host "1. Download deployment files for DocuBrain into a new '$($script:InstallRoot)' directory"
    ...
    if (-not (Initialize-ComposeCommand)) {
        if ($script:IsWindowsServer) {
            Install-ComposePlugin
            if (-not (Initialize-ComposeCommand)) { Print-DocubrainError "Docker Compose could not be installed."; exit 1 }
        } else {
            Print-DocubrainError "Docker Compose is not installed. Docker Desktop includes it."
    ...
    if ($LASTEXITCODE -ne 0) {
        ...
        if (-not (Start-DockerDaemon)) { Print-DocubrainError "Could not start Docker. Start it manually and re-run."; exit 1 }
    }
    Print-Success "Docker daemon is running"
    if ($script:IsWindowsServer) { Fix-DockerCredStore }

    # Verify Docker is running Linux containers (DocuBrain images are Linux-based)
    $osType = ((Invoke-NativeQuiet -PassThru { docker info --format '{{.OSType}}' }) -join "").Trim()
    if ($osType -eq "windows") {
        Print-Warning "Docker is running in Windows containers mode, but DocuBrain requires Linux containers."
    ...
    if ($resourceWarning) {
        Print-Warning "DocuBrain recommends at least $($script:ExpectedDockerRamGB)GB RAM and $($script:ExpectedDiskGB)GB disk for standard mode."
    ...
    else { Print-Step "Downloading DocuBrain configuration files" }

    $composeDest = Join-Path $deploymentDir "docker-compose.yml"
    if (-not (Ensure-DocubrainFile $composeDest "$($script:GitHubRawUrl)/docker-compose.yml" "docker-compose.yml")) { exit 1 }
    ...
    if (-not (Ensure-DocubrainFile $envTemplateDest "$($script:GitHubRawUrl)/env.template" "env.template")) { exit 1 }
    if (-not (Ensure-DocubrainFile (Join-Path $script:InstallRoot "data\nginx\app.conf.template") "$($script:NginxBaseUrl)/app.conf.template" "nginx/app.conf.template")) { exit 1 }
    if (-not (Ensure-DocubrainFile (Join-Path $script:InstallRoot "data\nginx\run-nginx.sh") "$($script:NginxBaseUrl)/run-nginx.sh" "nginx/run-nginx.sh")) { exit 1 }
    if (-not (Ensure-DocubrainFile (Join-Path $script:InstallRoot "README.md") "$($script:GitHubRawUrl)/README.md" "README.md")) { exit 1 }
    ...
    if ((Test-Path $composeDest) -and (Initialize-ComposeCommand)) {
        ...
        if ($running.Count -gt 0) {
            Print-DocubrainError "DocuBrain services are currently running!"
    ...
    if (Test-Path $envFile) {
        ...
        Prompt-DeploymentMode -LiteOverlayPath $liteOverlayPath
        if ($script:LiteMode -and $script:IncludeCraftMode) {
            Print-DocubrainError "-IncludeCraft cannot be used with Lite mode."
            exit 1
        }
        if ($script:LiteMode) { $script:ExpectedDockerRamGB = 4; $script:ExpectedDiskGB = 16 }
        ...
        if ($script:IncludeCraftMode -or $version -match '^craft-') {
            Set-EnvFileValue -Path $envFile -Key "ENABLE_CRAFT" -Value "true" -Uncomment
            Print-Success "DocuBrain Craft enabled"
        } else {
            Print-Info "DocuBrain Craft disabled (use -IncludeCraft to enable)"
        }
    ...
    if (-not $useLatest -and -not $Local) {
        $pinnedBase = "https://raw.githubusercontent.com/docubrain-dot-app/docubrain/$currentImageTag/deployment"
        Print-Info "Fetching config files matching tag $currentImageTag..."
        try {
            Download-DocubrainFile "$pinnedBase/docker_compose/docker-compose.yml" $composeDest
            try { Download-DocubrainFile "$pinnedBase/data/nginx/app.conf.template" (Join-Path $script:InstallRoot "data\nginx\app.conf.template") } catch {}
            try { Download-DocubrainFile "$pinnedBase/data/nginx/run-nginx.sh" (Join-Path $script:InstallRoot "data\nginx\run-nginx.sh") } catch {}
            if ($script:LiteMode) {
                try { Download-DocubrainFile "$pinnedBase/docker_compose/$($script:LiteComposeFile)" $liteOverlayPath } catch {}
            }
    ...
    if ((Invoke-Compose @pullArgs) -ne 0) { Print-DocubrainError "Failed to download Docker images"; exit 1 }
    Print-Success "Docker images downloaded successfully"

    # ── Step 8: Start Services ────────────────────────────────────────────
    Print-Step "Starting DocuBrain services"
    Print-Info "Launching containers..."
    $upArgs = @("up", "-d")
    if ($useLatest) { $upArgs += @("--pull", "always", "--force-recreate") }
    $upResult = Invoke-Compose @upArgs
    if ($upResult -ne 0) { Print-DocubrainError "Failed to start DocuBrain services"; exit 1 }
    ...
    foreach ($cid in $containerIds) {
        ...
        if ($status -eq "running" -and $restarts -gt 2) {
            Print-DocubrainError "$name is in a restart loop (restarted $restarts times)"; $restartIssues = $true
        } elseif ($status -eq "running") { Print-Success "$name is healthy" }
        elseif ($status -eq "restarting") { Print-DocubrainError "$name is stuck restarting"; $restartIssues = $true }
        else { Print-Warning "$name status: $status" }
    }

    if ($restartIssues) {
        Print-DocubrainError "Some containers are experiencing issues!"
        ...
        Print-Info "For help, contact: founders@docubrain.app"
        exit 1
    }
    ...
    if (Test-DocubrainHealth -Port $port) {
        Write-Host "============================================" -ForegroundColor Green
        Write-Host "   DocuBrain service is ready!                   " -ForegroundColor Green
        Write-Host "============================================" -ForegroundColor Green
    } else {
        ...
        Write-Host "   DocuBrain containers are running              " -ForegroundColor Yellow
        Write-Host "============================================" -ForegroundColor Yellow
    }

    Print-Info "Access DocuBrain at: http://localhost:$port"
    ...
    Print-Info "For help or issues, contact: founders@docubrain.app"
}
