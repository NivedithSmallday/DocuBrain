...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        --verbose)
            ...
            ;;
        --help|-h)
            echo "DocuBrain Installation Script"
            echo ""
            echo "Usage: $0 [OPTIONS]"
            echo ""
            echo "Options:"
            echo "  --include-craft  Enable DocuBrain Craft (AI-powered web app building)"
            echo "  --lite           Deploy DocuBrain Lite (no Vespa, Redis, or model servers)"
            echo "  --local          Use existing config files instead of downloading from GitHub"
            echo "  --shutdown       Stop (pause) DocuBrain containers"
            echo "  --delete-data    Remove all DocuBrain data (containers, volumes, and files)"
            ...
            echo ""
            echo "Examples:"
            echo "  $0                    # Install DocuBrain"
            echo "  $0 --lite             # Install DocuBrain Lite (minimal deployment)"
            echo "  $0 --include-craft    # Install DocuBrain with Craft enabled"
            echo "  $0 --shutdown         # Pause DocuBrain services"
            echo "  $0 --delete-data      # Completely remove DocuBrain and all data"
...
INSTALL_ROOT="${INSTALL_PREFIX:-docubrain_data}"

LITE_COMPOSE_FILE="docker-compose.docubrain-lite.yml"
...
if [ "$SHUTDOWN_MODE" = true ]; then
    echo ""
    echo -e "${BLUE}${BOLD}=== Shutting down DocuBrain ===${NC}"
    echo ""
    
    if [ -d "${INSTALL_ROOT}/deployment" ]; then
        print_info "Stopping DocuBrain containers..."
        ...
        if [ -f "${INSTALL_ROOT}/deployment/docker-compose.yml" ]; then
            ...
            if [ $? -eq 0 ]; then
                print_success "DocuBrain containers stopped (paused)"
            else
        ...
        print_warning "DocuBrain data directory not found. Nothing to shutdown."
    fi

    echo ""
    print_success "DocuBrain shutdown complete!"
    exit 0
fi
...
if [ "$DELETE_DATA_MODE" = true ]; then
    echo ""
    echo -e "${RED}${BOLD}=== WARNING: This will permanently delete all DocuBrain data ===${NC}"
    echo ""
    print_warning "This action will remove:"
    echo "  • All DocuBrain containers and volumes"
    ...
    print_info "Removing DocuBrain containers and volumes..."
    ...
    if [ -d "${INSTALL_ROOT}/deployment" ]; then
        ...
        if [ -f "${INSTALL_ROOT}/deployment/docker-compose.yml" ]; then
            ...
            if [ $? -eq 0 ]; then
                print_success "DocuBrain containers and volumes removed"
            else
    ...
    echo ""
    print_success "All DocuBrain data has been permanently deleted!"
    exit 0
fi
...
if ! command -v docker &> /dev/null; then
    if [[ "$OSTYPE" == "linux-gnu"* ]] || [[ -n "${WSL_DISTRO_NAME:-}" ]]; then
        ...
        if ! confirm_action "Docker Engine"; then
            print_error "Docker is required to run DocuBrain."
            exit 1
        fi
        install_docker_linux
        if ! command -v docker &> /dev/null; then
...
if command -v docker &> /dev/null \
    ...
    if ! confirm_action "Docker Compose plugin"; then
        print_error "Docker Compose is required to run DocuBrain."
        exit 1
    fi
...
if command -v docker &> /dev/null \
    ...
    exec sg docker -c "_DOCUBRAIN_REEXEC=1 bash $(printf '%q ' "$0" "$@")"
fi
...
echo "              |___/       "
echo -e "${NC}"
echo "Welcome to DocuBrain Installation Script"
echo "===================================="
echo ""

# User acknowledgment section
echo -e "${YELLOW}${BOLD}This script will:${NC}"
echo "1. Download deployment files for DocuBrain into a new '${INSTALL_ROOT}' directory"
...
GITHUB_RAW_URL="https://raw.githubusercontent.com/docubrain-dot-app/docubrain/main/deployment/docker_compose"
...
if [ "$RESOURCE_WARNING" = true ]; then
    echo ""
    print_warning "DocuBrain recommends at least ${EXPECTED_DOCKER_RAM_GB}GB RAM and ${EXPECTED_DISK_GB}GB disk space for optimal performance in standard mode."
...
NGINX_BASE_URL="https://raw.githubusercontent.com/docubrain-dot-app/docubrain/main/deployment/data/nginx"

if [[ "$USE_LOCAL_FILES" = true ]]; then
    print_step "Verifying existing configuration files"
else
    print_step "Downloading DocuBrain configuration files"
...
if [ -d "${INSTALL_ROOT}/deployment" ] && [ -f "${INSTALL_ROOT}/deployment/docker-compose.yml" ]; then
    ...
    if [ -n "$COMPOSE_CMD" ]; then
        ...
        if [ "$RUNNING_CONTAINERS" -gt 0 ]; then
            print_error "DocuBrain services are currently running!"
            echo ""
            print_info "To make configuration changes, you must first shut down the services."
            echo ""
            print_info "Please run the following command to shut down DocuBrain:"
...
if [ -f "$ENV_FILE" ]; then
    ...
    if [ "$INCLUDE_CRAFT" = true ] || [[ "$VERSION" == craft-* ]]; then
        ...
        sed -i.bak 's/^#* *ENABLE_CRAFT=.*/ENABLE_CRAFT=true/' "$ENV_FILE" 2>/dev/null || true
        print_success "DocuBrain Craft enabled (ENABLE_CRAFT=true)"
    else
        print_info "DocuBrain Craft disabled (use --include-craft to enable)"
    fi
    ...
    echo "  • DocuBrain Craft (set ENABLE_CRAFT=true)"
    echo ""
fi
...
if [[ "$USE_LATEST" = false ]] && [[ "$USE_LOCAL_FILES" = false ]]; then
    PINNED_BASE="https://raw.githubusercontent.com/docubrain-dot-app/docubrain/${CURRENT_IMAGE_TAG}/deployment"
...
# Start services
print_step "Starting DocuBrain services"
...
if [ $? -ne 0 ]; then
    print_error "Failed to start DocuBrain services"
    exit 1
fi
...
check_docubrain_health() {
    ...
    while [ $attempt -le $max_attempts ]; do
        ...
        printf "\r\033[KChecking DocuBrain service%s (%dm %ds elapsed)" "$dots" "$minutes" "$seconds"
...
# Run health check
if check_docubrain_health; then
    echo ""
    echo -e "${GREEN}${BOLD}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
    echo -e "${GREEN}${BOLD}   🎉 DocuBrain service is ready! 🎉${NC}"
    echo -e "${GREEN}${BOLD}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
else
    ...
    echo -e "${YELLOW}${BOLD}   ⚠️  DocuBrain containers are running ⚠️${NC}"
    echo -e "${YELLOW}${BOLD}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
fi
echo ""
print_info "Access DocuBrain at:"
...
echo ""
print_info "For help or issues, contact: founders@docubrain.app"
echo ""
...
if is_interactive && command -v gh &>/dev/null; then
    prompt_yn_or_default "Enjoying DocuBrain? Star the repo on GitHub? [Y/n] " "Y"
    if [[ ! "$REPLY" =~ ^[Nn] ]]; then
        if GH_PAGER= gh api -X PUT /user/starred/docubrain-dot-app/docubrain < /dev/null >/dev/null 2>&1; then
            print_success "Thanks for the star!"
        else
            print_info "Star us at: https://github.com/docubrain-dot-app/docubrain"
        fi
    fi
fi
