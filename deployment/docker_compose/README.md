...
To set up DocuBrain there are several options, DocuBrain supports the following for deployment:
...
**For more detailed guides, please refer to the documentation: https://docs.docubrain.app/deployment/overview**

## install.sh script

```
curl -fsSL https://raw.githubusercontent.com/docubrain-dot-app/docubrain/main/deployment/docker_compose/install.sh > install.sh && chmod +x install.sh && ./install.sh
```

This provides a guided installation of DocuBrain via Docker Compose. It will deploy the latest version of DocuBrain
and set up the volumes to ensure data is persisted across deployments or upgrades.

The script will create an docubrain_data directory, all necessary files for the deployment will be stored in
...
DocuBrain maintains backwards compatibility across all minor versions following SemVer. If following the install.sh script (or through Docker Compose), you can
...
IMAGE_TAG is the version of DocuBrain to run. It is recommended to leave it as latest to get all updates with each redeployment.
