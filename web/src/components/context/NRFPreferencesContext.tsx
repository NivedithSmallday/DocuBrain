...
interface NRFPreferencesContextValue {
  useDocubrainAsNewTab: boolean;
  setUseDocubrainAsNewTab: (v: boolean) => void;
}
...
export function NRFPreferencesProvider({
  ...
  const [useDocubrainAsNewTab, setUseDocubrainAsNewTab] = useLocalStorageState<boolean>(
    LocalStorageKeys.USE_DOCUBRAIN_AS_NEW_TAB,
    true
  );
  ...
  return (
    <NRFPreferencesContext.Provider
      value={{
        useDocubrainAsNewTab,
        setUseDocubrainAsNewTab,
      }}
    >
      {children}
    </NRFPreferencesContext.Provider>
