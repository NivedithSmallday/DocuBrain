...
import { IconProps, DocubrainIcon } from "@/components/icons/icons";
...
export function AppPopup() {
  ...
  // Header icon rules:
  // - If neither app name nor custom logo exists -> show DocuBrain icon
  // - If logo display is "name_only" -> show alert icon
  // - Otherwise -> show uploaded custom logo (fallback to DocuBrain icon)
  const headerIcon =
    !hasApplicationName && !hasCustomLogo
      ? (props: IconProps) => <DocubrainIcon size={24} {...props} />
  ...
  return (
    <Modal open onOpenChange={() => { }}>
      <Modal.Content width="sm" height="lg">
        <Modal.Header
          icon={headerIcon}
          title={popupTitle || "Welcome to DocuBrain!"}
        />
        <Modal.Body>
