...
import { MinimalDocubrainDocument } from "@/lib/search/interfaces";
...
export default function NRFPage({ isSidePanel = false }: NRFPageProps) {
  const { setUseDocubrainAsNewTab } = useNRFPreferences();
  ...
  // If user toggles the "Use DocuBrain" switch to off, prompt a modal
  const handleUseDocubrainToggle = (checked: boolean) => {
    if (!checked) {
      setShowTurnOffModal(true);
    } else {
      setUseDocubrainAsNewTab(true);
      sendSetDefaultNewTabMessage(true);
    }
  };

  const confirmTurnOff = () => {
    setUseDocubrainAsNewTab(false);
      ...
      {!isSidePanel && (
        <>
          <SettingsPanel
            ...
            handleUseDocubrainToggle={handleUseDocubrainToggle}
          />
          ...
          <Modal open={showTurnOffModal} onOpenChange={setShowTurnOffModal}>
            <Modal.Content width="sm">
              <Modal.Header
                ...
                title="Turn off DocuBrain new tab page?"
                description="You'll see your browser's default new tab page instead. You can turn it back on anytime in your DocuBrain settings."
      ...
      {!user && (
        <Modal open onOpenChange={() => { }}>
          <Modal.Content width="sm" height="sm">
            <Modal.Header icon={SvgUser} title="Welcome to DocuBrain" />
            <Modal.Body>
