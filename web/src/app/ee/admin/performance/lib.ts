...
import { DocubrainBotAnalytics, QueryAnalytics, UserAnalytics } from "./usage/types";
...
export const useDocubrainBotAnalytics = (timeRange: DateRangePickerValue) => {
  const url = buildApiPath("/api/analytics/admin/docubrainbot", {
    start: convertDateToStartOfDay(timeRange.from)?.toISOString(),
    end: convertDateToEndOfDay(timeRange.to)?.toISOString(),
  });
  const swrResponse = useSWR<DocubrainBotAnalytics[]>(url, errorHandlingFetcher); // TODO

  return {
    ...swrResponse,
    refreshDocubrainBotAnalytics: () => mutate(url),
  };
};
