...
import { DocubrainIcon } from "@/components/icons/icons";
...
function PreviewLogo({
  ...
  logoSrc?: string;
  forceDocubrainIcon?: boolean;
  size: number;
  className?: string;
}) {
  return logoSrc && !forceDocubrainIcon ? (
    <img
      src={logoSrc}
      alt="Logo"
      style={{
    ...
    <DocubrainIcon size={size} className={cn("flex-shrink-0", className)} />
  );
}
            ...
            {(logoDisplayStyle === "logo_and_name" ||
              logoDisplayStyle === "name_only") && (
                <Truncated mainUiAction text04 nowrap>
                  {applicationDisplayName || "DocuBrain"}
                </Truncated>
              )}
          </div>
        </div>
