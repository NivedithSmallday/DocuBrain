...
export const SettingsPanel = ({
  settingsOpen,
  toggleSettings,
  handleUseDocubrainToggle,
}: {
  settingsOpen: boolean;
  toggleSettings: () => void;
  handleUseDocubrainToggle: (checked: boolean) => void;
}) => {
  const { useDocubrainAsNewTab } = useNRFPreferences();
        ...
        <div className="px-6 pb-8 flex flex-col gap-8">
          ...
          <section className="flex flex-col gap-3">
            ...
            <div className="flex flex-col gap-1 bg-background-tint-01 rounded-2xl px-4">
              <SettingRow label="Use DocuBrain as new tab page">
                <Switch
                  checked={useDocubrainAsNewTab}
                  onCheckedChange={handleUseDocubrainToggle}
                />
              </SettingRow>
            </div>
          </section>
