...
export function useAgents(enabled = true) {
  // Agent management UI may be feature-gated, but the main chat experience
  // still needs access to available personas/default assistant metadata.
  const shouldFetchAgents = enabled;
  const { data, error, mutate } = useSWR<MinimalPersonaSnapshot[]>(
    shouldFetchAgents ? SWR_KEYS.personas : null,
  ...
  return {
    agents: data ?? [],
    isLoading: shouldFetchAgents && !error && !data,
    error,
    refresh: mutate,
  };
}
...
export function useAgent(agentId: number | null) {
  const shouldFetchAgent = agentId !== null;
  const { data, error, isLoading, mutate } = useSWR<FullPersona>(
    shouldFetchAgent ? SWR_KEYS.persona(agentId) : null,
    errorHandlingFetcher,
    {
      revalidateOnFocus: false,
      revalidateIfStale: false,
 ...
 */
export function usePinnedAgents(enabled = true) {
  const shouldUseAgents = enabled;
  const { user, refreshUser } = useUser();
  const { agents, isLoading: isLoadingAgents } = useAgents(shouldUseAgents);
  ...
  const togglePinnedAgent = useCallback(
    ...
    [localPinnedAgents, refreshUser, shouldUseAgents]
  );
 ...
 */
export function useCurrentAgent(enabled = true): MinimalPersonaSnapshot | null {
  const shouldUseAgents = enabled;
  const { agents } = useAgents(shouldUseAgents);
