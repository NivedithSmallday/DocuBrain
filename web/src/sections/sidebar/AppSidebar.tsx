...
const MemoizedAppSidebarInner = memo(function AppSidebarInner() {
  ...
  // Check if DocuBrain Craft is enabled via settings (backed by PostHog feature flag)
  // Only explicit true enables the feature; false or undefined = disabled
  const isDocubrainCraftEnabled =
    combinedSettings?.settings?.docubrain_craft_enabled === true;

  // Find build_mode feature announcement notification (only if DocuBrain Craft is enabled)
  const buildModeNotification = isDocubrainCraftEnabled
    ? notifications?.find(
      (n) =>
    ...
    () => (
      <div>
        ...
        <AccountPopover
          folded={folded}
          onShowBuildIntro={
            isDocubrainCraftEnabled ? handleShowBuildIntro : undefined
          }
        />
      </div>
    ),
    [folded, isAdmin, isCurator, handleShowBuildIntro, isDocubrainCraftEnabled]
  );
    ...
    <>
      ...
      <SidebarLayouts.Header>
        <div className="flex flex-col">
          {newSessionButton}
          {searchChatsButton}
          {isDocubrainCraftEnabled && buildButton}
          {folded && FEATURES.AGENTS && moreAgentsButton}
