...
function buildItems(
  ...
  if (!isCurator && FEATURES.INTEGRATIONS) {
  ...
  // 5. Permissions
  if (FEATURES.ORG_MANAGEMENT && !isCurator) {
  ...
  } else if (FEATURES.ORG_MANAGEMENT && isCurator && enableEnterprise) {
    add(SECTIONS.PERMISSIONS, ADMIN_ROUTES.GROUPS);
  }

  // 6. Organization (admin only)
  if (!isCurator && FEATURES.ORG_MANAGEMENT) {
    if (FEATURES.BILLING) {
  ...
  // 7. Usage (admin only)
  if (!isCurator && FEATURES.USAGE_ANALYTICS) {
