...
export async function pinAgents(pinnedAgentIds: number[]) {
  // TODO: rename to agent — https://linear.app/docubrain-app/issue/ENG-3766
  const response = await fetch(`/api/user/pinned-assistants`, {
    ...
    body: JSON.stringify({
      ordered_assistant_ids: pinnedAgentIds, // TODO: rename to agent — https://linear.app/docubrain-app/issue/ENG-3766
    }),
  });
  if (!response.ok) {
