...
import DocubrainInitializingLoader from "@/components/DocubrainInitializingLoader";
import { DocubrainDocument, MinimalDocubrainDocument } from "@/lib/search/interfaces";
import {
  useSettingsContext,
  useVectorDbEnabled,
...
export default function AppPage({ firstMessage }: ChatPageProps) {
  ...
  const [presentingDocument, setPresentingDocument] =
    useState<MinimalDocubrainDocument | null>(null);
  ...
  // Only treat this as a true "no agent available" state when the fetched
  // agent list itself is empty. `liveAgent` can be temporarily undefined while
  // selection/default resolution catches up, which should not block the app.
  ...
  const [selectedDocuments, setSelectedDocuments] = useState<DocubrainDocument[]>(
    []
  );
  ...
  const handleSearchDocumentClick = useCallback(
    (doc: MinimalDocubrainDocument) => setPresentingDocument(doc),
    []
  );
  ...
  // Only show the "no agent" modal when agent management is enabled and the
  // fetched list is truly empty. In CE/local setups the default assistant may
  // still power chat even when the agents UI is feature-gated.
  if (FEATURES.AGENTS && noAgents && !isLoadingAgents) {
    return <NoAgentModal />;
  }
  ...
  if (!isReady) return <DocubrainInitializingLoader />;
