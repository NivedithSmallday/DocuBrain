...
export type DocubrainSortDirection = "none" | "ascending" | "descending";
export type DocubrainSelectionState = "none" | "partial" | "all";
...
/**
 * Convert a TanStack sort direction to a DocuBrain sort direction string.
 *
 ...
 */
export function toDocubrainSortDirection(
  dir: false | "asc" | "desc"
): DocubrainSortDirection {
...
interface UseDataTableReturn<TData extends RowData> {
  ...
  // Pagination (1-based, matching DocuBrain Footer)
  ...
  // Selection (pre-computed for DocuBrain Footer)
  /** Aggregate selection state for the current page. */
  selectionState: DocubrainSelectionState;
...
/**
 * Wraps TanStack `useReactTable` with DocuBrain-specific defaults and derived
...
export default function useDataTable<TData extends RowData>(
  ...
  const selectionState: DocubrainSelectionState = isAllPageRowsSelected
    ? "all"
