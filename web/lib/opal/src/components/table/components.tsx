...
import type {
  ...
  DocubrainColumnDef,
  DocubrainDataColumn,
  DocubrainQualifierColumn,
  DocubrainActionsColumn,
...
interface ProcessedColumns<TData> {
  ...
  qualifierColumn: DocubrainQualifierColumn<TData> | null;
  /** Map from column ID → DocubrainColumnDef for dispatch in render loops. */
  columnKindMap: Map<string, DocubrainColumnDef<TData>>;
}

function processColumns<TData>(
  columns: DocubrainColumnDef<TData>[],
  size: TableSize
  ...
  const columnKindMap = new Map<string, DocubrainColumnDef<TData>>();
  let qualifierColumn: DocubrainQualifierColumn<TData> | null = null;
...
export function Table<TData>(props: DataTableProps<TData>) {
  ...
  const qualifierColDef = columns.find(
    (c): c is DocubrainQualifierColumn<TData> => c.kind === "qualifier"
  );
                ...
                <TableRow key={headerGroup.id}>
                  {headerGroup.headers.map((header, headerIndex) => {
                    ...
                    if (colDef?.kind === "actions") {
                      const actionsDef = colDef as DocubrainActionsColumn<TData>;
                      return (
                    ...
                    const dataCol =
                      colDef?.kind === "data"
                        ? (colDef as DocubrainDataColumn<TData>)
                        : null;
                    ...
                    return (
                      <TableHead
                        ...
                        sorted={
                          canSort ? toDocubrainSortDirection(sortDir) : undefined
                        }
                        onSort={
                          canSort
                  ...
                  >
                    {row.getVisibleCells().map((cell) => {
                      ...
                      if (cellColDef?.kind === "qualifier") {
                        const qDef = cellColDef as DocubrainQualifierColumn<TData>;
