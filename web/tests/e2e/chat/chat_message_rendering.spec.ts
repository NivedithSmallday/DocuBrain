...
const SHORT_USER_MESSAGE = "What is DocuBrain?";
...
const LONG_USER_MESSAGE = `I've been evaluating several enterprise search and AI platforms for our organization, and I have a number of detailed questions about DocuBrain that I'd like to understand before we make a decision.

First, can you explain how DocuBrain handles document indexing across multiple data sources? We currently use Confluence, Google Drive, Slack, and GitHub, and we need to ensure that all of these can be indexed simultaneously without performance degradation.

Second, I'm interested in understanding the security model. Specifically, how does DocuBrain handle document-level permissions when syncing from sources that have their own ACL systems? Does it respect the original source permissions, or does it create its own permission layer?

Third, we have a requirement for real-time or near-real-time indexing. What is the typical latency between a document being updated in a source system and it becoming searchable in DocuBrain?
...
const SHORT_AI_RESPONSE =
  "DocuBrain is an open-source AI-powered enterprise search platform that connects to your company's documents, apps, and people.";

const LONG_AI_RESPONSE = `DocuBrain is an open-source Gen-AI and Enterprise Search platform designed to connect to your company's documents, applications, and people. Let me address each of your questions in detail.

## Document Indexing

DocuBrain uses a **connector-based architecture** where each data source has a dedicated connector. These connectors run as background workers and can index simultaneously without interfering with each other. The supported connectors include:
...
DocuBrain implements a **document-level permission system** that syncs with source ACLs. When documents are indexed, their permissions are preserved:

\`\`\`
Source Permission → DocuBrain ACL Sync → Query-time Filtering
\`\`\`
...
\`\`\`python
from docubrain import DocubrainClient

client = DocubrainClient(api_key="your-key")
...
> **Note**: DocuBrain supports both cloud and self-hosted deployments. The self-hosted option gives you full control over your data.
...
async function openChat(page: Page): Promise<void> {
  ...
  await page.waitForSelector("#docubrain-chat-input-textarea", { timeout: 15000 });
}
  ...
  test.describe(`Chat Message Rendering (${theme} mode)`, () => {
    ...
    test.describe("Short Messages", () => {
      test("short user message with short AI response renders correctly", async ({
        ...
        const userMessage = page.locator("#docubrain-human-message").first();
        await expect(userMessage).toContainText(SHORT_USER_MESSAGE);

        const aiMessage = page.getByTestId("docubrain-ai-message").first();
    ...
    test.describe("Markdown and Code Rendering", () => {
      test("AI response with tables and code blocks renders correctly", async ({
        ...
        await sendMessage(page, "Give me an overview of DocuBrain features");

        const aiMessage = page.getByTestId("docubrain-ai-message").first();
        await expect(aiMessage).toContainText("Key Features");
        await expect(aiMessage).toContainText("DocubrainClient");
    ...
    test.describe("Multi-Turn Conversation", () => {
      test("multi-turn conversation renders all messages correctly", async ({
        ...
        const responses = [
          SHORT_AI_RESPONSE,
          "Yes, DocuBrain supports over 30 data source connectors including Confluence, Google Drive, Slack, GitHub, Jira, Notion, and many more.",
          "To get started, you can deploy DocuBrain using Docker Compose with a single command. The setup takes about 5 minutes.",
        ];

        await mockChatEndpointSequence(page, responses);

        await sendMessage(page, SHORT_USER_MESSAGE);
        await expect(page.getByTestId("docubrain-ai-message").first()).toContainText(
          "open-source AI-powered"
        );

        await sendMessage(page, "What connectors does it support?");
        await expect(page.getByTestId("docubrain-ai-message")).toHaveCount(2, {
          timeout: 30000,
        });

        await sendMessage(page, "How do I get started?");
        await expect(page.getByTestId("docubrain-ai-message")).toHaveCount(3, {
          timeout: 30000,
        });

        const userMessages = page.locator("#docubrain-human-message");
      ...
      const WEB_SEARCH_DOCUMENTS: MockDocument[] = [
        {
          ...
          semantic_identifier: "DocuBrain Documentation - Getting Started",
          link: "https://docs.docubrain.app/getting-started",
          source_type: "web",
          blurb:
            "DocuBrain is an open-source enterprise search and AI platform. Deploy in minutes with Docker Compose.",
          is_internet: true,
        },
        {
          document_id: "web-doc-2",
          semantic_identifier: "DocuBrain GitHub Repository",
          link: "https://github.com/docubrain-dot-app/docubrain",
          source_type: "web",
          blurb:
        ...
        },
        {
          ...
          source_type: "web",
          blurb:
            "Comparing top enterprise search platforms including DocuBrain, Glean, and Coveo.",
          is_internet: true,
        },
      ];

      const WEB_SEARCH_RESPONSE = `Based on my web search, here's what I found about DocuBrain:

DocuBrain is an open-source enterprise search and AI platform that can be deployed in minutes using Docker Compose [[D1]](https://docs.docubrain.app/getting-started). The project is hosted on GitHub and is MIT licensed for the community edition, with over 30 connectors available [[D2]](https://github.com/docubrain-dot-app/docubrain).

In comparisons with other enterprise search platforms, DocuBrain stands out for its open-source nature and self-hosted deployment option [[D3]](https://example.com/enterprise-search-comparison). Unlike proprietary alternatives, you maintain full control over your data and infrastructure.

Key advantages include:

- **Self-hosted**: Deploy on your own infrastructure
- **Open source**: Full visibility into the codebase [[D2]](https://github.com/docubrain-dot-app/docubrain)
- **Quick setup**: Get running in under 5 minutes [[D1]](https://docs.docubrain.app/getting-started)
- **Extensible**: 30+ pre-built connectors with custom connector support`;
      ...
      test("web search response with citations renders correctly", async ({
        ...
        await page.route("**/api/chat/send-chat-message", async (route) => {
          await route.fulfill({
            ...
            body: buildMockSearchStream({
              ...
              queries: ["DocuBrain enterprise search platform overview"],
        ...
        await sendMessage(page, "Search the web for information about DocuBrain");
      ...
      test("internal document search response renders correctly", async ({
        ...
        const internalDocs: MockDocument[] = [
          ...
          },
          {
            ...
            blurb:
              "DocuBrain platform architecture document covering microservices, data flow, and deployment topology.",
            is_internet: false,
          },
        ];
