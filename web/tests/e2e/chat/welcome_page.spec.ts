  ...
  test.describe(`Welcome page — /app (${theme} mode)`, () => {
    ...
    test("full page visual snapshot", async ({ page }) => {
      ...
      await expectScreenshot(page, {
        ...
        hide: ['[data-testid="docubrain-logo"]'], // greeting text is random, hide to prevent size variation
      });
    });
    ...
    test("input bar element snapshot", async ({ page }) => {
      const inputBar = page.locator("#docubrain-chat-input");
    ...
    test("displays greeting from default agent", async ({ page }) => {
      const greetingContainer = page.getByTestId("docubrain-logo");
    ...
    test("chat input is visible and focusable", async ({ page }) => {
      const textarea = page.locator("#docubrain-chat-input-textarea");
    ...
    test.skip("send button is visible in the input bar", async ({ page }) => {
      const sendButton = page.locator("#docubrain-chat-input-send-button");
