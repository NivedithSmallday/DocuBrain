...
test.describe("Web Search Provider Disconnect", () => {
  ...
  test.describe("Web Crawler (Content) Providers", () => {
    ...
    test("should not show disconnect for DocuBrain Web Crawler (built-in)", async ({
      page,
    }) => {
      ...
      const docubrainCard = findProviderCard(page, "DocuBrain Web Crawler");
      await docubrainCard.waitFor({ state: "visible", timeout: 10000 });

      const disconnectButton = docubrainCard.getByRole("button", {
        name: "Disconnect DocuBrain Web Crawler",
      });
      await expect(disconnectButton).not.toBeVisible();
    });
  });
