...
test.describe("Web Content Provider Configuration", () => {
  ...
  test.describe("Firecrawl Provider", () => {
    ...
    test("should switch back to DocuBrain Web Crawler from Firecrawl", async ({
      page,
    }) => {
      ...
      console.log(
        "[web-content-test] Firecrawl configured, now switching to DocuBrain Web Crawler..."
      );

      // Switch to DocuBrain Web Crawler
      const docubrainCrawlerCard = findProviderCard(page, "DocuBrain Web Crawler");
      await docubrainCrawlerCard.waitFor({ state: "visible", timeout: 10000 });

      const docubrainSetDefault = docubrainCrawlerCard.getByRole("button", {
        name: "Set as Default",
      });

      if (await docubrainSetDefault.isVisible()) {
        await docubrainSetDefault.click();
        await page.waitForLoadState("networkidle");
      }

      await expect(
        docubrainCrawlerCard.getByRole("button", { name: "Current Crawler" })
      ).toBeVisible({ timeout: 15000 });

      console.log("[web-content-test] Switched back to DocuBrain Web Crawler");
    });
  });
});
