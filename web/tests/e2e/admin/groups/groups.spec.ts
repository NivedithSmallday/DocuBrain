 ...
 * DocubrainApiClient for setup and cleaned up in afterAll/afterEach.
 */

import { test, expect } from "./fixtures";
import type { DocubrainApiClient } from "@tests/e2e/utils/docubrainApiClient";
...
async function withApiContext(
  browser: Browser,
  fn: (api: DocubrainApiClient) => Promise<void>
): Promise<void> {
  const context = await browser.newContext({
    storageState: "admin_auth.json",
  });
  try {
    const { DocubrainApiClient } = await import("@tests/e2e/utils/docubrainApiClient");
    const api = new DocubrainApiClient(context.request);
    await fn(api);
  } finally {
    await context.close();
  }
...
test.describe("Groups page — sync @lite", () => {
  test.beforeAll(async ({ browser }) => {
    ...
    try {
      const { DocubrainApiClient } = await import("@tests/e2e/utils/docubrainApiClient");
      const client = new DocubrainApiClient(context.request);
