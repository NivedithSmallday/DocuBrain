...
test.describe("Assistant Creation and Edit Verification", () => {
  ...
  test.describe("User Files Only", () => {
    ...
    test.afterAll(async ({ browser }: { browser: Browser }) => {
      if (userFilesAssistantId !== null) {
        ...
        const cleanupClient = new DocubrainApiClient(page.request);
  ...
  test.describe("With Knowledge", () => {
    ...
    test("should create and edit assistant with Knowledge enabled", async ({
      ...
      const docubrainApiClient = new DocubrainApiClient(page.request);
      ccPairId = await docubrainApiClient.createFileConnector("Test Connector");
      documentSetId = await docubrainApiClient.createDocumentSet(
        "Test Document Set",
        [ccPairId]
      );
