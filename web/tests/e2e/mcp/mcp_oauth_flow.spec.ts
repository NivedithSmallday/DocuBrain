...
async function completeOauthFlow(
  ...
  const waitForUrlOrRedirect = async (
    ...
    try {
      ...
      if (predicate(page.url())) {
        logOauthEvent(
          page,
          `${description} satisfied (after timeout) in ${Date.now() - waitStart
          }ms`
        );
        return;
      }
  ...
  logOauthEvent(
    page,
    `Initial post-return load wait completed in ${Date.now() - waitForReturnStart
    }ms`
  );
  ...
  logOauthEvent(
    page,
    `Post-return domcontentloaded wait finished in ${Date.now() - returnLoadStart
    }ms`
  );
...
test.describe("MCP OAuth flows", () => {
  ...
  test.beforeAll(async ({ browser }, workerInfo) => {
    ...
    const adminClient = new DocubrainApiClient(adminPage.request);
    try {
  ...
  test("Admin can configure OAuth MCP server and use tools end-to-end", async ({
    ...
    const adminApiClient = new DocubrainApiClient(page.request);
  ...
  test("Curator flow with access isolation", async ({
    ...
    const curatorApiClient = new DocubrainApiClient(page.request);
