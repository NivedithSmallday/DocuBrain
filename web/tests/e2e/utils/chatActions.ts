...
export async function verifyDefaultAgentIsChosen(page: Page) {
  await expect(page.getByTestId("docubrain-logo")).toBeVisible({ timeout: 5000 });
}
...
export async function sendMessage(page: Page, message: string) {
  ...
  const existingMessageCount = await page
    .locator('[data-testid="docubrain-ai-message"]')
    .count();

  await page.locator("#docubrain-chat-input-textarea").click();
  await page.locator("#docubrain-chat-input-textarea").fill(message);
  await page.locator("#docubrain-chat-input-send-button").click();

  // Wait for a NEW AI message to appear (count should increase)
  await expect(page.locator('[data-testid="docubrain-ai-message"]')).toHaveCount(
