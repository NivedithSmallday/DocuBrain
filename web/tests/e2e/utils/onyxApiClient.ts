...
/**
 * API Client for DocuBrain backend operations in E2E tests.
 *
 * Provides a type-safe, abstracted interface for interacting with the DocuBrain backend API.
 ...
 * const client = new DocubrainApiClient(page.request);
 *
 ...
 * const client = new DocubrainApiClient(ctx, baseURL);
 * ```
 *
 ...
 *                  is authoritative).
 */
export class DocubrainApiClient {
  ...
  private async handleResponseSoft(
    ...
    if (!response.ok()) {
      const errorText = await response.text();
      console.error(
        `[DocubrainApiClient] ${errorMessage}: ${response.status()} - ${errorText}`
      );
      return false;
    }
    return true;
   ...
   *
   * @param message - The message to log (will be prefixed with "[DocubrainApiClient]")
   */
  private log(message: string): void {
    console.log(`[DocubrainApiClient] ${message}`);
  }
