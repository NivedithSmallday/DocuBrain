...
async function deleteAllProviders(client: DocubrainApiClient): Promise<void> {
...
async function createFreshAdmin(
  ...
  const adminClient = new DocubrainApiClient(page.request);
...
test.describe("Onboarding Flow @exclusive", () => {
  test.describe("Scenario 1: Admin WITHOUT LLM providers", () => {
    ...
    test("chat input bar is disabled during onboarding", async ({ page }) => {
      ...
      const chatInput = page.locator("#docubrain-chat-input");
  ...
  test.describe("Scenario 2: Admin WITH LLM providers", () => {
    ...
    test("chat input bar is enabled", async ({ page }) => {
      ...
      await expect(page.locator("#docubrain-chat-input")).toBeVisible({
        timeout: 15000,
      });
