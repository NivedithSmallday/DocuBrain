...
async function waitForServer(baseURL: string): Promise<void> {
  ...
  while (Date.now() < deadline) {
    ...
    if (!warned && Date.now() - startTime >= PREFLIGHT_WARN_AFTER_MS) {
      warned = true;
      console.warn(
        `[global-setup] ⚠ Still waiting for server after ${PREFLIGHT_WARN_AFTER_MS / 1000
        }s.\n` +
  ...
  throw new Error(
    `DocuBrain is not running at ${baseURL}. ` +
    `Timed out after ${PREFLIGHT_TIMEOUT_MS / 1000
...
async function globalSetup(config: FullConfig) {
  ...
  try {
    const client = new DocubrainApiClient(adminCtx, baseURL);
