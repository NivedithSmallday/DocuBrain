...
from docubrain.connectors.slack.models import ChannelType
...
def test_slack_prune(
    ...
    docubrain_doc_message_strings = DocumentSearchManager.search_documents(
        query="favorite number",
        user_performing_action=admin_user,
    )
    print(
        "\ntop_documents content before deleting for admin: ",
        docubrain_doc_message_strings,
    )

    # Ensure admin user can see all messages
    assert public_message in docubrain_doc_message_strings
    assert private_message in docubrain_doc_message_strings
    assert message_to_delete in docubrain_doc_message_strings
    ...
    assert public_message in docubrain_doc_message_strings
    assert private_message in docubrain_doc_message_strings
    assert message_to_delete not in docubrain_doc_message_strings
