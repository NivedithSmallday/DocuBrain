...
from docubrain.main import fetch_versioned_implementation
...
def client() -> Generator[TestClient, Any, None]:
    ...
    app: FastAPI = fetch_versioned_implementation(
        module="docubrain.main", attribute="get_application"
    )()
...
def test_versions_endpoint(client: TestClient) -> None:
    ...
    assert STABLE_VERSION_PATTERN.match(
        stable["docubrain"]
    ), f"Stable version {stable['docubrain']} doesn't match pattern v(number).(number).(number)"

    # Verify dev configuration
    dev = data["dev"]
    assert "docubrain" in dev
    ...
    assert DEV_VERSION_PATTERN.match(
        dev["docubrain"]
    ), f"Dev version {dev['docubrain']} doesn't match pattern v(number).(number).(number)-beta.(number)"

    # Verify migration configuration
    migration = data["migration"]
    assert "docubrain" in migration
    assert "relational_db" in migration
    assert "index" in migration
    assert "nginx" in migration

    # Verify migration has expected values
    assert migration["docubrain"] == "airgapped-intfloat-nomic-migration"
    ...
    assert stable["docubrain"] != dev["docubrain"], "Stable and dev versions should be different"
