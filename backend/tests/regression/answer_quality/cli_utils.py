...
def start_docker_compose(
    ...
    command = (
        f"docker compose -f docker-compose.search-testing.yml -p docubrain-{env_name} up -d"
    )
    command += " --build"
...
def cleanup_docker(env_name: str) -> None:
    ...
    project_name = f"docubrain{env_name}"
...
if __name__ == "__main__":
    ...
    If no environment_name is indicated, will just clean up all docubrain docker containers/volumes/networks
