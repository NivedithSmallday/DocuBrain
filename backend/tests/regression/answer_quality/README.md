...
4. Set up the PYTHONPATH permanently:
   ...
   ```
   export PYTHONPATH=$PYTHONPATH:/path/to/docubrain/backend
   ```
   Replace `/path/to/docubrain` with the actual path to your DocuBrain repository.
   After adding this line, restart your terminal or run `source ~/.bashrc` (or the appropriate config file) to apply the changes.
5. Navigate to DocuBrain repo:

```
cd path/to/docubrain
```
