...
# add docubrain/backend to path (since this isn't done automatically when running as a script)
current_dir = Path(__file__).parent
docubrain_dir = current_dir.parent.parent.parent.parent
sys.path.append(str(docubrain_dir / "backend"))

# load env before app_config loads (since env doesn't get loaded when running as a script)
env_path = docubrain_dir / ".vscode" / ".env"
...
class SearchAnswerAnalyzer:
    ...
    def _perform_search(self, query: str) -> OneshotQAResult:
        """Perform a document search query against the DocuBrain API and time it."""
        ...
        try:
            ...
            if os.environ.get("DOCUBRAIN_API_KEY"):
                headers["Authorization"] = f"Bearer {os.environ.get('DOCUBRAIN_API_KEY')}"
            ...
            search_docs = result.get("search_docs") if isinstance(result, dict) else None
            if search_docs:
                top_documents = [
                    SavedSearchDoc.from_search_doc(SearchDoc.model_validate(doc))
                    for doc in search_docs[: self.config.max_search_results]
                ]
                return OneshotQAResult(
                    time_taken=time_taken,
                    top_documents=top_documents,
                    answer=result.get("answer"),  # may be absent for search-only responses
                )
...
def run_search_eval(
    ...
    # check docubrain api key is set (auth is always required)
    if not os.environ.get("DOCUBRAIN_API_KEY"):
        raise RuntimeError(
            "DOCUBRAIN_API_KEY is required. Please create one in the admin panel and add it to the root .vscode/.env file."
        )

    # check docubrain is running
    try:
        ...
        raise RuntimeError(f"Could not connect to DocuBrain API: {e}")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        help="Base URL of the DocuBrain API server (default: %(default)s).",
    )
    parser.add_argument(
        "-s",
        "--search_only",
