...
from docubrain.connectors.airtable.airtable_connector import AirtableConnector
...
def test_airtable_connector_basic(
    ...
    expected_docs = [
        create_test_document(
            ...
            assignee="Chris Weaver (chris@docubrain.app)",
            submitted_by="Chris Weaver (chris@docubrain.app)",
...
def test_airtable_connector_index_all(
    ...
    The test token has access to one base ("DocuBrain") with three tables:
    ...
    assert (
        ...
        == "DocuBrain > Tickets: Slow Internet"
    )
    assert (
        docs_by_id["airtable__rec5SgUDcHXcBc8kS"].semantic_identifier
        == "DocuBrain > Support Categories: Software Development"
    )
    assert (
    ...
    assert hierarchy["source_path"] == ["DocuBrain", "Tickets"]
    assert hierarchy["base_id"] == airtable_config.base_id
    assert hierarchy["base_name"] == "DocuBrain"
