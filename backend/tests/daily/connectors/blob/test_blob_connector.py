...
from docubrain.configs.constants import BlobType
from docubrain.connectors.blob.connector import BlobStorageConnector
...
@pytest.mark.parametrize(
    "blob_connector", [(BlobType.S3, "docubrain-connector-tests")], indirect=True
)
def test_blob_s3_connector(
    ...
    for doc in all_docs:
        ...
        if file_extension in DocubrainFileExtensions.TEXT_AND_DOCUMENT_EXTENSIONS:
...
@pytest.mark.parametrize(
    "blob_connector",
    [(BlobType.R2, "docubrain-daily-connector-tests", {"european_residency": True})],
    indirect=True,
)
...
@pytest.mark.parametrize(
    "blob_connector", [(BlobType.GOOGLE_CLOUD_STORAGE, "docubrain-test-1")], indirect=True
)
