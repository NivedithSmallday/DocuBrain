...
from docubrain.connectors.web.connector import WEB_CONNECTOR_VALID_SETTINGS
from docubrain.connectors.web.connector import WebConnector
...
def test_web_connector_recursive_www_redirect() -> None:
    # Check that https://docubrain.app can be recursed if re-directed to www.docubrain.app
    # Run in thread pool to avoid conflict with pytest-asyncio's event loop
    def _run_connector() -> list[Document]:
        connector = WebConnector(
            base_url="https://docubrain.app",
