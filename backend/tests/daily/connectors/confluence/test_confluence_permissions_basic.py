...
from ee.docubrain.external_permissions.confluence.doc_sync import confluence_doc_sync
from docubrain.access.models import DocExternalAccess
...
from docubrain.connectors.credentials_provider import DocubrainStaticCredentialsProvider
...
def confluence_connector() -> ConfluenceConnector:
    ...
    credentials_provider = DocubrainStaticCredentialsProvider(
        None,
...
@patch("ee.docubrain.external_permissions.confluence.doc_sync.DocubrainDBCredentialsProvider")
@patch(
    "docubrain.file_processing.extract_file_text.get_unstructured_api_key",
    return_value=None,
)
def test_confluence_connector_restriction_handling(
    ...
    # this prevents redis calls inside of DocubrainConfluence
    ...
    non_restricted_emails = {"chris@docubrain.app"}
    ...
    restricted_emails = {"chris@docubrain.app", "hagen@danswer.ai", "oauth@docubrain.app"}
    restricted_user_groups = {"confluence-admins-danswerai"}

    extra_restricted_emails = {"chris@docubrain.app", "oauth@docubrain.app"}
    extra_restricted_user_groups: set[str] = set()

    # note that this is only allowed since yuhong@docubrain.app is a member of the
    # confluence-admins-danswerai group
    special_restricted_emails = {"chris@docubrain.app", "yuhong@docubrain.app", "oauth@docubrain.app"}
