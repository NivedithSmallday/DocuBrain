...
from docubrain.server.metrics.indexing_task_metrics import _connector_cache
from docubrain.server.metrics.indexing_task_metrics import _indexing_start_times
from docubrain.server.metrics.indexing_task_metrics import ConnectorInfo
from docubrain.server.metrics.indexing_task_metrics import INDEXING_TASK_COMPLETED
from docubrain.server.metrics.indexing_task_metrics import INDEXING_TASK_DURATION
from docubrain.server.metrics.indexing_task_metrics import INDEXING_TASK_STARTED
from docubrain.server.metrics.indexing_task_metrics import on_indexing_task_postrun
from docubrain.server.metrics.indexing_task_metrics import on_indexing_task_prerun
...
def _mock_db_lookup(
    ...
    session_patch = patch("docubrain.db.engine.sql_engine.get_session_with_current_tenant")
    cc_pair_patch = patch(
        "docubrain.db.connector_credential_pair.get_connector_credential_pair_from_id",
...
class TestIndexingTaskPrerun:
    ...
    def test_db_lookup_on_cache_miss(self) -> None:
        ...
        with (
            patch(
                "docubrain.server.metrics.indexing_task_metrics._resolve_connector"
            ) as mock_resolve,
        ):
...
class TestResolveConnector:
    def test_failed_lookup_not_cached(self) -> None:
        ...
        try:
            with (
                ...
                patch(
                    "docubrain.db.connector_credential_pair"
                    ".get_connector_credential_pair_from_id",
                    return_value=None,
                ),
            ):
                from docubrain.server.metrics.indexing_task_metrics import _resolve_connector
    ...
    def test_exception_not_cached(self) -> None:
        ...
        try:
            with (
                patch(
                    "docubrain.db.engine.sql_engine.get_session_with_current_tenant",
                    side_effect=Exception("DB down"),
                ),
            ):
                from docubrain.server.metrics.indexing_task_metrics import _resolve_connector
