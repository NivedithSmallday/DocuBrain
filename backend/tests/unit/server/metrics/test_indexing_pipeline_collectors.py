...
from docubrain.server.metrics.indexing_pipeline import ConnectorHealthCollector
from docubrain.server.metrics.indexing_pipeline import IndexAttemptCollector
from docubrain.server.metrics.indexing_pipeline import QueueDepthCollector
...
def _mock_broker_client() -> Iterator[None]:
    ...
    with patch(
        "docubrain.background.celery.celery_redis.celery_get_broker_client",
...
class TestQueueDepthCollector:
    ...
    def test_collects_queue_depths(self) -> None:
        ...
        with (
            patch(
                "docubrain.server.metrics.indexing_pipeline.celery_get_queue_length",
                return_value=5,
            ),
            patch(
                "docubrain.server.metrics.indexing_pipeline.celery_get_unacked_task_ids",
        ...
        assert depth_family.name == "docubrain_queue_depth"
        assert len(depth_family.samples) > 0
        for sample in depth_family.samples:
            assert sample.value == 5

        assert unacked_family.name == "docubrain_queue_unacked"
        ...
        assert age_family.name == "docubrain_queue_oldest_task_age_seconds"
...
class TestIndexAttemptCollector:
    ...
    @patch("docubrain.db.engine.tenant_utils.get_all_tenant_ids")
    @patch("docubrain.db.engine.sql_engine.get_session_with_current_tenant")
    def test_collects_index_attempts(
        ...
        assert families[0].name == "docubrain_index_attempts_active"
...
class TestConnectorHealthCollector:
    ...
    def test_collects_connector_health(
        ...
        staleness = next(
            f for f in families if f.name == "docubrain_connector_last_success_age_seconds"
        )
        assert len(staleness.samples) == 1
        assert staleness.samples[0].value == pytest.approx(7200, abs=5)

        error_state = next(
            f for f in families if f.name == "docubrain_connector_in_error_state"
        )
        assert error_state.samples[0].value == 1.0

        by_status = next(f for f in families if f.name == "docubrain_connectors_by_status")
        ...
        error_total = next(
            f for f in families if f.name == "docubrain_connectors_in_error_total"
        )
