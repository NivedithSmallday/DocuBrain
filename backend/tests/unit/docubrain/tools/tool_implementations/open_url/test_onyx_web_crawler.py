...
import docubrain.tools.tool_implementations.open_url.docubrain_web_crawler as crawler_module
...
from docubrain.tools.tool_implementations.open_url.docubrain_web_crawler import DocubrainWebCrawler
...
def test_fetch_url_pdf_exceeds_size_limit(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    crawler = DocubrainWebCrawler(max_pdf_size_bytes=100)
    response = FakeResponse(
        status_code=200,
        headers={"Content-Type": "application/pdf"},
...
def test_fetch_url_pdf_within_size_limit(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    crawler = DocubrainWebCrawler(max_pdf_size_bytes=500)
    response = FakeResponse(
        status_code=200,
        headers={"Content-Type": "application/pdf"},
...
def test_fetch_url_html_exceeds_size_limit(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    crawler = DocubrainWebCrawler(max_html_size_bytes=50)
...
def test_fetch_url_html_within_size_limit(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    crawler = DocubrainWebCrawler(max_html_size_bytes=500)
...
class TestParallelExecution:
    ...
    @patch("docubrain.tools.tool_implementations.open_url.docubrain_web_crawler.ssrf_safe_get")
...
class TestTupleTimeout:
    ...
    def test_custom_tuple_timeout(self, mock_get: MagicMock) -> None:
        ...
        crawler = DocubrainWebCrawler(timeout_seconds=30, connect_timeout_seconds=3)
        crawler.contents(["http://example.com"])
