...
APP_CONFIGS_MODULE = "docubrain.configs.app_configs"
FILE_READER_MODULE = "docubrain.tools.tool_implementations.file_reader.file_reader_tool"
...
class TestSearchToolAvailability:
    ...
    @patch("docubrain.db.connector.check_user_files_exist", return_value=True)
    @patch(
...
class TestValidateVectorDbKnowledge:
    @patch(
        "docubrain.server.features.persona.api.DISABLE_VECTOR_DB",
        True,
    )
    def test_rejects_document_set_ids(self) -> None:
        from fastapi import HTTPException

        from docubrain.server.features.persona.api import _validate_vector_db_knowledge
