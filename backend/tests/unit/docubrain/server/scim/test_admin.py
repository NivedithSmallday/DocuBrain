...
from ee.docubrain.db.scim import ScimDAL
from ee.docubrain.server.enterprise_settings.api import create_scim_token
from ee.docubrain.server.enterprise_settings.api import get_active_scim_token
from ee.docubrain.server.scim.models import ScimTokenCreate
...
def _make_token(token_id: int, name: str, *, is_active: bool = True) -> ScimToken:
    return ScimToken(
        ...
        name=name,
        hashed_token="h" * 64,
        token_display="docubrain_scim_****abcd",
        is_active=is_active,
        created_by_id=uuid4(),
...
class TestCreateToken:
    @patch("ee.docubrain.server.enterprise_settings.api.generate_scim_token")
