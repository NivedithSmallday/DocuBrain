...
class TestGetOllamaAvailableModels:
    ...
    def test_returns_model_list(
        ...
        from docubrain.server.manage.llm.api import get_ollama_available_models

        mock_session = MagicMock()

        with patch("docubrain.server.manage.llm.api.httpx") as mock_httpx:
    ...
    def test_syncs_to_db_when_provider_name_specified(
        ...
        with (
            patch("docubrain.server.manage.llm.api.httpx") as mock_httpx,
            patch(
                "docubrain.db.llm.fetch_existing_llm_provider", return_value=mock_provider
            ),
        ):
            mock_get_response = MagicMock()
            mock_get_response.json.return_value = mock_ollama_tags_response
...
class TestGetOpenRouterAvailableModels:
    ...
    def test_returns_model_list(self, mock_openrouter_response: dict) -> None:
        """Test that endpoint returns properly formatted model list."""
        from docubrain.server.manage.llm.api import get_openrouter_available_models

        mock_session = MagicMock()

        with patch("docubrain.server.manage.llm.api.httpx.get") as mock_get:
    ...
    def test_syncs_to_db_when_provider_name_specified(
        ...
        with (
            patch("docubrain.server.manage.llm.api.httpx.get") as mock_get,
            patch(
                "docubrain.db.llm.fetch_existing_llm_provider", return_value=mock_provider
            ),
        ):
...
class TestGetLMStudioAvailableModels:
    ...
    def test_returns_model_list(self, mock_lm_studio_response: dict) -> None:
        ...
        from docubrain.server.manage.llm.api import get_lm_studio_available_models
    ...
    def test_falls_back_to_stored_api_key(self) -> None:
        ...
        with (
            patch("docubrain.server.manage.llm.api.httpx") as mock_httpx,
            patch(
                "docubrain.server.manage.llm.api.fetch_existing_llm_provider",
    ...
    def test_raises_on_empty_models(self) -> None:
        ...
        with patch("docubrain.server.manage.llm.api.httpx") as mock_httpx:
            ...
            request = LMStudioModelsRequest(api_base="http://localhost:1234")
            with pytest.raises(DocubrainError):
                get_lm_studio_available_models(request, MagicMock(), mock_session)
...
class TestGetLitellmAvailableModels:
    ...
    def test_returns_model_list(self, mock_litellm_response: dict) -> None:
        """Test that endpoint returns properly formatted model list."""
        from docubrain.server.manage.llm.api import get_litellm_available_models
    ...
    def test_empty_data_raises_docubrain_error(self) -> None:
        """Test that empty model list raises DocubrainError."""
        from docubrain.server.manage.llm.api import get_litellm_available_models
        ...
        with patch("docubrain.server.manage.llm.api.httpx.get") as mock_get:
            ...
            with pytest.raises(DocubrainError, match="No models found"):
                get_litellm_available_models(request, MagicMock(), mock_session)

    def test_missing_data_key_raises_docubrain_error(self) -> None:
        """Test that response without 'data' key raises DocubrainError."""
        from docubrain.server.manage.llm.api import get_litellm_available_models
    ...
    def test_all_entries_unparseable_raises_docubrain_error(self) -> None:
        """Test that DocubrainError is raised when all entries fail to parse."""
        from docubrain.server.manage.llm.api import get_litellm_available_models
        ...
        with patch("docubrain.server.manage.llm.api.httpx.get") as mock_get:
            ...
            with pytest.raises(DocubrainError, match="No compatible models"):
                get_litellm_available_models(request, MagicMock(), mock_session)
    ...
    def test_connection_failure_raises_docubrain_error(self) -> None:
        """Test that connection failures are wrapped in DocubrainError."""
        from docubrain.server.manage.llm.api import get_litellm_available_models
        ...
        with patch("docubrain.server.manage.llm.api.httpx.get") as mock_get:
            ...
            with pytest.raises(DocubrainError, match="Failed to fetch LiteLLM proxy models"):
                get_litellm_available_models(request, MagicMock(), mock_session)

    def test_401_raises_authentication_error(self) -> None:
        """Test that a 401 response raises DocubrainError with authentication message."""
        from docubrain.server.manage.llm.api import get_litellm_available_models
        ...
        with patch("docubrain.server.manage.llm.api.httpx.get") as mock_get:
            ...
            with pytest.raises(DocubrainError, match="Authentication failed"):
                get_litellm_available_models(request, MagicMock(), mock_session)

    def test_404_raises_not_found_error(self) -> None:
        """Test that a 404 response raises DocubrainError with endpoint not found message."""
        from docubrain.server.manage.llm.api import get_litellm_available_models
        ...
        with patch("docubrain.server.manage.llm.api.httpx.get") as mock_get:
            ...
            with pytest.raises(DocubrainError, match="endpoint not found"):
                get_litellm_available_models(request, MagicMock(), mock_session)
...
class TestGetBifrostAvailableModels:
    ...
    def test_returns_model_list(self, mock_bifrost_response: dict) -> None:
        ...
        from docubrain.server.manage.llm.api import get_bifrost_available_models
    ...
    def test_request_failure_is_logged_and_wrapped(self) -> None:
        """Test that request-layer failures are logged before raising DocubrainError."""
        from docubrain.server.manage.llm.api import get_bifrost_available_models
        ...
        with (
            patch("docubrain.server.manage.llm.api.httpx.get") as mock_get,
            patch("docubrain.server.manage.llm.api.logger.warning") as mock_warning,
        ):
            mock_get.side_effect = httpx.ConnectError(
                "Connection refused", request=MagicMock()
            )
            ...
            request = BifrostModelsRequest(api_base="https://bifrost.example.com")
            with pytest.raises(DocubrainError, match="Failed to fetch Bifrost models"):
                get_bifrost_available_models(request, MagicMock(), mock_session)
