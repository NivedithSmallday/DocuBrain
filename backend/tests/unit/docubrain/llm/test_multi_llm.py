...
from docubrain.llm.multi_llm import LitellmLLM
...
def test_openai_auto_reasoning_effort_maps_to_medium() -> None:
    ...
    with (
        ...
        patch("docubrain.llm.multi_llm.model_is_reasoning_model", return_value=True),
        patch("docubrain.llm.multi_llm.is_true_openai_model", return_value=True),
    ):
        mock_completion.return_value = []
...
def test_openai_chat_omits_reasoning_params() -> None:
    ...
    with (
        patch("litellm.completion") as mock_completion,
        patch(
            "docubrain.llm.multi_llm.model_is_reasoning_model", return_value=True
        ) as mock_is_reasoning,
        patch(
            "docubrain.llm.multi_llm.is_true_openai_model", return_value=True
        ) as mock_is_openai,
    ):
        mock_stream_chunks = [
            litellm.ModelResponse(
...
def test_user_identity_metadata_enabled(default_multi_llm: LitellmLLM) -> None:
    with (
        patch("litellm.completion") as mock_completion,
        patch("docubrain.llm.utils.SEND_USER_METADATA_TO_LLM_PROVIDER", True),
    ):
        mock_stream_chunks = [
            litellm.ModelResponse(
                id="chatcmpl-123",
...
def test_user_identity_metadata_disabled_omits_identity(
    ...
    with (
        patch("litellm.completion") as mock_completion,
        patch("docubrain.llm.utils.SEND_USER_METADATA_TO_LLM_PROVIDER", False),
    ):
        mock_stream_chunks = [
            litellm.ModelResponse(
                id="chatcmpl-123",
...
def test_multithreaded_invoke_without_custom_config_skips_env_lock() -> None:
    ...
    """
    from docubrain.llm import multi_llm as multi_llm_module
...
def test_messages_contain_tool_content_with_tool_role() -> None:
    from docubrain.llm.multi_llm import _messages_contain_tool_content
...
def test_strip_tool_content_converts_assistant_tool_calls_to_text() -> None:
    from docubrain.llm.multi_llm import _strip_tool_content_from_messages
...
def test_prompt_contains_tool_call_history_true() -> None:
    from docubrain.llm.multi_llm import _prompt_contains_tool_call_history
...
def test_bedrock_claude_drops_thinking_when_thinking_blocks_missing() -> None:
    ...
    messages: LanguageModelInput = [
        ...
        docubrain.llm.models.ToolMessage(
            content="22°C sunny",
            tool_call_id="tc_1",
        ),
    ]
