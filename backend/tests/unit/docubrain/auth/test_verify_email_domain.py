...
from docubrain.auth.users import verify_email_domain
...
def test_verify_email_domain_allows_plus_for_docubrain_app(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setattr(users, "VALID_EMAIL_DOMAINS", [], raising=False)
    monkeypatch.setattr(users, "AUTH_TYPE", AuthType.CLOUD, raising=False)

    # Should not raise for docubrain.app domain
    verify_email_domain("user+tag@docubrain.app")
