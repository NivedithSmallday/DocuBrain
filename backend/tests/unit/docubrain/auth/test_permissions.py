"""
Unit tests for docubrain.auth.permissions — pure logic and FastAPI dependency.
"""
...
from docubrain.auth.permissions import ALL_PERMISSIONS
...
from docubrain.auth.permissions import resolve_effective_permissions
...
class TestRequirePermission:
    ...
    async def test_missing_permission_raises(self) -> None:
        ...
        with pytest.raises(DocubrainError) as exc_info:
            await dep(user=user)
        assert exc_info.value.error_code == DocubrainErrorCode.INSUFFICIENT_PERMISSIONS
