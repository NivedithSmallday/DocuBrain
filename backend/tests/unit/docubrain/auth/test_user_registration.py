...
from docubrain.auth.users import UserManager
...
class TestDisposableEmailValidation:
    ...
    @pytest.mark.asyncio
    @patch("docubrain.auth.users.is_disposable_email")
    @patch("docubrain.auth.users.fetch_ee_implementation_or_noop")
    @patch("docubrain.auth.users.get_async_session_context_manager")
    @patch("docubrain.auth.users.get_user_count", new_callable=AsyncMock)
    ...
    @pytest.mark.asyncio
    @patch("docubrain.auth.users.is_disposable_email")
    @patch("docubrain.auth.users.verify_email_domain")
    @patch("docubrain.auth.users.fetch_ee_implementation_or_noop")
    @patch("docubrain.auth.users.get_async_session_context_manager")
    @patch("docubrain.auth.users.get_user_count", new_callable=AsyncMock)
    @patch("docubrain.auth.users.MULTI_TENANT", False)
...
class TestMultiTenantInviteLogic:
    ...
    @patch("docubrain.auth.users.SQLAlchemyUserAdminDB")
    @patch("docubrain.auth.users.is_disposable_email", return_value=False)
    @patch("docubrain.auth.users.verify_email_domain")
    @patch("docubrain.auth.users.fetch_ee_implementation_or_noop")
    @patch("docubrain.auth.users.get_async_session_context_manager")
    @patch("docubrain.auth.users.get_user_count", new_callable=AsyncMock)
    @patch("docubrain.auth.users.verify_email_is_invited")
    @patch("docubrain.auth.users.MULTI_TENANT", True)
    @patch("docubrain.auth.users.CURRENT_TENANT_ID_CONTEXTVAR")
    @pytest.mark.asyncio
...
class TestSAMLOIDCBehavior:
    ...
    @patch("docubrain.auth.users.get_invited_users")
    @patch("docubrain.auth.users.workspace_invite_only_enabled", return_value=True)
    @patch("docubrain.auth.users.AUTH_TYPE")
    def test_sso_bypasses_whitelist(
        ...
        with patch("docubrain.auth.users.AUTH_TYPE", auth_type):
            verify_email_is_invited("newuser@example.com")  # Should not raise

    @patch("docubrain.auth.users.get_invited_users")
    @patch("docubrain.auth.users.workspace_invite_only_enabled", return_value=True)
    @patch("docubrain.auth.users.AUTH_TYPE", AuthType.BASIC)
...
class TestWhitelistBehavior:
    ...
    @patch("docubrain.auth.users.workspace_invite_only_enabled", return_value=False)
    @patch("docubrain.auth.users.get_invited_users")
    @patch("docubrain.auth.users.AUTH_TYPE", AuthType.BASIC)
...
class TestSeatLimitEnforcement:
    ...
    def test_adding_user_fails_when_seats_full(self) -> None:
        ...
        with patch(
            "docubrain.auth.users.fetch_ee_implementation_or_noop",
    ...
    def test_seat_limit_only_enforced_for_self_hosted(self) -> None:
        ...
        with patch("docubrain.auth.users.MULTI_TENANT", True):
