...
from docubrain.federated_connectors.oauth_utils import OAUTH_STATE_TTL
from docubrain.federated_connectors.oauth_utils import OAuthSession
...
def _patched(cache: _MemoryCacheBackend):  # type: ignore[no-untyped-def]
    return patch(
        "docubrain.federated_connectors.oauth_utils.get_cache_backend",
        return_value=cache,
    )
