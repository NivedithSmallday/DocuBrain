...
from docubrain.connectors.canvas.client import CanvasApiClient
from docubrain.connectors.canvas.connector import CanvasConnector
...
def _build_connector(base_url: str = FAKE_BASE_URL) -> CanvasConnector:
    ...
    with patch("docubrain.connectors.canvas.client.rl_requests") as mock_req:
...
class TestGet:
    ...
    @patch("docubrain.connectors.canvas.client.rl_requests")
    ...
    def test_invalid_json_on_success_raises(self, mock_requests: MagicMock) -> None:
        """Invalid JSON on a 2xx response raises DocubrainError."""
        resp = MagicMock()
        ...
        with pytest.raises(DocubrainError, match="Invalid JSON"):
            self.client.get("courses")
...
class TestParseNextLink:
    ...
    def test_rejects_host_mismatch(self) -> None:
        ...
        with pytest.raises(DocubrainError, match="unexpected host"):
    ...
    def test_rejects_non_https_link(self) -> None:
        ...
        with pytest.raises(DocubrainError, match="must use https"):
            self.client._parse_next_link(header)
...
class TestDocumentConversion:
    ...
    def test_convert_page_to_document(self) -> None:
        from docubrain.connectors.canvas.connector import CanvasPage
    ...
    def test_convert_assignment_to_document(self) -> None:
        from docubrain.connectors.canvas.connector import CanvasAssignment
    ...
    def test_convert_announcement_to_document(self) -> None:
        from docubrain.connectors.canvas.connector import CanvasAnnouncement
