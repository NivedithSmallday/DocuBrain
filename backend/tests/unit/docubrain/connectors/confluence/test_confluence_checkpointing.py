...
from docubrain.connectors.confluence.connector import ConfluenceCheckpoint
from docubrain.connectors.confluence.connector import ConfluenceConnector
from docubrain.connectors.confluence.docubrain_confluence import DocubrainConfluence
...
from tests.unit.docubrain.connectors.utils import (
...
def confluence_connector(
    confluence_base_url: str, space_key: str, mock_confluence_client: DocubrainConfluence
    ...
    with patch("docubrain.connectors.confluence.connector._SLIM_DOC_BATCH_SIZE", 2):
        yield connector
...
def test_load_from_checkpoint_with_page_processing_error(
    ...
    with patch(
        "docubrain.connectors.confluence.connector.ConfluenceConnector._convert_page_to_document",
...
def test_validate_connector_settings_errors(
    ...
    with patch(
        "docubrain.connectors.confluence.docubrain_confluence.DocubrainConfluence.retrieve_confluence_spaces"
    ) as mock_retrieve:
