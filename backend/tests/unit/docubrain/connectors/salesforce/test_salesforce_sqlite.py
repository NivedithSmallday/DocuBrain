...
from docubrain.connectors.cross_connector_utils.miscellaneous_utils import time_str_to_utc
...
from docubrain.connectors.salesforce.doc_conversion import _extract_section
from docubrain.connectors.salesforce.doc_conversion import ID_PREFIX
from docubrain.connectors.salesforce.docubrain_salesforce import DocubrainSalesforce
from docubrain.connectors.salesforce.salesforce_calls import _bulk_retrieve_from_salesforce
from docubrain.connectors.salesforce.salesforce_calls import _make_time_filter_for_sf_type
from docubrain.connectors.salesforce.salesforce_calls import _make_time_filtered_query
from docubrain.connectors.salesforce.salesforce_calls import get_object_by_id_query
from docubrain.connectors.salesforce.sqlite_functions import DocubrainSalesforceSQLite
from docubrain.connectors.salesforce.utils import ACCOUNT_OBJECT_TYPE
from docubrain.connectors.salesforce.utils import MODIFIED_FIELD
from docubrain.connectors.salesforce.utils import USER_OBJECT_TYPE
from docubrain.utils.logger import setup_logger

# from docubrain.connectors.salesforce.docubrain_salesforce_type import DocubrainSalesforceType
# from docubrain.connectors.salesforce.salesforce_calls import get_children_of_sf_type
...
def _create_csv_file_and_update_db(
    sf_db: DocubrainSalesforceSQLite,
    object_type: str,
    records: list[dict],
...
def _create_csv_with_example_data(sf_db: DocubrainSalesforceSQLite) -> None:
    """
...
def _test_query(sf_db: DocubrainSalesforceSQLite) -> None:
    """
...
def _test_upsert(sf_db: DocubrainSalesforceSQLite) -> None:
    """
...
def _test_relationships(sf_db: DocubrainSalesforceSQLite) -> None:
    """
...
def _test_account_with_children(sf_db: DocubrainSalesforceSQLite) -> None:
    """
...
def _test_relationship_updates(sf_db: DocubrainSalesforceSQLite) -> None:
    """
...
def _test_get_affected_parent_ids(sf_db: DocubrainSalesforceSQLite) -> None:
    """
...
def test_salesforce_sqlite() -> None:
    with tempfile.TemporaryDirectory() as directory:
        ...
        sf_db = DocubrainSalesforceSQLite(filename)
        sf_db.connect()
        sf_db.apply_schema()
...
def test_salesforce_bulk_retrieve() -> None:
    ...
    # docubrain_sf_type = DocubrainSalesforceType("Contact", sf_client)
...
def test_normalize_record() -> None:
    ...
    with open(current_dir / "test_account.csv", "r", newline="", encoding="utf-8") as f:
        ...
        for row in reader:
            ...
            normalized_record, parent_ids = DocubrainSalesforceSQLite.normalize_record(row)
...
def test_salesforce_connector_single() -> None:
    ...
    # docubrain_parent_sf_type = DocubrainSalesforceType(parent_type, sf_client)
    ...
    for parent_type_working in parent_types:
        ...
        for child_type, child_relationship in child_types_working.items():
            # docubrain_sf_type = DocubrainSalesforceType(child_type, sf_client)
    ...
    normalized_record, _ = DocubrainSalesforceSQLite.normalize_record(record)
