...
_HANDLE_MSG = "docubrain.docubrainbot.slack.handlers.handle_message"
_LISTENER = "docubrain.docubrainbot.slack.listener"
...
class TestCheckTenantGated:
    ...
    def _call(
        ...
        from docubrain.docubrainbot.slack.listener import _check_tenant_gated
...
class TestExtractChannelFromRequest:
    ...
    def test_channel_extraction(
        ...
        from docubrain.docubrainbot.slack.listener import _extract_channel_from_request
...
class TestHandleMessageSeatCheck:
    ...
    def test_new_user_blocked_when_seats_exceeded(
        ...
        assert "DocuBrain administrator" in mock_respond.call_args[1]["text"]
...
class TestCheckSeatAvailability:
    ...
    def _check(self, used: int, total: int) -> Any:
        from ee.docubrain.db.license import check_seat_availability

        metadata = MagicMock(seats=total)
        with (
            patch("ee.docubrain.db.license.get_used_seats", return_value=used),
            patch("ee.docubrain.db.license.get_license_metadata", return_value=metadata),
        ):
    ...
    def test_no_license_allows_unlimited(self) -> None:
        ...
        with patch("ee.docubrain.db.license.get_license_metadata", return_value=None):
...
class TestGetUsedSeats:
    ...
    @patch("ee.docubrain.db.license.MULTI_TENANT", False)
    ...
    def test_excludes_anonymous_user(self, mock_get_session: MagicMock) -> None:
        from ee.docubrain.db.license import get_used_seats
