...
class TestCacheInitialization:
    ...
    async def test_cache_refresh_all_loads_guilds(self) -> None:
        ...
        with (
            patch(
                "docubrain.docubrainbot.discord.cache.get_all_tenant_ids",
                return_value=["tenant1"],
            ),
            patch(
                "docubrain.docubrainbot.discord.cache.fetch_ee_implementation_or_noop",
                return_value=lambda: set(),
            ),
            patch("docubrain.docubrainbot.discord.cache.get_session_with_tenant") as mock_session,
            patch(
                "docubrain.docubrainbot.discord.cache.get_guild_configs",
                return_value=[mock_config1, mock_config2],
            ),
            patch(
                "docubrain.docubrainbot.discord.cache.get_or_create_discord_service_api_key",
