"""Tests for DocubrainError and the global exception handler."""
...
class TestDocubrainError:
    """Unit tests for DocubrainError construction and properties."""

    def test_basic_construction(self) -> None:
        err = DocubrainError(DocubrainErrorCode.NOT_FOUND, "Session not found")
        assert err.error_code is DocubrainErrorCode.NOT_FOUND
        assert err.detail == "Session not found"
        assert err.status_code == 404

    def test_message_defaults_to_code(self) -> None:
        err = DocubrainError(DocubrainErrorCode.UNAUTHENTICATED)
    ...
    def test_no_override_uses_error_code_status(self) -> None:
        err = DocubrainError(DocubrainErrorCode.RATE_LIMITED, "slow down")
        assert err.status_code == 429

    def test_is_exception(self) -> None:
        err = DocubrainError(DocubrainErrorCode.INTERNAL_ERROR)
        assert isinstance(err, Exception)


class TestExceptionHandler:
    """Integration test: DocubrainError → JSON response via FastAPI TestClient."""

    @pytest.fixture()
    def client(self) -> TestClient:
        app = FastAPI()
        register_docubrain_exception_handlers(app)

        @app.get("/boom")
        def _boom() -> None:
            raise DocubrainError(DocubrainErrorCode.NOT_FOUND, "Thing not found")
