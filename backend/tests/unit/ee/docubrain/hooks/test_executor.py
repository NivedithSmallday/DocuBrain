...
from ee.docubrain.hooks.executor import _execute_hook_impl as execute_hook
from docubrain.db.enums import HookFailStrategy
...
def test_early_exit_returns_skipped_with_no_db_writes(
    ...
    with (
        patch("ee.docubrain.hooks.executor.MULTI_TENANT", multi_tenant),
        patch(
            "ee.docubrain.hooks.executor.get_non_deleted_hook_by_hook_point",
            return_value=hook,
        ),
        patch("ee.docubrain.hooks.executor.update_hook__no_commit") as mock_update,
        patch(
            "ee.docubrain.hooks.executor.create_hook_execution_log__no_commit"
        ) as mock_log,
    ):
        result = execute_hook(
...
def test_success_returns_validated_model_and_sets_reachable(
    ...
    with (
        patch("ee.docubrain.hooks.executor.MULTI_TENANT", False),
        patch(
            "ee.docubrain.hooks.executor.get_non_deleted_hook_by_hook_point",
            return_value=hook,
        ),
        patch("ee.docubrain.hooks.executor.get_session_with_current_tenant"),
        patch("ee.docubrain.hooks.executor.update_hook__no_commit") as mock_update,
        patch(
            "ee.docubrain.hooks.executor.create_hook_execution_log__no_commit"
        ) as mock_log,
...
def test_success_skips_reachable_write_when_already_true(db_session: MagicMock) -> None:
    ...
    with (
        ...
        patch("ee.docubrain.hooks.executor.get_session_with_current_tenant"),
        patch("ee.docubrain.hooks.executor.update_hook__no_commit") as mock_update,
        patch("ee.docubrain.hooks.executor.create_hook_execution_log__no_commit"),
        patch("httpx.Client") as mock_client_cls,
    ):
        _setup_client(mock_client_cls, response=_make_response())
        result = execute_hook(
...
def test_http_failure_paths(
    ...
    with (
        ...
        if expected_type is DocubrainError:
            ...
            assert exc_info.value.error_code is DocubrainErrorCode.HOOK_EXECUTION_FAILED
        else:
            result = execute_hook(
                db_session=db_session,
                hook_point=HookPoint.QUERY_PROCESSING,
...
def test_authorization_header(
    ...
    with (
        ...
        patch("ee.docubrain.hooks.executor.get_session_with_current_tenant"),
        patch("ee.docubrain.hooks.executor.update_hook__no_commit"),
        patch("ee.docubrain.hooks.executor.create_hook_execution_log__no_commit"),
        patch("httpx.Client") as mock_client_cls,
    ):
...
@pytest.mark.parametrize(
    "http_exception,expect_docubrain_error",
    [
...
def test_persist_session_failure_is_swallowed(
    ...
    expect_docubrain_error: bool,
) -> None:
    """DB session failure in _persist_result must not mask the real return value or DocubrainError."""
    ...
    with (
        ...
        patch(
            "ee.docubrain.hooks.executor.get_session_with_current_tenant",
...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(HookFailStrategy.HARD, DocubrainError, id="validation_failure_hard"),
    ],
)
...
def test_unexpected_exception_in_inner_respects_fail_strategy(
    ...
    """An unexpected exception raised by _execute_hook_inner (not an DocubrainError from
    ...
    with (
        ...
        patch(
            "ee.docubrain.hooks.executor._execute_hook_inner",
...
def test_is_reachable_failure_does_not_prevent_log(db_session: MagicMock) -> None:
    ...
    DocubrainError(NOT_FOUND) as it would if the hook was concurrently deleted
    ...
    with (
        ...
        patch("ee.docubrain.hooks.executor.get_session_with_current_tenant"),
        patch(
            "ee.docubrain.hooks.executor.update_hook__no_commit",
            side_effect=DocubrainError(DocubrainErrorCode.NOT_FOUND, "hook deleted"),
        ),
        patch(
            "ee.docubrain.hooks.executor.create_hook_execution_log__no_commit"
        ) as mock_log,
