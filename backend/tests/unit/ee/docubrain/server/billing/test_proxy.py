...
class TestProxySeatUpdate:
    ...
    @pytest.mark.asyncio
    @patch("ee.docubrain.server.tenants.proxy.forward_to_control_plane")
    async def test_proxies_seat_update(
        ...
        from ee.docubrain.server.tenants.proxy import proxy_seat_update
...
class TestForwardToControlPlane:
    ...
    @pytest.mark.asyncio
    @patch("ee.docubrain.server.tenants.proxy.generate_data_plane_token")
    @patch("ee.docubrain.server.tenants.proxy.CONTROL_PLANE_API_BASE_URL", "https://cp.test")
...
class TestVerifyLicenseAuth:
    ...
    @pytest.mark.asyncio
    @patch("ee.docubrain.server.tenants.proxy.LICENSE_ENFORCEMENT_ENABLED", True)
    @patch("ee.docubrain.server.tenants.proxy.verify_license_signature")
    @patch("ee.docubrain.server.tenants.proxy.is_license_valid")
