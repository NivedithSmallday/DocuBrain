...
from ee.docubrain.server.license.models import LicensePayload
from ee.docubrain.server.license.models import PlanType
from ee.docubrain.server.tenants.proxy import _check_license_enforcement_enabled
from ee.docubrain.server.tenants.proxy import _extract_license_from_header
from ee.docubrain.server.tenants.proxy import forward_to_control_plane
from ee.docubrain.server.tenants.proxy import get_license_payload
from ee.docubrain.server.tenants.proxy import get_license_payload_allow_expired
from ee.docubrain.server.tenants.proxy import get_optional_license_payload
from ee.docubrain.server.tenants.proxy import verify_license_auth


# All tests that use license auth need LICENSE_ENFORCEMENT_ENABLED=True
LICENSE_ENABLED_PATCH = patch(
    "ee.docubrain.server.tenants.proxy.LICENSE_ENFORCEMENT_ENABLED", True
)
...
class TestLicenseEnforcementCheck:
    ...
    def test_raises_when_disabled(self) -> None:
        ...
        with patch("ee.docubrain.server.tenants.proxy.LICENSE_ENFORCEMENT_ENABLED", False):
    ...
    def test_passes_when_enabled(self) -> None:
        ...
        with patch("ee.docubrain.server.tenants.proxy.LICENSE_ENFORCEMENT_ENABLED", True):
...
class TestVerifyLicenseAuth:
    ...
    def test_valid_license(self) -> None:
        ...
        with (
            ...
            patch(
                "ee.docubrain.server.tenants.proxy.verify_license_signature"
            ) as mock_verify,
        ):
    ...
    def test_expired_license_rejected(self) -> None:
        ...
        with (
            ...
            patch("ee.docubrain.server.tenants.proxy.is_license_valid") as mock_valid,
        ):
            mock_verify.return_value = payload
...
class TestForwardToControlPlane:
    ...
    async def test_successful_get_request(self) -> None:
        ...
        with (
            patch(
                "ee.docubrain.server.tenants.proxy.generate_data_plane_token"
            ) as mock_token,
            patch("ee.docubrain.server.tenants.proxy.httpx.AsyncClient") as mock_client,
            patch(
                "ee.docubrain.server.tenants.proxy.CONTROL_PLANE_API_BASE_URL",
    ...
    async def test_unsupported_method(self) -> None:
        ...
        with (
            ...
            patch("ee.docubrain.server.tenants.proxy.httpx.AsyncClient"),
            patch(
                "ee.docubrain.server.tenants.proxy.CONTROL_PLANE_API_BASE_URL",
                "https://control.example.com",
            ),
...
class TestProxyCheckoutSessionWithSeats:
    ...
    async def test_includes_seats_in_body_when_provided(self) -> None:
        ...
        from ee.docubrain.server.tenants.proxy import proxy_create_checkout_session
        from ee.docubrain.server.tenants.proxy import CreateCheckoutSessionRequest
