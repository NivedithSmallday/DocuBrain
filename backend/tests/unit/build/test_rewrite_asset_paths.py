...
from docubrain.server.features.build.api import api
from docubrain.server.features.build.api.api import _inject_hmr_fixer
from docubrain.server.features.build.api.api import _rewrite_asset_paths
from docubrain.server.features.build.api.api import _rewrite_proxy_response_headers
...
class TestNextjsPathRewriting:
    ...
    def test_rewrites_absolute_next_font_url(self) -> None:
        html = '<link rel="preload" as="font" href="https://craft-dev.docubrain.app/_next/static/media/font.woff2">'
        result = rewrite(html)
        assert f'"{BASE}/_next/static/media/font.woff2"' in result

    def test_rewrites_root_hmr_path(self) -> None:
        html = 'new WebSocket("wss://craft-dev.docubrain.app/_next/webpack-hmr?id=abc")'
        result = rewrite(html)
        assert '"wss://craft-dev.docubrain.app/_next/webpack-hmr?id=abc"' not in result
        assert '"/_next/webpack-hmr?id=abc"' in result

    def test_rewrites_escaped_absolute_next_font_url(self) -> None:
        html = (
            r'{"href":"https:\/\/craft-dev.docubrain.app\/_next\/static\/media\/font.woff2"}'
        )
        result = rewrite(html)
        assert (
...
class TestRuntimeFixerInjection:
    ...
    def test_rewritten_hmr_url_still_matches_shim_intercept_logic(self) -> None:
        html = '<html><head></head><body>new WebSocket("wss://craft-dev.docubrain.app/_next/webpack-hmr?id=abc")</body></html>'

        rewritten = rewrite(html)
        assert '"wss://craft-dev.docubrain.app/_next/webpack-hmr?id=abc"' not in rewritten
...
class TestProxyRequestWiring:
    ...
    def test_rewrites_absolute_link_header_font_preload_paths(self) -> None:
        headers = {
            "link": (
                '<https://craft-dev.docubrain.app/_next/static/media/font.woff2>; rel=preload; as="font"; crossorigin'
            )
        }
