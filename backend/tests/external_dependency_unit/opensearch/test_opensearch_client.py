...
from docubrain.access.utils import prefix_user_email
...
from docubrain.document_index.opensearch.opensearch_document_index import (
...
def _patch_global_tenant_state(monkeypatch: pytest.MonkeyPatch, state: bool) -> None:
    ...
    monkeypatch.setattr("docubrain.document_index.opensearch.schema.MULTI_TENANT", state)
...
def _patch_hybrid_search_subquery_configuration(
    ...
    """
    monkeypatch.setattr(
        "docubrain.document_index.opensearch.constants.HYBRID_SEARCH_SUBQUERY_CONFIGURATION",
        configuration,
    )
    monkeypatch.setattr(
        "docubrain.document_index.opensearch.search.HYBRID_SEARCH_SUBQUERY_CONFIGURATION",
        configuration,
    )
...
def _patch_hybrid_search_normalization_pipeline(
    ...
    """
    monkeypatch.setattr(
        "docubrain.document_index.opensearch.constants.HYBRID_SEARCH_NORMALIZATION_PIPELINE",
        pipeline,
    )
    monkeypatch.setattr(
        "docubrain.document_index.opensearch.search.HYBRID_SEARCH_NORMALIZATION_PIPELINE",
        pipeline,
    )
...
def _patch_opensearch_match_highlights_disabled(
    ...
    """
    monkeypatch.setattr(
        "docubrain.configs.app_configs.OPENSEARCH_MATCH_HIGHLIGHTS_DISABLED",
        disabled,
    )
    monkeypatch.setattr(
        "docubrain.document_index.opensearch.search.OPENSEARCH_MATCH_HIGHLIGHTS_DISABLED",
        disabled,
    )
