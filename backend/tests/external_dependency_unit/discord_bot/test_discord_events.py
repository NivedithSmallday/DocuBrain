...
from docubrain.docubrainbot.discord.handle_commands import get_text_channels
from docubrain.docubrainbot.discord.handle_commands import handle_dm
from docubrain.docubrainbot.discord.handle_commands import handle_registration_command
from docubrain.docubrainbot.discord.handle_commands import handle_sync_channels_command
from docubrain.docubrainbot.discord.handle_message import process_chat_message
from docubrain.docubrainbot.discord.handle_message import send_error_response
from docubrain.docubrainbot.discord.handle_message import send_response
...
class TestGuildRegistrationCommand:
    ...
    async def test_register_guild_success(
        ...
        with (
            patch(
                "docubrain.docubrainbot.discord.handle_commands.parse_discord_registration_key",
                return_value="public",
            ),
            patch(
                "docubrain.docubrainbot.discord.handle_commands.get_session_with_tenant"
            ) as mock_session,
            patch(
                "docubrain.docubrainbot.discord.handle_commands.get_guild_config_by_registration_key"
            ) as mock_get_config,
            patch("docubrain.docubrainbot.discord.handle_commands.bulk_create_channel_configs"),
        ):
            mock_db = MagicMock()
...
class TestSyncChannelsCommand:
    ...
    async def test_sync_channels_adds_new(
        ...
        with (
            ...
            patch(
                "docubrain.docubrainbot.discord.handle_commands.get_guild_config_by_discord_id"
            ) as mock_get_guild,
            patch(
                "docubrain.docubrainbot.discord.handle_commands.get_guild_config_by_internal_id"
            ) as mock_get_guild_internal,
            patch(
                "docubrain.docubrainbot.discord.handle_commands.sync_channel_configs"
            ) as mock_sync,
        ):
            mock_db = MagicMock()
            mock_session.return_value.__enter__ = MagicMock(return_value=mock_db)
...
class TestBotLifecycle:
    ...
    async def test_setup_hook_initializes_cache(
        ...
        from docubrain.docubrainbot.discord.client import DocubrainDiscordClient

        with (
            patch.object(
                DocubrainDiscordClient,
                "__init__",
                lambda self: None,  # noqa: ARG005
            ),
            patch(
                "docubrain.docubrainbot.discord.client.DiscordCacheManager",
                return_value=mock_cache_manager,
            ),
            patch(
                "docubrain.docubrainbot.discord.client.DocubrainAPIClient",
                return_value=mock_api_client,
            ),
        ):
            bot = DocubrainDiscordClient()
    ...
    async def test_close_closes_api_client(
        ...
        with (
            ...
            patch.object(DocubrainDiscordClient, "is_closed", return_value=True),
        ):
            bot = DocubrainDiscordClient()
            bot.cache = mock_cache_manager
            bot.api_client = mock_api_client
