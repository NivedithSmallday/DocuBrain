...
from docubrain.background.celery.tasks.opensearch_migration.constants import (
    GET_VESPA_CHUNKS_SLICE_COUNT,
)
from docubrain.background.celery.tasks.opensearch_migration.tasks import (
    is_continuation_token_done_for_all_slices,
)
from docubrain.background.celery.tasks.opensearch_migration.tasks import (
    migrate_chunks_from_vespa_to_opensearch_task,
)
from docubrain.background.celery.tasks.opensearch_migration.transformer import (
    transform_vespa_chunks_to_opensearch_chunks,
)
from docubrain.configs.constants import PUBLIC_DOC_PAT
from docubrain.configs.constants import SOURCE_TYPE
from docubrain.db.engine.sql_engine import get_session_with_current_tenant
from docubrain.db.models import Document
from docubrain.db.models import OpenSearchDocumentMigrationRecord
from docubrain.db.models import OpenSearchTenantMigrationRecord
from docubrain.db.opensearch_migration import build_sanitized_to_original_doc_id_mapping
...
from docubrain.document_index.opensearch.client import wait_for_opensearch_with_timeout
from docubrain.document_index.opensearch.constants import DEFAULT_MAX_CHUNK_SIZE
...
from docubrain.document_index.vespa.shared_utils.utils import wait_for_vespa_with_timeout
from docubrain.document_index.vespa.vespa_document_index import VespaDocumentIndex
from docubrain.document_index.vespa_constants import ACCESS_CONTROL_LIST
from docubrain.document_index.vespa_constants import BLURB
from docubrain.document_index.vespa_constants import BOOST
from docubrain.document_index.vespa_constants import CHUNK_CONTEXT
from docubrain.document_index.vespa_constants import CHUNK_ID
from docubrain.document_index.vespa_constants import CONTENT
from docubrain.document_index.vespa_constants import DOC_SUMMARY
from docubrain.document_index.vespa_constants import DOC_UPDATED_AT
from docubrain.document_index.vespa_constants import DOCUMENT_ID
from docubrain.document_index.vespa_constants import DOCUMENT_SETS
from docubrain.document_index.vespa_constants import EMBEDDINGS
from docubrain.document_index.vespa_constants import FULL_CHUNK_EMBEDDING_KEY
from docubrain.document_index.vespa_constants import HIDDEN
from docubrain.document_index.vespa_constants import IMAGE_FILE_NAME
from docubrain.document_index.vespa_constants import METADATA_LIST
from docubrain.document_index.vespa_constants import METADATA_SUFFIX
from docubrain.document_index.vespa_constants import PRIMARY_OWNERS
from docubrain.document_index.vespa_constants import SECONDARY_OWNERS
from docubrain.document_index.vespa_constants import SEMANTIC_IDENTIFIER
from docubrain.document_index.vespa_constants import SOURCE_LINKS
from docubrain.document_index.vespa_constants import TITLE
from docubrain.document_index.vespa_constants import TITLE_EMBEDDING
from docubrain.document_index.vespa_constants import USER_PROJECT
...
def full_deployment_setup() -> Generator[None, None, None]:
    ...
    """
    # Patch ENABLE_OPENSEARCH_INDEXING_FOR_DOCUBRAIN just for this test because we
    # don't yet want that enabled for all tests.
    # TODO(andrei): Remove this once CI enables OpenSearch for all tests.
    with (
        patch(
            "docubrain.configs.app_configs.ENABLE_OPENSEARCH_INDEXING_FOR_DOCUBRAIN",
            True,
        ),
        patch("docubrain.document_index.factory.ENABLE_OPENSEARCH_INDEXING_FOR_DOCUBRAIN", True),
    ):
...
def patch_get_vespa_chunks_page_size() -> Generator[int, None, None]:
    ...
    with (
        patch(
            "docubrain.background.celery.tasks.opensearch_migration.tasks.GET_VESPA_CHUNKS_PAGE_SIZE",
            test_page_size,
        ),
        patch(
            "docubrain.background.celery.tasks.opensearch_migration.constants.GET_VESPA_CHUNKS_PAGE_SIZE",
            test_page_size,
        ),
    ):
...
@pytest.fixture(scope="function")
def enable_opensearch_indexing_for_docubrain() -> Generator[None, None, None]:
    with patch(
        "docubrain.background.celery.tasks.opensearch_migration.tasks.ENABLE_OPENSEARCH_INDEXING_FOR_DOCUBRAIN",
        True,
    ):
        yield  # Test runs here.
...
@pytest.fixture(scope="function")
def disable_opensearch_indexing_for_docubrain() -> Generator[None, None, None]:
    with patch(
        "docubrain.background.celery.tasks.opensearch_migration.tasks.ENABLE_OPENSEARCH_INDEXING_FOR_DOCUBRAIN",
        False,
    ):
...
class TestMigrateChunksFromVespaToOpenSearchTask:
    ...
    def test_chunk_migration_completes_successfully(
        ...
        enable_opensearch_indexing_for_docubrain: None,  # noqa: ARG002
    ) -> None:
        """
    ...
    def test_chunk_migration_resumes_from_continuation_token(
        ...
        with patch(
            "docubrain.background.celery.tasks.opensearch_migration.tasks.get_redis_client",
    ...
    def test_returns_none_when_feature_disabled(
        self,
        disable_opensearch_indexing_for_docubrain: None,  # noqa: ARG002
    ) -> None:
