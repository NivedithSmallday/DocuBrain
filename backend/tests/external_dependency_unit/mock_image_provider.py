...
from docubrain.image_gen.interfaces import ImageGenerationProvider
...
def use_mock_image_generation_provider() -> (
    ...
    with (
        ...
        patch(
            "docubrain.tools.tool_implementations.images.image_generation_tool.get_image_generation_provider",
            return_value=image_gen_provider,
        ),
        # Mock is_available to return True so the tool is registered
        patch(
            "docubrain.tools.tool_implementations.images.image_generation_tool.ImageGenerationTool.is_available",
            return_value=True,
        ),
        # Mock the config lookup in tool_constructor to return a valid LLMConfig
        patch(
            "docubrain.tools.tool_constructor._get_image_generation_config",
