...
from docubrain.background.indexing.run_docfetching import connector_document_extraction
...
class TestDocprocessingPriorityInDocumentExtraction:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (False, DocubrainCeleryPriority.HIGH),
            # Re-indexing (has last_successful_index_time) should get MEDIUM priority
            (True, DocubrainCeleryPriority.MEDIUM),
        ],
    )
    @patch("docubrain.background.indexing.run_docfetching.get_document_batch_storage")
    @patch("docubrain.background.indexing.run_docfetching.MemoryTracer")
    @patch("docubrain.background.indexing.run_docfetching._get_connector_runner")
    @patch(
        "docubrain.background.indexing.run_docfetching.strip_null_characters",
        side_effect=lambda batch: batch,
    )
    @patch(
        "docubrain.background.indexing.run_docfetching.get_recent_completed_attempts_for_cc_pair"
    )
    @patch(
        "docubrain.background.indexing.run_docfetching.get_last_successful_attempt_poll_range_end"
    )
    @patch("docubrain.background.indexing.run_docfetching.save_checkpoint")
    @patch("docubrain.background.indexing.run_docfetching.get_latest_valid_checkpoint")
    @patch("docubrain.background.indexing.run_docfetching.get_redis_client")
    @patch("docubrain.background.indexing.run_docfetching.ensure_source_node_exists")
    @patch("docubrain.background.indexing.run_docfetching.get_source_node_id_from_cache")
    @patch("docubrain.background.indexing.run_docfetching.get_node_id_from_raw_id")
    @patch("docubrain.background.indexing.run_docfetching.cache_hierarchy_nodes_batch")
    def test_docprocessing_priority_based_on_last_successful_index_time(
        ...
        expected_priority: DocubrainCeleryPriority,
    ) -> None:
        """
