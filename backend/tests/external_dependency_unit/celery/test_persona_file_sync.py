...
from docubrain.db.models import Persona__UserFile
from docubrain.db.models import User
from docubrain.db.models import UserFile
from docubrain.db.persona import upsert_persona
...
_PATCH_QUEUE_DEPTH = "docubrain.background.celery.tasks.user_file_processing.tasks.get_user_file_project_sync_queue_depth"
...
def _patch_task_app(task: Any, mock_app: MagicMock) -> Generator[None, None, None]:
    ...
    with (
        ...
        patch(
            "docubrain.background.celery.tasks.user_file_processing.tasks.celery_get_broker_client",
...
_PATCH_GET_SETTINGS = (
    "docubrain.background.celery.tasks.user_file_processing.tasks.get_active_search_settings"
)
_PATCH_GET_INDICES = (
    "docubrain.background.celery.tasks.user_file_processing.tasks.get_all_document_indices"
)
_PATCH_HTTPX_INIT = (
    "docubrain.background.celery.tasks.user_file_processing.tasks.httpx_init_vespa_pool"
)
_PATCH_DISABLE_VDB = (
    "docubrain.background.celery.tasks.user_file_processing.tasks.DISABLE_VECTOR_DB"
)
