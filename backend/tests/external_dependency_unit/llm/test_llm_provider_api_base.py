...
from docubrain.server.manage.llm.api import _mask_string
from docubrain.server.manage.llm.api import put_llm_provider
from docubrain.server.manage.llm.api import test_llm_configuration as run_llm_config_test
...
class TestLLMProviderChanges:
    ...
    def test_blocks_api_base_change_without_key_change__multi_tenant(
        ...
        try:
            ...
            with patch("docubrain.server.manage.llm.api.MULTI_TENANT", True):
    ...
    def test_allows_api_base_change__single_tenant(
        ...
        try:
            ...
            with patch("docubrain.server.manage.llm.api.MULTI_TENANT", False):
                update_request = LLMProviderUpsertRequest(
                    id=provider.id,
                    name=provider_name,
                    provider=LlmProviderNames.OPENAI,
...
def test_upload_with_custom_config_then_change(
    ...
    try:
        ...
        with patch("docubrain.server.manage.llm.api.test_llm", side_effect=capture_test_llm):
            run_llm_config_test(
                LLMTestRequest(
