...
from docubrain.server.manage.llm.models import TestLLMRequest as LLMTestRequest
...
class TestLLMConfigurationEndpoint:
    ...
    def test_successful_llm_test_with_new_provider(
        ...
        try:
            with patch(
                "docubrain.server.manage.llm.api.test_llm", side_effect=mock_test_llm_success
            ):
    ...
    def test_failed_llm_test_raises_docubrain_error(
        self,
        db_session: Session,
        provider_name: str,  # noqa: ARG002
    ) -> None:
        """
        Test that a failed LLM test raises an DocubrainError with VALIDATION_ERROR.

        When test_llm returns an error message, the endpoint should raise
        an DocubrainError with the error details.
        """
        ...
        try:
            with patch(
                "docubrain.server.manage.llm.api.test_llm", side_effect=mock_test_llm_failure
            ):
                ...
                assert exc_info.value.error_code == DocubrainErrorCode.VALIDATION_ERROR
