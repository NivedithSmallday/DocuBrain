...
from docubrain.auth.api_key import get_hashed_api_key_from_request
from docubrain.auth.disposable_email_validator import is_disposable_email
from docubrain.auth.email_utils import send_forgot_password_email
from docubrain.auth.email_utils import send_user_verification_email
...
from docubrain.auth.jwt import verify_jwt_token
from docubrain.auth.pat import get_hashed_pat_from_request
from docubrain.auth.schemas import AuthBackend
...
from docubrain.configs.app_configs import AUTH_COOKIE_EXPIRE_TIME_SECONDS
from docubrain.configs.app_configs import AUTH_TYPE
from docubrain.configs.app_configs import EMAIL_CONFIGURED
from docubrain.configs.app_configs import JWT_PUBLIC_KEY_URL
from docubrain.configs.app_configs import PASSWORD_MAX_LENGTH
from docubrain.configs.app_configs import PASSWORD_MIN_LENGTH
from docubrain.configs.app_configs import PASSWORD_REQUIRE_DIGIT
from docubrain.configs.app_configs import PASSWORD_REQUIRE_LOWERCASE
from docubrain.configs.app_configs import PASSWORD_REQUIRE_SPECIAL_CHAR
from docubrain.configs.app_configs import PASSWORD_REQUIRE_UPPERCASE
from docubrain.configs.app_configs import REDIS_AUTH_KEY_PREFIX
from docubrain.configs.app_configs import REQUIRE_EMAIL_VERIFICATION
from docubrain.configs.app_configs import SESSION_EXPIRE_TIME_SECONDS
from docubrain.configs.app_configs import TRACK_EXTERNAL_IDP_EXPIRY
...
from docubrain.configs.constants import ANONYMOUS_USER_COOKIE_NAME
from docubrain.configs.constants import ANONYMOUS_USER_EMAIL
from docubrain.configs.constants import ANONYMOUS_USER_UUID
from docubrain.configs.constants import AuthType
from docubrain.configs.constants import DANSWER_API_KEY_DUMMY_EMAIL_DOMAIN
from docubrain.configs.constants import DANSWER_API_KEY_PREFIX
...
from docubrain.configs.constants import PASSWORD_SPECIAL_CHARS
from docubrain.configs.constants import UNNAMED_KEY_PLACEHOLDER
from docubrain.db.api_key import fetch_user_for_api_key
from docubrain.db.auth import get_access_token_db
from docubrain.db.auth import get_default_admin_user_emails
from docubrain.db.auth import get_user_count
from docubrain.db.auth import get_user_db
from docubrain.db.auth import SQLAlchemyUserAdminDB
from docubrain.db.engine.async_sql_engine import get_async_session
from docubrain.db.engine.async_sql_engine import get_async_session_context_manager
from docubrain.db.engine.sql_engine import get_session_with_current_tenant
from docubrain.db.engine.sql_engine import get_session_with_tenant
from docubrain.db.enums import AccountType
from docubrain.db.models import AccessToken
from docubrain.db.models import OAuthAccount
from docubrain.db.models import Persona
from docubrain.db.models import User
from docubrain.db.pat import fetch_user_for_pat
from docubrain.db.users import assign_user_to_default_groups__no_commit
from docubrain.db.users import get_user_by_email
from docubrain.db.users import is_limited_user
from docubrain.error_handling.error_codes import DocubrainErrorCode
from docubrain.error_handling.exceptions import log_docubrain_error
from docubrain.error_handling.exceptions import docubrain_error_to_json_response
from docubrain.error_handling.exceptions import DocubrainError
from docubrain.redis.redis_pool import get_async_redis_connection
from docubrain.redis.redis_pool import retrieve_ws_token_data
from docubrain.server.settings.store import load_settings
from docubrain.server.utils import BasicAuthenticationError
from docubrain.utils.logger import setup_logger
from docubrain.utils.telemetry import mt_cloud_alias
from docubrain.utils.telemetry import mt_cloud_get_anon_id
from docubrain.utils.telemetry import mt_cloud_identify
...
from docubrain.utils.url import add_url_params
...
def anonymous_user_enabled(*, tenant_id: str | None = None) -> bool:
    ...
    value = cache.get(DocubrainRedisLocks.ANONYMOUS_USER_ENABLED)
...
def verify_email_is_invited(email: str) -> None:
    ...
    if not email:
        raise DocubrainError(DocubrainErrorCode.INVALID_INPUT, "Email must be specified")

    try:
        email_info = validate_email(email, check_deliverability=False)
    except EmailUndeliverableError:
        raise DocubrainError(DocubrainErrorCode.INVALID_INPUT, "Email is not valid")
    ...
    raise DocubrainError(
        DocubrainErrorCode.UNAUTHORIZED,
...
def verify_email_domain(email: str, *, is_registration: bool = False) -> None:
    ...
    if AUTH_TYPE == AuthType.CLOUD:
        ...
        if "+" in local_part and domain != "docubrain.app":
            raise DocubrainError(
                DocubrainErrorCode.INVALID_INPUT,
    ...
    if VALID_EMAIL_DOMAINS:
        if domain not in VALID_EMAIL_DOMAINS:
            raise DocubrainError(DocubrainErrorCode.INVALID_INPUT, "Email domain is not valid")
...
def enforce_seat_limit(db_session: Session, seats_needed: int = 1) -> None:
    ...
    result = fetch_ee_implementation_or_noop(
        "docubrain.db.license", "check_seat_availability", None
    )(db_session, seats_needed=seats_needed)

    if result is not None and not result.available:
        raise DocubrainError(DocubrainErrorCode.SEAT_LIMIT_EXCEEDED, result.error_message)
...
class UserManager(UUIDIDMixin, BaseUserManager[User, uuid.UUID]):
    ...
    async def create(
        ...
        from docubrain.auth.captcha import CaptchaVerificationError
        from docubrain.auth.captcha import is_captcha_enabled
        from docubrain.auth.captcha import verify_captcha_token
        ...
        if is_captcha_enabled() and request is not None:
            ...
            try:
                ...
                raise DocubrainError(DocubrainErrorCode.INVALID_INPUT, str(e))
        ...
        tenant_id = await fetch_ee_implementation_or_noop(
            "docubrain.server.tenants.provisioning",
    ...
    async def on_after_register(
        ...
        get_marketing_posthog_cookie_name = fetch_ee_implementation_or_noop(
            module="docubrain.utils.posthog_client",
        ...
        if (
            ...
            properties = {
                "email": user.email,
                "docubrain_cloud_user_id": str(user.id),
...
class FastAPIUserWithLogoutRouter(FastAPIUsers[models.UP, models.ID]):
    ...
    def get_refresh_router(
        ...
        from docubrain.auth.oauth_refresher import check_and_refresh_oauth_tokens
...
def get_default_admin_user_emails_() -> list[str]:
    # No default seeding available for DocuBrain MIT
    return []
...
def create_docubrain_oauth_router(
...
def get_oauth_router(
    ...
    *,
    ...
    async def callback(
        ...
        def build_error_response(exc: DocubrainError) -> JSONResponse:
            log_docubrain_error(exc)
            error_response = docubrain_error_to_json_response(exc)
        ...
        if enable_pkce:
            ...
            if access_token_state is None:
                raise DocubrainError(
                    DocubrainErrorCode.INTERNAL_ERROR, "Missing OAuth callback state"
                )
