...
from docubrain.configs.app_configs import EMAIL_FROM
from docubrain.configs.app_configs import SENDGRID_API_KEY
from docubrain.configs.app_configs import SMTP_PASS
from docubrain.configs.app_configs import SMTP_PORT
from docubrain.configs.app_configs import SMTP_SERVER
from docubrain.configs.app_configs import SMTP_USER
...
def build_html_email(
    ...
    community_link_fragment = ""
    if application_name == DOCUBRAIN_DEFAULT_APPLICATION_NAME:
        community_link_fragment = f'<br>Have questions? Join our Discord community <a href="{DOCUBRAIN_DISCORD_URL}">here</a>.'
...
def send_email_with_sendgrid(
    ...
    from_email = Email(mail_from) if mail_from else Email("noreply@docubrain.app")
...
def send_email_with_smtplib(
    ...
    msg["Message-ID"] = make_msgid(domain="docubrain.app")
...
def send_subscription_cancellation_email(user_email: str) -> None:
    ...
    try:
        load_runtime_settings_fn = fetch_versioned_implementation(
            "docubrain.server.enterprise_settings.store", "load_runtime_settings"
        )
    ...
    cta_link = "https://www.docubrain.app/pricing"
    ...
    text_content = (
        ...
        "If you change your mind, visit https://www.docubrain.app/pricing"
    )
    send_email(
        user_email,
        subject,
