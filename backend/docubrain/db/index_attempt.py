...
from docubrain.db.models import IndexAttemptError
...
# from docubrain.auth.models import UserRole
# from docubrain.configs.constants import MAX_LAST_VALID_CHECKPOINT_AGE_SECONDS
# from docubrain.db.connector_credential_pair import ConnectorCredentialPairIdentifier
# from docubrain.db.engine import async_query_for_dms
...
def mark_attempt_failed(
    ...
    try:
        ...
        db_session.rollback()
        logger.warning(
            "Skipping mark_attempt_failed for missing index attempt %s. "
            "This can happen when a stale background task outlives the DB row.",
            index_attempt_id,
        )
    except Exception:
        db_session.rollback()
