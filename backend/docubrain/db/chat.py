...
from docubrain.context.search.models import SearchDoc as ServerSearchDoc
from docubrain.db.models import ChatMessage
from docubrain.db.models import ChatMessage__SearchDoc
from docubrain.db.models import ChatSession
from docubrain.db.models import ChatSessionSharedStatus
from docubrain.db.models import Persona
from docubrain.db.models import SearchDoc as DBSearchDoc
from docubrain.db.models import ToolCall
from docubrain.db.models import User
from docubrain.db.persona import get_best_persona_id_for_user
...
# Chat sessions do not include docubrainbot flows
def get_chat_sessions_by_user(
    user_id: UUID | None,
    deleted: bool | None,
    db_session: Session,
    include_docubrainbot_flows: bool = False,
    limit: int = 50,
    ...
    if not include_docubrainbot_flows:
        stmt = stmt.where(ChatSession.docubrainbot_flow.is_(False))
...
def create_chat_session(
    ...
    chat_session = ChatSession(
        ...
        docubrainbot_flow=docubrainbot_flow,
...
def delete_all_chat_sessions_for_user(
    ...
    chat_sessions = (
        ...
        .filter(ChatSession.user_id == user_id, ChatSession.docubrainbot_flow.is_(False))
        .all()
    )
    ...
    if hard_delete:
        ...
        db_session.execute(
            delete(ChatSession).where(
                ChatSession.user_id == user_id, ChatSession.docubrainbot_flow.is_(False)
            )
        )
    else:
        db_session.execute(
            update(ChatSession)
            .where(ChatSession.user_id == user_id, ChatSession.docubrainbot_flow.is_(False))
            .values(deleted=True)
        )
...
def add_search_docs_to_tool_call(
    ...
    Args:
        ...
        db_session: The database session
    """
    from docubrain.db.models import ToolCall__SearchDoc
