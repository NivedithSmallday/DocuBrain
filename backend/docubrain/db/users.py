...
def validate_user_role_update(
    ...
    if current_account_type == AccountType.BOT:
        raise HTTPException(
            status_code=400,
            detail="To change a Slack User's role, they must first login to DocuBrain via the web app.",
        )

    if current_account_type == AccountType.EXT_PERM_USER:
        raise HTTPException(
            status_code=400,
            detail="To change an External Permissioned User's role, they must first login to DocuBrain via the web app.",
        )
    ...
    if requested_role == UserRole.SLACK_USER:
        ...
        raise HTTPException(
            ...
            detail=(
                ...
                "This role is automatically assigned to users who only use DocuBrain via Slack."
            ),
        )
...
def assign_user_to_default_groups__no_commit(
    ...
    from docubrain.db.permissions import recompute_user_permissions__no_commit
