...
from docubrain.configs.constants import (
    ANONYMOUS_USER_UUID,
    DEFAULT_BOOST,
...
from docubrain.db.enums import (
    AccessType,
    ArtifactType,
    BuildSessionStatus,
    EmbeddingPrecision,
...
from docubrain.configs.constants import NotificationType
from docubrain.configs.constants import SearchFeedbackType
from docubrain.configs.constants import TokenRateLimitScope
from docubrain.connectors.models import InputType
from docubrain.db.enums import ChatSessionSharedStatus
...
from docubrain.db.enums import TaskStatus
from docubrain.db.pydantic_type import PydanticListType, PydanticType
from docubrain.kg.models import KGEntityTypeAttributes
from docubrain.utils.logger import setup_logger
from docubrain.utils.special_types import JSON_ro
from docubrain.file_store.models import FileDescriptor
from docubrain.llm.override_models import LLMOverride
from docubrain.llm.override_models import PromptOverride
from docubrain.kg.models import KGStage
from docubrain.utils.encryption import decrypt_bytes_to_string
from docubrain.utils.encryption import encrypt_string_to_bytes
from docubrain.utils.sensitive import SensitiveValue
from docubrain.utils.headers import HeaderItemDict
...
class ConnectorCredentialPair(Base):
    ...
    # {"customer_id": "123567", "company_domain": "@docubrain.app"}
...
class Document(Base):
    ...
    # (as is passed around in DocuBrain)
...
class OpenSearchDocumentMigrationRecord(Base):
    ...
    This table can be dropped when the migration is complete for all DocuBrain
    instances.
    """
...
class ChunkStats(Base):
    ...
    # this should correspond to the ID of the document
    # (as is passed around in DocuBrain)x
...
class ChatSession(Base):
    ...
    # This chat created by DocubrainBot
    docubrainbot_flow: Mapped[bool] = mapped_column(Boolean, default=False)
...
These models are only used in Enterprise Edition only features in DocuBrain.
They are kept here to simplify the codebase and avoid having different assumptions
on the shape of data being passed around between the MIT and EE versions of DocuBrain.

In the MIT version of DocuBrain, assume these tables are always empty.
"""
...
class User__ExternalUserGroupId(Base):
    ...
    directly adding the DocuBrain user to the doc ACL list"""
...
class ScimUserMapping(Base):
    """Maps SCIM externalId from the IdP to a DocuBrain User."""
...
class ScimGroupMapping(Base):
    """Maps SCIM externalId from the IdP to a DocuBrain UserGroup."""
