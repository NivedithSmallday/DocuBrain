...
from docubrain.db.constants import UNSET
from docubrain.db.constants import UnsetType
...
from docubrain.server.features.tool.models import Header
from docubrain.tools.built_in_tools import BUILT_IN_TOOL_TYPES
from docubrain.utils.headers import HeaderItemDict
from docubrain.utils.logger import setup_logger
from docubrain.utils.postgres_sanitization import sanitize_json_like
...
def cleanup_stale_builtin_tools(db_session: Session) -> int:
    """Delete DB rows for stripped built-in tools that must no longer exist."""
    deleted_in_code_tool_ids = {
        "ImageGenerationTool",
        "WebSearchTool",
        "PythonTool",
    }
    stale_tools = list(
        db_session.scalars(
            select(Tool).where(
                Tool.in_code_tool_id.is_not(None),
                Tool.in_code_tool_id.in_(deleted_in_code_tool_ids),
            )
        ).all()
    )
    ...
    for tool in stale_tools:
        logger.warning(
            "Deleting stale built-in tool row: id=%s in_code_tool_id=%s",
            tool.id,
