...
from docubrain.llm.utils import get_max_input_tokens
from docubrain.llm.utils import model_supports_image_input
from docubrain.llm.well_known_providers.auto_update_models import LLMRecommendations
from docubrain.llm.well_known_providers.auto_update_service import (
    fetch_llm_recommendations_from_github,
)
from docubrain.llm.well_known_providers.constants import ANTHROPIC_PROVIDER_NAME
from docubrain.llm.well_known_providers.constants import AZURE_PROVIDER_NAME
from docubrain.llm.well_known_providers.constants import BEDROCK_PROVIDER_NAME
...
from docubrain.llm.well_known_providers.constants import OPENROUTER_PROVIDER_NAME
from docubrain.llm.well_known_providers.constants import VERTEXAI_PROVIDER_NAME
from docubrain.llm.well_known_providers.models import WellKnownLLMProviderDescriptor
from docubrain.server.manage.llm.models import ModelConfigurationView
...
def get_provider_display_name(provider_name: str) -> str:
    """Get human-friendly display name for a DocuBrain-supported provider.

    First checks DocuBrain-specific display names, then falls back to
    PROVIDER_DISPLAY_NAMES from constants.
    """
    # Display names for DocuBrain-supported LLM providers (used in admin UI provider selection).
    # These override PROVIDER_DISPLAY_NAMES for DocuBrain-specific branding.
    _DOCUBRAIN_PROVIDER_DISPLAY_NAMES: dict[str, str] = {
    ...
    if provider_name in _DOCUBRAIN_PROVIDER_DISPLAY_NAMES:
        return _DOCUBRAIN_PROVIDER_DISPLAY_NAMES[provider_name]
