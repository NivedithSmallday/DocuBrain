...
via fetch_versioned_implementation so that:
  - CE: docubrain.hooks.executor._execute_hook_impl → no-op, returns HookSkipped()
  - EE: ee.docubrain.hooks.executor._execute_hook_impl → real HTTP call
"""
...
def execute_hook(
    *,
    ...
    """
    impl = fetch_versioned_implementation("docubrain.hooks.executor", "_execute_hook_impl")
    return impl(
