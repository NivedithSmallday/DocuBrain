...
from docubrain.access.models import ElementExternalAccess
from docubrain.configs.constants import CELERY_GENERIC_BEAT_LOCK_TIMEOUT
from docubrain.configs.constants import CELERY_PERMISSIONS_SYNC_LOCK_TIMEOUT
...
class RedisConnectorPermissionSync:
    ...
    def get_active_task_count(self) -> int:
        ...
        for _ in self.redis.sscan_iter(
            DocubrainRedisConstants.ACTIVE_FENCES,
    ...
    def set_fence(
        ...
        if not payload:
            self.redis.srem(DocubrainRedisConstants.ACTIVE_FENCES, self.fence_key)
            self.redis.delete(self.fence_key)
            return

        self.redis.set(self.fence_key, payload.model_dump_json(), ex=self.FENCE_TTL)
        self.redis.sadd(DocubrainRedisConstants.ACTIVE_FENCES, self.fence_key)
    ...
    def update_db(
        ...
        element_update_permissions_fn = fetch_versioned_implementation(
            "docubrain.background.celery.tasks.doc_permission_syncing.tasks",
