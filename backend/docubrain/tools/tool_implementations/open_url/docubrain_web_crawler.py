...
from docubrain.file_processing.html_utils import ParsedHTML
from docubrain.file_processing.html_utils import web_html_cleanup
from docubrain.tools.tool_implementations.open_url.models import (
    WebContent,
)
from docubrain.tools.tool_implementations.open_url.models import (
    WebContentProvider,
...
from docubrain.utils.url import ssrf_safe_get
from docubrain.utils.url import SSRFException
from docubrain.utils.web_content import decode_html_bytes
from docubrain.utils.web_content import extract_pdf_text
from docubrain.utils.web_content import is_pdf_resource
from docubrain.utils.web_content import title_from_pdf_metadata
from docubrain.utils.web_content import title_from_url

logger = setup_logger()

DEFAULT_READ_TIMEOUT_SECONDS = 15
DEFAULT_CONNECT_TIMEOUT_SECONDS = 5
DEFAULT_USER_AGENT = "DocubrainWebCrawler/1.0 (+https://www.docubrain.app)"
...
class DocubrainWebCrawler(WebContentProvider):
    """
    ...
    def _fetch_url_safe(self, url: str) -> WebContent:
        ...
        try:
            ...
            logger.warning(
                "DocuBrain crawler unexpected error for %s (%s)",
                url,
    ...
    def _fetch_url(self, url: str) -> WebContent:
        try:
            ...
            logger.warning(
                "DocuBrain crawler failed to fetch %s (%s)",
                url,
                exc.__class__.__name__,
            )
            return _failed_result(url)
        ...
        if response.status_code >= 400:
            logger.warning("DocuBrain crawler received %s for %s", response.status_code, url)
            return _failed_result(url)
        ...
        try:
            ...
            logger.warning(
                "DocuBrain crawler failed to parse %s (%s)", url, exc.__class__.__name__
            )
            text_content = ""
            title = ""
