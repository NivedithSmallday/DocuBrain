...
from docubrain.context.search.utils import inference_section_from_chunks
from docubrain.db.document import fetch_document_ids_by_links
from docubrain.db.document import filter_existing_document_ids
...
from docubrain.tools.models import OpenURLToolOverrideKwargs
from docubrain.tools.models import ToolCallException
from docubrain.tools.models import ToolResponse
from docubrain.tools.tool_implementations.open_url.models import WebContentProvider
from docubrain.tools.tool_implementations.open_url.models import WebContent
from docubrain.tools.tool_implementations.open_url.docubrain_web_crawler import (
    DocubrainWebCrawler,
)
from docubrain.tools.tool_implementations.open_url.url_normalization import (
    _default_url_normalizer,
)
from docubrain.tools.tool_implementations.open_url.url_normalization import normalize_url
from docubrain.tools.tool_implementations.open_url.utils import (
...
from docubrain.utils.url import normalize_url as normalize_web_content_url
...
def inference_section_from_internet_page_scrape(
    ...
    normalized_url = normalize_web_content_url(content.link)
    page_content = content.full_content[:MAX_CHARS_PER_URL]
    blurb = (snippet or page_content[:300]).strip()

    chunk = InferenceChunk(
        chunk_id=0,
        blurb=blurb,
        content=page_content,
        source_links={0: normalized_url} if normalized_url else None,
        image_file_id=None,
        section_continuation=False,
        document_id=normalized_url or content.link,
        source_type=DocumentSource.WEB,
        semantic_identifier=content.title or normalized_url or content.link,
        ...
        doc_summary="",
        chunk_context="",
        updated_at=content.published_date,
    )
    ...
    def __init__(
        ...
        Args:
            ...
            content_provider: Optional content provider. If not provided,
                ...
                to the built-in DocuBrain web crawler.
        """
        ...
        if content_provider is not None:
            ...
            if provider is None:
                raise RuntimeError(
                    ...
                    "built-in DocuBrain web crawler can be initialized."
                )
