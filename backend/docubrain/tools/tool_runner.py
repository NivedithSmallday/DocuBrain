...
import docubrain.tracing.framework._error_tracing as _error_tracing
...
from docubrain.tools.models import ChatMinimalTextMessage
from docubrain.tools.models import OpenURLToolOverrideKwargs
from docubrain.tools.models import ParallelToolCallResponse
...
from docubrain.tools.models import ToolExecutionException
...
from docubrain.tracing.framework.spans import SpanError
...
def run_tool_calls(
    ...
    Before execution, tool calls for `SearchTool` and `OpenURLTool`
    are merged so repeated calls are collapsed into a single call per tool:
    - `SearchTool`: merge the `queries` list
    ...
    """
    # Merge tool calls for SearchTool and OpenURLTool
