...
from docubrain.chat.tool_call_args_streaming import maybe_emit_argument_delta
from docubrain.configs.app_configs import LOG_DOCUBRAIN_MODEL_INTERACTIONS
from docubrain.configs.app_configs import PROMPT_CACHE_CHAT_HISTORY
...
from docubrain.llm.interfaces import LanguageModelInput
...
from docubrain.llm.model_response import Delta
from docubrain.llm.models import AssistantMessage
from docubrain.llm.models import ChatCompletionMessage
from docubrain.llm.models import FunctionCall
from docubrain.llm.models import ImageContentPart
from docubrain.llm.models import ImageUrlDetail
from docubrain.llm.models import ReasoningEffort
from docubrain.llm.models import SystemMessage
from docubrain.llm.models import TextContentPart
from docubrain.llm.models import ToolCall
from docubrain.llm.models import ToolMessage
from docubrain.llm.models import UserMessage
from docubrain.llm.prompt_cache.processor import process_with_prompt_cache
from docubrain.llm.utils import model_needs_formatting_reenabled
from docubrain.prompts.chat_prompts import CODE_BLOCK_MARKDOWN
from docubrain.prompts.constants import SYSTEM_REMINDER_TAG_CLOSE
from docubrain.prompts.constants import SYSTEM_REMINDER_TAG_OPEN
...
from docubrain.server.query_and_chat.streaming_models import CitationInfo
from docubrain.server.query_and_chat.streaming_models import Packet
from docubrain.server.query_and_chat.streaming_models import ReasoningDelta
from docubrain.server.query_and_chat.streaming_models import ReasoningDone
from docubrain.server.query_and_chat.streaming_models import ReasoningStart
from docubrain.tools.models import ToolCallKickoff
from docubrain.tracing.framework.create import generation_span
from docubrain.utils.b64 import get_image_type_from_bytes
from docubrain.utils.jsonriver import Parser
from docubrain.utils.logger import setup_logger
from docubrain.utils.postgres_sanitization import sanitize_string
from docubrain.utils.text_processing import find_all_json_objects
...
class _JsonToolCallContentFilter:
    """Streaming filter that strips JSON-style tool-call payloads from text."""

    _DISCARDABLE_PREFIX_CHARS = set(" \t\r\n,}]")

    def __init__(self, tool_definitions: list[dict]) -> None:
        self._pending = ""
        self._tool_name_to_def = _build_tool_name_to_def(tool_definitions)

    def process(self, content: str) -> str:
        if not content or not self._tool_name_to_def:
            return content
        ...
        while self._pending:
            brace_idx = self._pending.find("{")
            if brace_idx == -1:
                output_parts.append(self._pending)
                self._pending = ""
                break

            prefix = self._pending[:brace_idx]
            candidate_info = _extract_json_dict_prefix(self._pending[brace_idx:])
            if candidate_info is None:
                if prefix:
                    output_parts.append(prefix)
                    self._pending = self._pending[brace_idx:]
                break

            candidate_text, json_obj = candidate_info
            matched_tool_call = _try_match_json_to_tool(
                json_obj=json_obj,
                tool_name_to_def=self._tool_name_to_def,
            )
            if matched_tool_call is not None:
                if prefix and not all(
                    char in self._DISCARDABLE_PREFIX_CHARS for char in prefix
                ):
                    output_parts.append(prefix)
                self._pending = self._pending[brace_idx + len(candidate_text) :]
                continue

            output_parts.append(prefix + candidate_text)
            self._pending = self._pending[brace_idx + len(candidate_text) :]

        return "".join(output_parts)

    def flush(self) -> str:
        if _looks_like_malformed_tool_call_payload_from_defs(
            self._pending,
...
def _extract_json_dict_prefix(text: str) -> tuple[str, dict[str, Any]] | None:
    """Return the first complete JSON dict prefix, if one starts at text[0]."""
...
def _build_tool_name_to_def(tool_definitions: list[dict]) -> dict[str, dict]:
    tool_name_to_def: dict[str, dict] = {}
    for tool_def in tool_definitions:
        if tool_def.get("type") != "function" or "function" not in tool_def:
            continue
        func_def = tool_def["function"]
        tool_name = func_def.get("name")
        if tool_name:
            tool_name_to_def[tool_name] = func_def
    return tool_name_to_def
...
def _normalize_tool_arg_value(
    key: str,
    value: Any,
    schema: dict[str, Any],
) -> Any:
    normalized = _try_parse_json_string(value)

    if isinstance(normalized, dict) and set(normalized.keys()) == {key}:
        return _normalize_tool_arg_value(key, normalized[key], schema)

    if isinstance(normalized, dict):
        schema_like_value = _extract_schema_like_value(normalized)
        if schema_like_value is not None:
            return _normalize_tool_arg_value(key, schema_like_value, schema)
...
def _extract_schema_like_value(value: dict[str, Any]) -> Any | None:
    ...
    if isinstance(items, dict):
        nested_items_value = _extract_schema_like_value(items)
        if nested_items_value is not None:
            return nested_items_value

    properties = value.get("properties")
    if isinstance(properties, dict):
        for nested_value in properties.values():
            if isinstance(nested_value, dict):
                extracted = _extract_schema_like_value(nested_value)
...
def _normalize_and_validate_tool_args(
    ...
    func_def = tool_name_to_def.get(tool_name)
    if not func_def:
        return None
    ...
    for key, value in arguments.items():
        ...
        normalized_value = _normalize_tool_arg_value(key, value, properties[key])
        if not _value_matches_schema(normalized_value, properties[key]):
            return None
        normalized_args[key] = normalized_value

    if required and not all(key in normalized_args for key in required):
        return None

    return normalized_args if normalized_args else None
...
def extract_tool_calls_from_response_text(
    ...
    tool_name_to_def = _build_tool_name_to_def(tool_definitions)
    ...
    if not matched_tool_calls:
        matched_tool_calls = _extract_schema_hint_tool_calls_from_response_text(
            response_text=response_text,
            tool_name_to_def=tool_name_to_def,
        )
...
def strip_tool_call_payload_text(
    response_text: str | None,
    tool_definitions: list[dict],
) -> str | None:
    """Remove XML/JSON tool-call payload text from a model response."""
    if not response_text:
        return response_text

    xml_filter = _XmlToolCallContentFilter()
    filtered_text = xml_filter.process(response_text) + xml_filter.flush()

    json_filter = _JsonToolCallContentFilter(tool_definitions)
    filtered_text = json_filter.process(filtered_text) + json_filter.flush()

    if looks_like_malformed_tool_call_payload(
        filtered_text,
        tool_definitions,
    ):
        return None
...
def looks_like_malformed_tool_call_payload(
    response_text: str | None,
    tool_definitions: list[dict],
) -> bool:
    return _looks_like_malformed_tool_call_payload_from_defs(
        response_text,
        _build_tool_name_to_def(tool_definitions),
    )


def _looks_like_malformed_tool_call_payload_from_defs(
    response_text: str | None,
    tool_name_to_def: dict[str, dict],
) -> bool:
    if not response_text or not tool_name_to_def:
        return False
    ...
    tool_name = sanitize_string(tool_name_match.group("name"))
    if tool_name not in tool_name_to_def:
        return False

    if not re.search(r'"(?:arguments|parameters)"\s*:', stripped):
        return False

    if _extract_json_dict_prefix(stripped) is not None:
        return False

    schema_markers = ('"type"', '"items"', '"properties"', '"required"', '"value"')
    return any(marker in stripped for marker in schema_markers) or stripped.startswith(
        "{"
    )


def _extract_schema_hint_tool_calls_from_response_text(
    response_text: str,
    tool_name_to_def: dict[str, dict],
) -> list[tuple[str, dict[str, Any]]]:
    tool_name_match = _TOOL_NAME_PATTERN.search(response_text)
    if not tool_name_match:
        return []
    ...
    properties: dict[str, dict[str, Any]] = func_def.get("parameters", {}).get(
        "properties", {}
    )
    if not properties:
        return []
    ...
    for prop_name, schema in properties.items():
        prop_pos = response_text.find(f'"{prop_name}"')
        if prop_pos == -1:
            continue

        prop_window = response_text[prop_pos : prop_pos + 600]
        candidate_value: Any | None = None

        direct_value_match = re.search(
            rf'"{re.escape(prop_name)}"\s*:\s*"(?P<value>[^"]+)"',
            prop_window,
        )
        if direct_value_match:
            candidate_value = sanitize_string(direct_value_match.group("value"))
        else:
            direct_list_match = re.search(
                rf'"{re.escape(prop_name)}"\s*:\s*(?P<value>\[[^\]]+\])',
                prop_window,
            )
            if direct_list_match:
                candidate_value = direct_list_match.group("value")
            else:
                nested_value_match = re.search(
                    r'"value"\s*:\s*"(?P<value>[^"]+)"',
                    prop_window,
                )
                if nested_value_match:
                    candidate_value = sanitize_string(
                        nested_value_match.group("value")
                    )

        if candidate_value is None:
            continue

        normalized_value = _normalize_tool_arg_value(prop_name, candidate_value, schema)
        if _value_matches_schema(normalized_value, schema):
            extracted_args[prop_name] = normalized_value
...
def run_llm_step_pkt_generator(
    ...
    if LOG_DOCUBRAIN_MODEL_INTERACTIONS:
        logger.debug(
    ...
    with generation_span(
        ...
        filtered_content_tail = json_tool_call_content_filter.process(
