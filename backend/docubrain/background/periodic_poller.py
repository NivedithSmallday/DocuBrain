...
def _run_cache_cleanup() -> None:
    from docubrain.cache.postgres_backend import cleanup_expired_cache_entries

    cleanup_expired_cache_entries()


def _run_scheduled_eval() -> None:
    from docubrain.configs.app_configs import BRAINTRUST_API_KEY
    from docubrain.configs.app_configs import SCHEDULED_EVAL_DATASET_NAMES
    from docubrain.configs.app_configs import SCHEDULED_EVAL_PERMISSIONS_EMAIL
    from docubrain.configs.app_configs import SCHEDULED_EVAL_PROJECT
    ...
    from docubrain.evals.eval import run_eval
...
def _try_claim_task(task_def: _PeriodicTaskDef) -> bool:
    ...
    from docubrain.db.engine.sql_engine import get_session_with_current_tenant
    from docubrain.db.models import KVStore
...
def _run_drain_loops(tenant_id: str) -> None:
    ...
    from docubrain.background.task_utils import drain_processing_loop
