...
from docubrain.configs.constants import DOCUBRAIN_CELERY_BEAT_HEARTBEAT_KEY
from docubrain.configs.constants import DocubrainCeleryTask
from docubrain.db.document import delete_document_by_connector_credential_pair__no_commit
from docubrain.db.document import delete_documents_complete__no_commit
from docubrain.db.document import fetch_chunk_count_for_document
from docubrain.db.document import get_document
from docubrain.db.document import get_document_connector_count
from docubrain.db.document import mark_document_as_modified
...
from docubrain.db.relationships import delete_document_references_from_kg
...
class DocubrainCeleryTaskCompletionStatus(str, Enum):
...
@shared_task(
    name=DocubrainCeleryTask.DOCUMENT_BY_CC_PAIR_CLEANUP_TASK,
...
def document_by_cc_pair_cleanup_task(
    ...
    if completion_status != DocubrainCeleryTaskCompletionStatus.SUCCEEDED:
        return False
...
@shared_task(name=DocubrainCeleryTask.CELERY_BEAT_HEARTBEAT, ignore_result=True, bind=True)
def celery_beat_heartbeat(self: Task, *, tenant_id: str) -> None:  # noqa: ARG001
    ...
    r.set(DOCUBRAIN_CELERY_BEAT_HEARTBEAT_KEY, 1, ex=600)
