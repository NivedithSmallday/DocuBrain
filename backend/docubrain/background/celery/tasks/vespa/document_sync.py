...
from docubrain.db.document import construct_document_id_select_by_needs_sync
from docubrain.db.document import count_documents_by_needs_sync
...
def set_document_sync_fence(r: Redis, payload: int | None) -> None:
    ...
    if payload is None:
        r.srem(DocubrainRedisConstants.ACTIVE_FENCES, DOCUMENT_SYNC_FENCE_KEY)
        r.delete(DOCUMENT_SYNC_FENCE_KEY)
        return

    r.set(DOCUMENT_SYNC_FENCE_KEY, payload, ex=FENCE_TTL)
    r.sadd(DocubrainRedisConstants.ACTIVE_FENCES, DOCUMENT_SYNC_FENCE_KEY)
