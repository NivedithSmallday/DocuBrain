...
from docubrain.background.celery.celery_redis import celery_get_queued_task_ids
from docubrain.background.celery.celery_redis import celery_get_unacked_task_ids
from docubrain.background.celery.celery_utils import extract_ids_from_runnable_connector
from docubrain.background.celery.tasks.beat_schedule import CLOUD_BEAT_MULTIPLIER_DEFAULT
from docubrain.background.celery.tasks.docprocessing.utils import IndexingCallbackBase
from docubrain.configs.app_configs import ALLOW_SIMULTANEOUS_PRUNING
from docubrain.configs.app_configs import JOB_TIMEOUT
from docubrain.configs.constants import CELERY_GENERIC_BEAT_LOCK_TIMEOUT
from docubrain.configs.constants import CELERY_PRUNING_LOCK_TIMEOUT
from docubrain.configs.constants import CELERY_TASK_WAIT_FOR_FENCE_TIMEOUT
from docubrain.configs.constants import DANSWER_REDIS_FUNCTION_LOCK_PREFIX
...
from docubrain.connectors.factory import instantiate_connector
from docubrain.connectors.models import InputType
from docubrain.db.connector import mark_ccpair_as_pruned
...
from docubrain.db.connector_credential_pair import get_connector_credential_pairs
from docubrain.db.document import get_documents_for_connector_credential_pair
...
from docubrain.db.hierarchy import delete_orphaned_hierarchy_nodes
from docubrain.db.hierarchy import link_hierarchy_nodes_to_documents
from docubrain.db.hierarchy import remove_stale_hierarchy_node_cc_pair_entries
from docubrain.db.hierarchy import reparent_orphaned_hierarchy_nodes
from docubrain.db.hierarchy import update_document_parent_hierarchy_nodes
from docubrain.db.hierarchy import upsert_hierarchy_node_cc_pair_entries
from docubrain.db.hierarchy import upsert_hierarchy_nodes_batch
from docubrain.db.models import ConnectorCredentialPair
from docubrain.db.models import HierarchyNode as DBHierarchyNode
from docubrain.db.sync_record import insert_sync_record
from docubrain.db.sync_record import update_sync_record_status
from docubrain.db.tag import delete_orphan_tags__no_commit
...
from docubrain.redis.redis_connector_prune import RedisConnectorPrunePayload
from docubrain.redis.redis_hierarchy import cache_hierarchy_nodes_batch
from docubrain.redis.redis_hierarchy import ensure_source_node_exists
from docubrain.redis.redis_hierarchy import evict_hierarchy_nodes_from_cache
from docubrain.redis.redis_hierarchy import get_node_id_from_raw_id
from docubrain.redis.redis_hierarchy import get_source_node_id_from_cache
from docubrain.redis.redis_hierarchy import HierarchyNodeCacheEntry
from docubrain.redis.redis_pool import get_redis_client
from docubrain.redis.redis_pool import get_redis_replica_client
from docubrain.server.metrics.pruning_metrics import observe_pruning_diff_duration
from docubrain.server.runtime.docubrain_runtime import DocubrainRuntime
from docubrain.server.utils import make_short_id
from docubrain.utils.logger import format_error_for_logging
from docubrain.utils.logger import LoggerContextVars
from docubrain.utils.logger import pruning_ctx
...
@shared_task(
    name=DocubrainCeleryTask.CHECK_FOR_PRUNING,
    ignore_result=True,
...
def check_for_pruning(self: Task, *, tenant_id: str) -> bool | None:
    ...
    lock_beat: RedisLock = r.lock(
        DocubrainRedisLocks.CHECK_PRUNE_BEAT_LOCK,
    ...
    try:
        ...
        if not r.exists(DocubrainRedisSignals.BLOCK_PRUNING):
            ...
            r.set(DocubrainRedisSignals.BLOCK_PRUNING, 1, ex=_get_pruning_block_expiration())

        # we want to run this less frequently than the overall task
        lock_beat.reacquire()
        if not r.exists(DocubrainRedisSignals.BLOCK_VALIDATE_PRUNING_FENCES):
            ...
            r.set(
                DocubrainRedisSignals.BLOCK_VALIDATE_PRUNING_FENCES,
                1,
        ...
        lock_beat.reacquire()
        keys = cast(set[Any], r_replica.smembers(DocubrainRedisConstants.ACTIVE_FENCES))
        for key in keys:
            key_bytes = cast(bytes, key)

            if not r.exists(key_bytes):
                r.srem(DocubrainRedisConstants.ACTIVE_FENCES, key_bytes)
                continue
...
def try_creating_prune_generator_task(
    ...
    try:
        ...
        result = celery_app.send_task(
            DocubrainCeleryTask.CONNECTOR_PRUNING_GENERATOR_TASK,
            kwargs=dict(
            ...
            queue=DocubrainCeleryQueues.CONNECTOR_PRUNING,
...
@shared_task(
    name=DocubrainCeleryTask.CONNECTOR_PRUNING_GENERATOR_TASK,
    acks_late=False,
    soft_time_limit=JOB_TIMEOUT,
    track_started=True,
    trail=False,
...
def connector_pruning_generator_task(
    ...
    lock: RedisLock = r.lock(
        DocubrainRedisLocks.PRUNING_LOCK_PREFIX + f"_{redis_connector.cc_pair_id}",
...
def validate_pruning_fences(
    ...
    queue_len = celery_get_queue_length(DocubrainCeleryQueues.CONNECTOR_DELETION, r_celery)
...
def validate_pruning_fence(
    ...
    found = celery_find_task(
        ...
        DocubrainCeleryQueues.CONNECTOR_PRUNING,
        r_celery,
    )
    if found:
