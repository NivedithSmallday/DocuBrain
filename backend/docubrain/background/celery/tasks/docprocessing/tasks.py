...
from docubrain.background.celery.apps.app_base import task_logger
from docubrain.background.celery.celery_redis import celery_find_task
from docubrain.background.celery.celery_redis import celery_get_broker_client
from docubrain.background.celery.celery_redis import celery_get_unacked_task_ids
from docubrain.background.celery.celery_utils import httpx_init_vespa_pool
from docubrain.background.celery.memory_monitoring import emit_process_memory
from docubrain.background.celery.tasks.beat_schedule import CLOUD_BEAT_MULTIPLIER_DEFAULT
from docubrain.background.celery.tasks.docfetching.task_creation_utils import (
    try_creating_docfetching_task,
)
from docubrain.background.celery.tasks.docprocessing.heartbeat import start_heartbeat
from docubrain.background.celery.tasks.docprocessing.heartbeat import stop_heartbeat
from docubrain.background.celery.tasks.docprocessing.utils import IndexingCallback
from docubrain.background.celery.tasks.docprocessing.utils import is_in_repeated_error_state
from docubrain.background.celery.tasks.docprocessing.utils import should_index
from docubrain.background.celery.tasks.models import DocProcessingContext
from docubrain.background.indexing.checkpointing_utils import cleanup_checkpoint
from docubrain.background.indexing.checkpointing_utils import (
    get_index_attempts_with_old_checkpoints,
)
from docubrain.background.indexing.index_attempt_utils import cleanup_index_attempts
from docubrain.background.indexing.index_attempt_utils import get_old_index_attempts
from docubrain.configs.app_configs import AUTH_TYPE
from docubrain.configs.app_configs import MANAGED_VESPA
from docubrain.configs.app_configs import VESPA_CLOUD_CERT_PATH
from docubrain.configs.app_configs import VESPA_CLOUD_KEY_PATH
from docubrain.configs.constants import AuthType
from docubrain.configs.constants import CELERY_GENERIC_BEAT_LOCK_TIMEOUT
from docubrain.configs.constants import CELERY_INDEXING_LOCK_TIMEOUT
...
from docubrain.configs.constants import DocubrainRedisConstants
from docubrain.configs.constants import DocubrainRedisLocks
from docubrain.configs.constants import DocubrainRedisSignals
...
from docubrain.db.connector_credential_pair import get_connector_credential_pair_from_id
from docubrain.db.connector_credential_pair import set_cc_pair_repeated_error_state
from docubrain.db.connector_credential_pair import update_connector_credential_pair_from_id
from docubrain.db.engine.sql_engine import get_session_with_current_tenant
from docubrain.db.engine.time_utils import get_db_current_time
from docubrain.db.enums import ConnectorCredentialPairStatus
from docubrain.db.enums import IndexingMode
from docubrain.db.enums import IndexingStatus
from docubrain.db.enums import SwitchoverType
from docubrain.db.index_attempt import create_index_attempt_error
from docubrain.db.index_attempt import get_index_attempt
from docubrain.db.index_attempt import get_index_attempt_errors_for_cc_pair
from docubrain.db.index_attempt import IndexAttemptError
from docubrain.db.index_attempt import mark_attempt_canceled
from docubrain.db.index_attempt import mark_attempt_failed
from docubrain.db.index_attempt import mark_attempt_partially_succeeded
from docubrain.db.index_attempt import mark_attempt_succeeded
from docubrain.db.indexing_coordination import CoordinationStatus
from docubrain.db.indexing_coordination import INDEXING_PROGRESS_TIMEOUT_HOURS
from docubrain.db.indexing_coordination import IndexingCoordination
from docubrain.db.models import IndexAttempt
from docubrain.db.models import SearchSettings
from docubrain.db.search_settings import get_current_search_settings
from docubrain.db.search_settings import get_secondary_search_settings
from docubrain.db.swap_index import check_and_perform_index_swap
from docubrain.document_index.factory import get_all_document_indices
from docubrain.file_store.document_batch_storage import DocumentBatchStorage
from docubrain.file_store.document_batch_storage import get_document_batch_storage
from docubrain.httpx.httpx_pool import HttpxPool
from docubrain.indexing.adapters.document_indexing_adapter import (
    DocumentIndexingBatchAdapter,
)
from docubrain.indexing.embedder import DefaultIndexingEmbedder
from docubrain.indexing.indexing_pipeline import run_indexing_pipeline
from docubrain.natural_language_processing.search_nlp_models import EmbeddingModel
from docubrain.natural_language_processing.search_nlp_models import warm_up_bi_encoder
from docubrain.redis.redis_connector import RedisConnector
from docubrain.redis.redis_pool import get_redis_client
from docubrain.redis.redis_pool import get_redis_replica_client
from docubrain.redis.redis_pool import redis_lock_dump
from docubrain.redis.redis_pool import SCAN_ITER_COUNT_DEFAULT
from docubrain.redis.redis_utils import is_fence
from docubrain.server.runtime.docubrain_runtime import DocubrainRuntime
from docubrain.utils.logger import setup_logger
from docubrain.utils.middleware import make_randomized_docubrain_request_id
...
def _get_fence_validation_block_expiration() -> int:
    ...
    try:
        beat_multiplier = DocubrainRuntime.get_beat_multiplier()
    except Exception:
        ...
        if attempt and timed_out:
            if attempt.status == IndexingStatus.IN_PROGRESS:
                ...
                task_exists = celery_find_task(
                    ...
                    DocubrainCeleryQueues.CONNECTOR_DOC_FETCHING,
                    redis_celery,
                )
                unacked_task_ids = celery_get_unacked_task_ids(
                    DocubrainCeleryQueues.CONNECTOR_DOC_FETCHING, redis_celery
                )
...
@shared_task(
    name=DocubrainCeleryTask.CHECK_FOR_INDEXING,
    soft_time_limit=300,
    bind=True,
)
def check_for_indexing(self: Task, *, tenant_id: str) -> int | None:
    ...
    the docfetching task (DocubrainCeleryTask.CONNECTOR_DOC_FETCHING_TASK).
    ...
    lock_beat: RedisLock = redis_client.lock(
        DocubrainRedisLocks.CHECK_INDEXING_BEAT_LOCK,
    ...
    try:
        ...
        if not redis_client.exists(DocubrainRedisSignals.BLOCK_BUILD_FENCE_LOOKUP_TABLE):
            ...
            for key_bytes in redis_client_replica.scan_iter(
                ...
                if is_fence(key_bytes) and not redis_client.sismember(
                    DocubrainRedisConstants.ACTIVE_FENCES, key_bytes
                ):
                    logger.warning(f"Adding {key_bytes} to the lookup table.")
                    redis_client.sadd(DocubrainRedisConstants.ACTIVE_FENCES, key_bytes)

            redis_client.set(
                DocubrainRedisSignals.BLOCK_BUILD_FENCE_LOOKUP_TABLE,
                1,
                ex=DocubrainRuntime.get_build_fence_lookup_table_interval(),
            )
        ...
        lock_beat.reacquire()
        # we want to run this less frequently than the overall task
        if not redis_client.exists(DocubrainRedisSignals.BLOCK_VALIDATE_INDEXING_FENCES):
            ...
            redis_client.set(
                DocubrainRedisSignals.BLOCK_VALIDATE_INDEXING_FENCES,
                1,
...
# primary
@shared_task(
    name=DocubrainCeleryTask.CHECK_FOR_CHECKPOINT_CLEANUP,
    soft_time_limit=300,
    bind=True,
)
def check_for_checkpoint_cleanup(self: Task, *, tenant_id: str) -> None:
    ...
    lock: RedisLock = redis_client.lock(
        DocubrainRedisLocks.CHECK_CHECKPOINT_CLEANUP_BEAT_LOCK,
        timeout=CELERY_GENERIC_BEAT_LOCK_TIMEOUT,
    )
    ...
    try:
        ...
        with get_session_with_current_tenant() as db_session:
            ...
            for attempt in old_attempts:
                ...
                self.app.send_task(
                    DocubrainCeleryTask.CLEANUP_CHECKPOINT,
                    kwargs={
                        "index_attempt_id": attempt.id,
                        "tenant_id": tenant_id,
                    },
                    queue=DocubrainCeleryQueues.CHECKPOINT_CLEANUP,
...
# light worker
@shared_task(
    name=DocubrainCeleryTask.CLEANUP_CHECKPOINT,
    bind=True,
)
...
# primary
@shared_task(
    name=DocubrainCeleryTask.CHECK_FOR_INDEX_ATTEMPT_CLEANUP,
    soft_time_limit=300,
    bind=True,
)
def check_for_index_attempt_cleanup(self: Task, *, tenant_id: str) -> None:
    ...
    redis_client = get_redis_client(tenant_id=tenant_id)
    lock: RedisLock = redis_client.lock(
        DocubrainRedisLocks.CHECK_INDEX_ATTEMPT_CLEANUP_BEAT_LOCK,
        timeout=CELERY_GENERIC_BEAT_LOCK_TIMEOUT,
    )
    ...
    try:
        ...
        with get_session_with_current_tenant() as db_session:
            ...
            for i in range(0, len(old_attempts), batch_size):
                ...
                self.app.send_task(
                    DocubrainCeleryTask.CLEANUP_INDEX_ATTEMPT,
                    kwargs={
                        "index_attempt_ids": [attempt.id for attempt in batch],
                        "tenant_id": tenant_id,
                    },
                    queue=DocubrainCeleryQueues.INDEX_ATTEMPT_CLEANUP,
                    priority=DocubrainCeleryPriority.MEDIUM,
                )
    except Exception:
...
# light worker
@shared_task(
    name=DocubrainCeleryTask.CLEANUP_INDEX_ATTEMPT,
    bind=True,
)
...
@shared_task(
    name=DocubrainCeleryTask.DOCPROCESSING_TASK,
    bind=True,
)
...
def _check_chunk_usage_limit(tenant_id: str) -> None:
    ...
    from docubrain.db.usage import UsageType
    from docubrain.server.usage_limits import check_usage_and_raise
    ...
    try:
        ...
        with get_session_with_current_tenant() as db_session:
            ...
            index_attempt_metadata = IndexAttemptMetadata(
                ...
                request_id=make_randomized_docubrain_request_id("DIP"),
        ...
        if USAGE_LIMITS_ENABLED and index_pipeline_result.total_chunks > 0:
            try:
                from docubrain.db.usage import increment_usage
                from docubrain.db.usage import UsageType
