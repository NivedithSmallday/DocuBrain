...
from docubrain.background.celery.celery_redis import celery_get_queue_length
...
from docubrain.configs.constants import DOCUBRAIN_CLOUD_TENANT_ID
...
from docubrain.db.engine.sql_engine import get_session_with_shared_schema
from docubrain.db.engine.tenant_utils import get_all_tenant_ids
...
from docubrain.db.enums import SyncType
from docubrain.db.models import ConnectorCredentialPair
from docubrain.db.models import DocumentSet
from docubrain.db.models import IndexAttempt
from docubrain.db.models import SyncRecord
from docubrain.db.models import UserGroup
from docubrain.db.search_settings import get_active_search_settings_list
from docubrain.redis.redis_pool import get_redis_client
from docubrain.redis.redis_pool import redis_lock_dump
from docubrain.utils.logger import is_running_in_container
...
def _collect_queue_metrics(redis_celery: Redis) -> list[Metric]:
    ...
    metrics = []
    queue_mappings = {
        "celery_queue_length": DocubrainCeleryQueues.PRIMARY,
        "docprocessing_queue_length": DocubrainCeleryQueues.DOCPROCESSING,
        "docfetching_queue_length": DocubrainCeleryQueues.CONNECTOR_DOC_FETCHING,
        "sync_queue_length": DocubrainCeleryQueues.VESPA_METADATA_SYNC,
        "deletion_queue_length": DocubrainCeleryQueues.CONNECTOR_DELETION,
        "pruning_queue_length": DocubrainCeleryQueues.CONNECTOR_PRUNING,
        "permissions_sync_queue_length": DocubrainCeleryQueues.CONNECTOR_DOC_PERMISSIONS_SYNC,
        "external_group_sync_queue_length": DocubrainCeleryQueues.CONNECTOR_EXTERNAL_GROUP_SYNC,
        "permissions_upsert_queue_length": DocubrainCeleryQueues.DOC_PERMISSIONS_UPSERT,
        "hierarchy_fetching_queue_length": DocubrainCeleryQueues.CONNECTOR_HIERARCHY_FETCHING,
        "llm_model_update_queue_length": DocubrainCeleryQueues.LLM_MODEL_UPDATE,
        "checkpoint_cleanup_queue_length": DocubrainCeleryQueues.CHECKPOINT_CLEANUP,
        "index_attempt_cleanup_queue_length": DocubrainCeleryQueues.INDEX_ATTEMPT_CLEANUP,
        "csv_generation_queue_length": DocubrainCeleryQueues.CSV_GENERATION,
        "user_file_processing_queue_length": DocubrainCeleryQueues.USER_FILE_PROCESSING,
        "user_file_project_sync_queue_length": DocubrainCeleryQueues.USER_FILE_PROJECT_SYNC,
        "user_file_delete_queue_length": DocubrainCeleryQueues.USER_FILE_DELETE,
        "monitoring_queue_length": DocubrainCeleryQueues.MONITORING,
        "sandbox_queue_length": DocubrainCeleryQueues.SANDBOX,
        "opensearch_migration_queue_length": DocubrainCeleryQueues.OPENSEARCH_MIGRATION,
    }
...
@shared_task(
    name=DocubrainCeleryTask.MONITOR_BACKGROUND_PROCESSES,
    ignore_result=True,
...
def monitor_background_processes(self: Task, *, tenant_id: str) -> None:
    ...
    lock_monitoring: RedisLock = r.lock(
        DocubrainRedisLocks.MONITOR_BACKGROUND_PROCESSES_LOCK,
...
@shared_task(
    name=DocubrainCeleryTask.CLOUD_MONITOR_ALEMBIC,
)
def cloud_check_alembic() -> bool | None:
    ...
    redis_client = get_redis_client(tenant_id=DOCUBRAIN_CLOUD_TENANT_ID)

    lock_beat: RedisLock = redis_client.lock(
        DocubrainRedisLocks.CLOUD_CHECK_ALEMBIC_BEAT_LOCK,
...
@shared_task(
    name=DocubrainCeleryTask.CLOUD_MONITOR_CELERY_QUEUES, ignore_result=True, bind=True
)
...
@shared_task(name=DocubrainCeleryTask.MONITOR_CELERY_QUEUES, ignore_result=True, bind=True)
...
def monitor_celery_queues_helper(
    ...
    n_celery = celery_get_queue_length(DocubrainCeleryQueues.PRIMARY, r_celery)
    n_docfetching = celery_get_queue_length(
        DocubrainCeleryQueues.CONNECTOR_DOC_FETCHING, r_celery
    )
    n_docprocessing = celery_get_queue_length(DocubrainCeleryQueues.DOCPROCESSING, r_celery)

    n_user_file_processing = celery_get_queue_length(
        DocubrainCeleryQueues.USER_FILE_PROCESSING, r_celery
    )
    n_user_file_project_sync = celery_get_queue_length(
        DocubrainCeleryQueues.USER_FILE_PROJECT_SYNC, r_celery
    )
    n_user_file_delete = celery_get_queue_length(
        DocubrainCeleryQueues.USER_FILE_DELETE, r_celery
    )
    n_sync = celery_get_queue_length(DocubrainCeleryQueues.VESPA_METADATA_SYNC, r_celery)
    n_deletion = celery_get_queue_length(DocubrainCeleryQueues.CONNECTOR_DELETION, r_celery)
    n_pruning = celery_get_queue_length(DocubrainCeleryQueues.CONNECTOR_PRUNING, r_celery)
    n_permissions_sync = celery_get_queue_length(
        DocubrainCeleryQueues.CONNECTOR_DOC_PERMISSIONS_SYNC, r_celery
    )
    n_external_group_sync = celery_get_queue_length(
        DocubrainCeleryQueues.CONNECTOR_EXTERNAL_GROUP_SYNC, r_celery
    )
    n_permissions_upsert = celery_get_queue_length(
        DocubrainCeleryQueues.DOC_PERMISSIONS_UPSERT, r_celery
    )
    n_hierarchy_fetching = celery_get_queue_length(
        DocubrainCeleryQueues.CONNECTOR_HIERARCHY_FETCHING, r_celery
    )
    n_llm_model_update = celery_get_queue_length(
        DocubrainCeleryQueues.LLM_MODEL_UPDATE, r_celery
    )
    n_checkpoint_cleanup = celery_get_queue_length(
        DocubrainCeleryQueues.CHECKPOINT_CLEANUP, r_celery
    )
    n_index_attempt_cleanup = celery_get_queue_length(
        DocubrainCeleryQueues.INDEX_ATTEMPT_CLEANUP, r_celery
    )
    n_csv_generation = celery_get_queue_length(
        DocubrainCeleryQueues.CSV_GENERATION, r_celery
    )
    n_monitoring = celery_get_queue_length(DocubrainCeleryQueues.MONITORING, r_celery)
    n_sandbox = celery_get_queue_length(DocubrainCeleryQueues.SANDBOX, r_celery)
    n_opensearch_migration = celery_get_queue_length(
        DocubrainCeleryQueues.OPENSEARCH_MIGRATION, r_celery
    )
    ...
    n_docprocessing_prefetched = celery_get_unacked_task_ids(
        DocubrainCeleryQueues.DOCPROCESSING, r_celery
    )
...
def _classify_supervisor_process(cmdline: str) -> str | None:
    """Map a process cmdline to a supervised process type we actually expect."""
    if "supervisord_watchdog.py" in cmdline or "tail -qF" in cmdline:
        return None

    if "celery -A" in cmdline:
        if "versioned_apps.beat beat" in cmdline:
            return "beat"

        hostname_to_process_type = {
        ...
        for hostname, process_type in hostname_to_process_type.items():
            if hostname in cmdline:
                return process_type

    if "slack/listener.py" in cmdline:
        return "slack"
...
@shared_task(
    name=DocubrainCeleryTask.MONITOR_PROCESS_MEMORY,
    ignore_result=True,
    soft_time_limit=_MONITORING_SOFT_TIME_LIMIT,
    time_limit=_MONITORING_TIME_LIMIT,
    queue=DocubrainCeleryQueues.MONITORING,
...
def monitor_process_memory(self: Task, *, tenant_id: str) -> None:  # noqa: ARG001
    ...
    try:
        ...
        for proc in psutil.process_iter():
            ...
            process_type = _classify_supervisor_process(cmdline)
        ...
        missing_processes = expected_process_types.difference(supervisor_processes.values())
...
@shared_task(
    name=DocubrainCeleryTask.CLOUD_MONITOR_CELERY_PIDBOX, ignore_result=True, bind=True
)
