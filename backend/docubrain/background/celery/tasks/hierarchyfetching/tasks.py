...
from docubrain.connectors.factory import ConnectorMissingException
from docubrain.connectors.factory import identify_connector_class
from docubrain.connectors.factory import instantiate_connector
from docubrain.connectors.interfaces import HierarchyConnector
from docubrain.connectors.models import HierarchyNode as PydanticHierarchyNode
from docubrain.db.connector import mark_cc_pair_as_hierarchy_fetched
...
def _try_creating_hierarchy_fetching_task(
    ...
    try:
        ...
        result = celery_app.send_task(
            DocubrainCeleryTask.CONNECTOR_HIERARCHY_FETCHING_TASK,
            kwargs=dict(
                cc_pair_id=cc_pair.id,
                tenant_id=tenant_id,
            ),
            queue=DocubrainCeleryQueues.CONNECTOR_HIERARCHY_FETCHING,
...
@shared_task(
    name=DocubrainCeleryTask.CHECK_FOR_HIERARCHY_FETCHING,
    soft_time_limit=300,
    bind=True,
)
def check_for_hierarchy_fetching(self: Task, *, tenant_id: str) -> int | None:
    ...
    lock_beat: RedisLock = redis_client.lock(
        DocubrainRedisLocks.CHECK_HIERARCHY_FETCHING_BEAT_LOCK,
...
@shared_task(
    name=DocubrainCeleryTask.CONNECTOR_HIERARCHY_FETCHING_TASK,
