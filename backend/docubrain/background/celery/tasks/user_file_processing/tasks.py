...
from docubrain.access.access import build_access_for_user_files
...
from docubrain.background.celery.tasks.shared.RetryDocumentIndex import RetryDocumentIndex
...
from docubrain.configs.constants import CELERY_USER_FILE_DELETE_TASK_EXPIRES
from docubrain.configs.constants import CELERY_USER_FILE_PROCESSING_LOCK_TIMEOUT
from docubrain.configs.constants import CELERY_USER_FILE_PROCESSING_TASK_EXPIRES
from docubrain.configs.constants import CELERY_USER_FILE_PROJECT_SYNC_LOCK_TIMEOUT
from docubrain.configs.constants import CELERY_USER_FILE_PROJECT_SYNC_TASK_EXPIRES
...
from docubrain.configs.constants import USER_FILE_DELETE_MAX_QUEUE_DEPTH
from docubrain.configs.constants import USER_FILE_PROCESSING_MAX_QUEUE_DEPTH
from docubrain.configs.constants import USER_FILE_PROJECT_SYNC_MAX_QUEUE_DEPTH
...
from docubrain.db.user_file import fetch_user_files_with_access_relationships
from docubrain.document_index.factory import get_all_document_indices
from docubrain.document_index.interfaces import VespaDocumentFields
from docubrain.document_index.interfaces import VespaDocumentUserFields
from docubrain.document_index.vespa_constants import DOCUMENT_ID_ENDPOINT
...
from docubrain.file_store.utils import store_user_file_plaintext
from docubrain.file_store.utils import user_file_id_to_plaintext_file_name
from docubrain.httpx.httpx_pool import HttpxPool
from docubrain.indexing.adapters.user_file_indexing_adapter import UserFileIndexingAdapter
...
def _user_file_lock_key(user_file_id: str | UUID) -> str:
    return f"{DocubrainRedisLocks.USER_FILE_PROCESSING_LOCK_PREFIX}:{user_file_id}"
...
def _user_file_queued_key(user_file_id: str | UUID) -> str:
    ...
    in flight.
    """
    return f"{DocubrainRedisLocks.USER_FILE_QUEUED_PREFIX}:{user_file_id}"


def user_file_project_sync_lock_key(user_file_id: str | UUID) -> str:
    return f"{DocubrainRedisLocks.USER_FILE_PROJECT_SYNC_LOCK_PREFIX}:{user_file_id}"


def _user_file_project_sync_queued_key(user_file_id: str | UUID) -> str:
    return f"{DocubrainRedisLocks.USER_FILE_PROJECT_SYNC_QUEUED_PREFIX}:{user_file_id}"


def _user_file_delete_lock_key(user_file_id: str | UUID) -> str:
    return f"{DocubrainRedisLocks.USER_FILE_DELETE_LOCK_PREFIX}:{user_file_id}"
...
def _user_file_delete_queued_key(user_file_id: str | UUID) -> str:
    ...
    in flight.
    """
    return f"{DocubrainRedisLocks.USER_FILE_DELETE_QUEUED_PREFIX}:{user_file_id}"


def _load_user_file_document(uf: UserFile) -> Document:
    """Build an indexable Document for a user-uploaded file from the file store."""
    file_store = get_default_file_store()
    file_io = file_store.read_file(uf.file_id, mode="b")
    text_content = extract_file_text(
        file=file_io,
        file_name=uf.name or uf.file_id,
    )

    return Document(
        id=str(uf.id),
        source=DocumentSource.USER_FILE,
        semantic_identifier=uf.name or uf.file_id,
        title=uf.name,
        sections=[TextSection(text=text_content)],
        metadata={},
    )
...
def get_user_file_project_sync_queue_depth(celery_app: Celery) -> int:
    ...
    return celery_get_queue_length(
        DocubrainCeleryQueues.USER_FILE_PROJECT_SYNC, redis_celery
    )
...
def enqueue_user_file_project_sync_task(
    *,
    ...
    tenant_id: str,
    priority: DocubrainCeleryPriority = DocubrainCeleryPriority.HIGH,
) -> bool:
    ...
    try:
        celery_app.send_task(
            DocubrainCeleryTask.PROCESS_SINGLE_USER_FILE_PROJECT_SYNC,
            kwargs={"user_file_id": str(user_file_id), "tenant_id": tenant_id},
            queue=DocubrainCeleryQueues.USER_FILE_PROJECT_SYNC,
            priority=priority,
...
@shared_task(
    name=DocubrainCeleryTask.CHECK_FOR_USER_FILE_PROCESSING,
    soft_time_limit=300,
    bind=True,
    ignore_result=True,
)
def check_user_file_processing(self: Task, *, tenant_id: str) -> None:
    ...
    lock: RedisLock = redis_client.lock(
        DocubrainRedisLocks.USER_FILE_PROCESSING_BEAT_LOCK,
    ...
    try:
        ...
        with get_session_with_current_tenant() as db_session:
            ...
            for user_file_id in user_file_ids:
                ...
                try:
                    self.app.send_task(
                        DocubrainCeleryTask.PROCESS_SINGLE_USER_FILE,
                        kwargs={
                            "user_file_id": str(user_file_id),
                            "tenant_id": tenant_id,
                        },
                        queue=DocubrainCeleryQueues.USER_FILE_PROCESSING,
...
def _process_user_file_without_vector_db(
    ...
    from docubrain.llm.factory import get_llm_tokenizer_encode_func
    ...
    *, user_file_id: str, tenant_id: str, redis_locking: bool
    ...
    try:
        with get_session_with_current_tenant() as db_session:
            ...
            try:
                documents = [_load_user_file_document(uf)]
...
@shared_task(
    name=DocubrainCeleryTask.PROCESS_SINGLE_USER_FILE,
    bind=True,
    ignore_result=True,
)
...
@shared_task(
    name=DocubrainCeleryTask.CHECK_FOR_USER_FILE_DELETE,
    soft_time_limit=300,
    bind=True,
    ignore_result=True,
)
def check_for_user_file_delete(self: Task, *, tenant_id: str) -> None:
    ...
    redis_client = get_redis_client(tenant_id=tenant_id)
    lock: RedisLock = redis_client.lock(
        DocubrainRedisLocks.USER_FILE_DELETE_BEAT_LOCK,
        timeout=CELERY_GENERIC_BEAT_LOCK_TIMEOUT,
    )
    if not lock.acquire(blocking=False):
        return None
    ...
    try:
        ...
        r_celery = celery_get_broker_client(self.app)
        queue_len = celery_get_queue_length(DocubrainCeleryQueues.USER_FILE_DELETE, r_celery)
        ...
        with get_session_with_current_tenant() as db_session:
            ...
            for user_file_id in user_file_ids:
                ...
                try:
                    self.app.send_task(
                        DocubrainCeleryTask.DELETE_SINGLE_USER_FILE,
                        kwargs={
                            "user_file_id": str(user_file_id),
                            "tenant_id": tenant_id,
                        },
                        queue=DocubrainCeleryQueues.USER_FILE_DELETE,
                        priority=DocubrainCeleryPriority.HIGH,
...
@shared_task(
    name=DocubrainCeleryTask.DELETE_SINGLE_USER_FILE,
    bind=True,
    ignore_result=True,
)
...
@shared_task(
    name=DocubrainCeleryTask.CHECK_FOR_USER_FILE_PROJECT_SYNC,
    soft_time_limit=300,
    bind=True,
    ignore_result=True,
)
def check_for_user_file_project_sync(self: Task, *, tenant_id: str) -> None:
    ...
    redis_client = get_redis_client(tenant_id=tenant_id)
    lock: RedisLock = redis_client.lock(
        DocubrainRedisLocks.USER_FILE_PROJECT_SYNC_BEAT_LOCK,
        timeout=CELERY_GENERIC_BEAT_LOCK_TIMEOUT,
    )
...
@shared_task(
    name=DocubrainCeleryTask.PROCESS_SINGLE_USER_FILE_PROJECT_SYNC,
    bind=True,
    ignore_result=True,
)
