...
import docubrain.background.celery.apps.app_base as app_base
...
from docubrain.configs.app_configs import CELERY_WORKER_PRIMARY_POOL_OVERFLOW
from docubrain.configs.constants import CELERY_PRIMARY_WORKER_LOCK_TIMEOUT
...
from docubrain.configs.constants import POSTGRES_CELERY_WORKER_PRIMARY_APP_NAME
...
from docubrain.redis.redis_connector_stop import RedisConnectorStop
...
celery_app.config_from_object("docubrain.background.celery.configs.primary")
...
def on_worker_init(sender: Worker, **kwargs: Any) -> None:
    ...
    r.delete(DocubrainRedisLocks.PRIMARY_WORKER)
    ...
    lock: RedisLock = r.lock(
        DocubrainRedisLocks.PRIMARY_WORKER,
    ...
    r.delete(DocubrainRedisLocks.CHECK_VESPA_SYNC_BEAT_LOCK)

    r.delete(DocubrainRedisConstants.ACTIVE_FENCES)
...
celery_app.autodiscover_tasks(
    app_base.filter_task_modules(
        [
            ...
            "docubrain.background.celery.tasks.evals",
            "docubrain.background.celery.tasks.hierarchyfetching",
            "docubrain.background.celery.tasks.pruning",
            "docubrain.background.celery.tasks.shared",
            "docubrain.background.celery.tasks.vespa",
            "docubrain.background.celery.tasks.llm_model_update",
            "docubrain.background.celery.tasks.user_file_processing",
        ]
    )
)
