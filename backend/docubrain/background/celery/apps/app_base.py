...
from docubrain.background.celery.apps.task_formatters import CeleryTaskColoredFormatter
from docubrain.background.celery.apps.task_formatters import CeleryTaskPlainFormatter
from docubrain.background.celery.celery_utils import celery_is_worker_primary
from docubrain.background.celery.celery_utils import make_probe_path
from docubrain.background.celery.tasks.vespa.document_sync import DOCUMENT_SYNC_PREFIX
from docubrain.background.celery.tasks.vespa.document_sync import DOCUMENT_SYNC_TASKSET_KEY
...
from docubrain.db.engine.sql_engine import get_sqlalchemy_engine
from docubrain.document_index.opensearch.client import (
...
from docubrain.redis.redis_connector_ext_group_sync import RedisConnectorExternalGroupSync
...
from docubrain.utils.logger import ColoredFormatter
from docubrain.utils.logger import LoggerContextVars
from docubrain.utils.logger import PlainFormatter
...
def on_task_postrun(
    ...
    **kwds: Any,  # noqa: ARG001
    ...
    if task.name.startswith(DOCUBRAIN_CLOUD_CELERY_TASK_PREFIX):
...
def on_secondary_worker_init(sender: Any, **kwargs: Any) -> None:  # noqa: ARG001
    ...
    while True:
        if r.exists(DocubrainRedisLocks.PRIMARY_WORKER):
            break
...
class TenantContextFilter(logging.Filter):
    ...
    def filter(self, record: logging.LogRecord) -> bool:
        ...
        if tenant_id:
            # Match the 8 character tenant abbreviation used in DocubrainLoggingAdapter
...
_VECTOR_DB_TASK_MODULES: set[str] = {
    "docubrain.background.celery.tasks.connector_deletion",
    "docubrain.background.celery.tasks.docprocessing",
    "docubrain.background.celery.tasks.docfetching",
    "docubrain.background.celery.tasks.pruning",
    "docubrain.background.celery.tasks.vespa",
    "docubrain.background.celery.tasks.opensearch_migration",
    "docubrain.background.celery.tasks.doc_permission_syncing",
    "docubrain.background.celery.tasks.hierarchyfetching",
}
# NOTE: "docubrain.background.celery.tasks.shared" is intentionally NOT in the set
