...
from docubrain.access.access import source_should_fetch_permissions_during_indexing
from docubrain.background.indexing.checkpointing_utils import check_checkpoint_size
from docubrain.background.indexing.checkpointing_utils import get_latest_valid_checkpoint
from docubrain.background.indexing.checkpointing_utils import save_checkpoint
from docubrain.background.indexing.memory_tracer import MemoryTracer
from docubrain.configs.app_configs import INDEX_BATCH_SIZE
from docubrain.configs.app_configs import INDEXING_SIZE_WARNING_THRESHOLD
from docubrain.configs.app_configs import INDEXING_TRACER_INTERVAL
from docubrain.configs.app_configs import INTEGRATION_TESTS_MODE
from docubrain.configs.app_configs import LEAVE_CONNECTOR_ACTIVE_ON_INITIALIZATION_FAILURE
from docubrain.configs.app_configs import MAX_FILE_SIZE_BYTES
from docubrain.configs.app_configs import POLL_CONNECTOR_OFFSET
...
from docubrain.connectors.connector_runner import ConnectorRunner
from docubrain.connectors.exceptions import ConnectorValidationError
from docubrain.connectors.exceptions import UnexpectedValidationError
from docubrain.connectors.factory import instantiate_connector
from docubrain.connectors.interfaces import CheckpointedConnector
...
from docubrain.db.connector_credential_pair import get_last_successful_attempt_poll_range_end
from docubrain.db.connector_credential_pair import update_connector_credential_pair
from docubrain.db.constants import CONNECTOR_VALIDATION_ERROR_MESSAGE_PREFIX
from docubrain.db.document import mark_document_as_indexed_for_cc_pair__no_commit
...
from docubrain.db.enums import IndexModelStatus
...
from docubrain.db.index_attempt import get_recent_completed_attempts_for_cc_pair
from docubrain.db.index_attempt import mark_attempt_canceled
from docubrain.db.index_attempt import mark_attempt_failed
from docubrain.db.index_attempt import transition_attempt_to_in_progress
...
from docubrain.indexing.indexing_pipeline import index_doc_batch_prepare
...
from docubrain.server.features.build.indexing.persistent_document_writer import (
...
from docubrain.utils.postgres_sanitization import sanitize_document_for_postgres
from docubrain.utils.postgres_sanitization import sanitize_hierarchy_nodes_for_postgres
...
def connector_document_extraction(
    ...
    with get_session_with_current_tenant() as db_session:
        ...
        docprocessing_priority = (
            DocubrainCeleryPriority.MEDIUM
            if has_successful_attempt
            else DocubrainCeleryPriority.HIGH
        )
    ...
    try:
                ...
                if processing_mode == ProcessingMode.FILE_SYSTEM:
                    ...
                    with get_session_with_current_tenant() as db_session:
                        ...
                        index_attempt_metadata = IndexAttemptMetadata(
                            ...
                            connector_id=db_connector.id,
                            credential_id=db_credential.id,
                            request_id=make_randomized_docubrain_request_id("FSI"),
                    ...
                    app.send_task(
                        DocubrainCeleryTask.DOCPROCESSING_TASK,
                        kwargs=processing_batch_data,
                        queue=DocubrainCeleryQueues.DOCPROCESSING,
        ...
        if processing_mode == ProcessingMode.FILE_SYSTEM:
            ...
            if creator_id:
                ...
                app.send_task(
                    DocubrainCeleryTask.SANDBOX_FILE_SYNC,
                    kwargs={
...
def reissue_old_batches(
    ...
    priority: DocubrainCeleryPriority,
) -> tuple[int, int]:
