...
from docubrain.access.models import DocumentAccess
from docubrain.configs.app_configs import MAX_CHUNKS_PER_DOC_BATCH
from docubrain.configs.app_configs import VERIFY_CREATE_OPENSEARCH_INDEX_ON_INIT_MT
from docubrain.configs.chat_configs import NUM_RETURNED_HITS
from docubrain.configs.chat_configs import TITLE_CONTENT_RATIO
from docubrain.configs.constants import PUBLIC_DOC_PAT
from docubrain.connectors.cross_connector_utils.miscellaneous_utils import (
    get_experts_stores_representations,
)
from docubrain.connectors.models import convert_metadata_list_of_strings_to_dict
from docubrain.context.search.enums import QueryType
from docubrain.context.search.models import IndexFilters
from docubrain.context.search.models import InferenceChunk
from docubrain.context.search.models import InferenceChunkUncleaned
from docubrain.context.search.models import QueryExpansionType
from docubrain.db.enums import EmbeddingPrecision
from docubrain.db.models import DocumentSource
from docubrain.document_index.chunk_content_enrichment import cleanup_content_for_chunks
from docubrain.document_index.chunk_content_enrichment import (
    generate_enriched_content_for_chunk_text,
)
from docubrain.document_index.interfaces import DocumentIndex as OldDocumentIndex
from docubrain.document_index.interfaces import (
    DocumentInsertionRecord as OldDocumentInsertionRecord,
)
from docubrain.document_index.interfaces import IndexBatchParams
from docubrain.document_index.interfaces import VespaChunkRequest
...
from docubrain.document_index.interfaces_new import DocumentIndex
from docubrain.document_index.interfaces_new import DocumentInsertionRecord
from docubrain.document_index.interfaces_new import DocumentSectionRequest
from docubrain.document_index.interfaces_new import IndexingMetadata
from docubrain.document_index.interfaces_new import MetadataUpdateRequest
from docubrain.document_index.interfaces_new import TenantState
from docubrain.document_index.opensearch.client import OpenSearchClient
from docubrain.document_index.opensearch.client import OpenSearchIndexClient
from docubrain.document_index.opensearch.client import SearchHit
from docubrain.document_index.opensearch.cluster_settings import OPENSEARCH_CLUSTER_SETTINGS
from docubrain.document_index.opensearch.constants import OpenSearchSearchType
from docubrain.document_index.opensearch.schema import ACCESS_CONTROL_LIST_FIELD_NAME
from docubrain.document_index.opensearch.schema import CONTENT_FIELD_NAME
from docubrain.document_index.opensearch.schema import DOCUMENT_SETS_FIELD_NAME
from docubrain.document_index.opensearch.schema import DocumentChunk
from docubrain.document_index.opensearch.schema import DocumentChunkWithoutVectors
from docubrain.document_index.opensearch.schema import DocumentSchema
from docubrain.document_index.opensearch.schema import get_opensearch_doc_chunk_id
from docubrain.document_index.opensearch.schema import GLOBAL_BOOST_FIELD_NAME
from docubrain.document_index.opensearch.schema import HIDDEN_FIELD_NAME
from docubrain.document_index.opensearch.schema import PERSONAS_FIELD_NAME
from docubrain.document_index.opensearch.schema import USER_PROJECTS_FIELD_NAME
from docubrain.document_index.opensearch.search import DocumentQuery
from docubrain.document_index.opensearch.search import (
...
from docubrain.indexing.models import DocMetadataAwareIndexChunk
from docubrain.indexing.models import Document
from docubrain.utils.logger import setup_logger
from docubrain.utils.text_processing import remove_invalid_unicode_chars
...
def set_cluster_state(client: OpenSearchClient) -> None:
    if not client.put_cluster_settings(settings=OPENSEARCH_CLUSTER_SETTINGS):
        logger.error(
            ...
            "is not the first time running DocuBrain against this instance of OpenSearch, these "
...
def _convert_retrieved_opensearch_chunk_to_inference_chunk_uncleaned(
    ...
    Returns:
        A DocuBrain inference chunk representation.
    """
...
def _convert_docubrain_chunk_to_opensearch_document(
    ...
    def index(
        ...
        def _flush_chunks(doc_chunks: list[DocMetadataAwareIndexChunk]) -> None:
            ...
            chunk_batch: list[DocumentChunk] = [
                _convert_docubrain_chunk_to_opensearch_document(chunk)
                for chunk in doc_chunks
            ]
            docubrain_document: Document = doc_chunks[0].source_document
            ...
            if docubrain_document.id not in deleted_doc_ids:
                num_chunks_deleted = self.delete(
                    docubrain_document.id, docubrain_document.chunk_count
                )
                deleted_doc_ids.add(docubrain_document.id)
                ...
                document_indexing_results.append(
                    DocumentInsertionRecord(
                        document_id=docubrain_document.id,
    ...
    def delete(
        ...
        Args:
            document_id: The unique identifier for the document as represented
                in DocuBrain, not necessarily in the document index.
