...
from docubrain.configs.chat_configs import VESPA_SEARCHER_THREADS
from docubrain.configs.constants import KV_REINDEX_KEY
...
from docubrain.document_index.interfaces import DocumentIndex
...
from docubrain.document_index.vespa.vespa_document_index import TenantState
...
from docubrain.document_index.vespa_constants import VESPA_APPLICATION_ENDPOINT
...
from docubrain.key_value_store.factory import get_shared_kv_store
from docubrain.kg.utils.formatting_utils import split_relationship_id
...
logger = setup_logger()
VESPA_CONTAINER_ALLOCATED_MEMORY = os.environ.get(
    "VESPA_CONTAINER_ALLOCATED_MEMORY", "5%"
)
...
class VespaIndex(DocumentIndex):
    ...
    def ensure_indices_exist(
        ...
        vespa_schema_path = os.path.join(
            os.getcwd(), "docubrain", "document_index", "vespa", "app_config"
        )
        ...
        with open(services_jinja_file, "r") as services_f:
            ...
            services = services_template.render(
                ...
                container_jvm_allocated_memory=VESPA_CONTAINER_ALLOCATED_MEMORY,
            )
        ...
        if response.status_code != 200:
            logger.error(
                f"Failed to prepare Vespa DocuBrain Index. Response: {response.text}"
            )
            raise RuntimeError(
                f"Failed to prepare Vespa DocuBrain Index. Response: {response.text}"
            )
    ...
    def register_multitenant_indices(
        ...
        if response.status_code != 200:
            raise RuntimeError(
                f"Failed to prepare Vespa DocuBrain Indexes. Response: {response.text}"
            )
