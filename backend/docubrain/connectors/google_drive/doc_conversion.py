...
from docubrain.connectors.google_drive.constants import DRIVE_FOLDER_TYPE
from docubrain.connectors.google_drive.constants import DRIVE_SHORTCUT_TYPE
from docubrain.connectors.google_drive.models import GDriveMimeType
from docubrain.connectors.google_drive.models import GoogleDriveFileType
from docubrain.connectors.google_drive.section_extraction import get_document_sections
from docubrain.connectors.google_drive.section_extraction import HEADING_DELIMITER
from docubrain.connectors.google_utils.resources import get_drive_service
from docubrain.connectors.google_utils.resources import get_google_docs_service
from docubrain.connectors.google_utils.resources import GoogleDocsService
...
from docubrain.file_processing.extract_file_text import get_file_ext
...
from docubrain.file_processing.image_utils import store_image_and_create_section
...
def docubrain_document_id_from_drive_file(file: GoogleDriveFileType) -> str:
...
def _download_and_extract_sections_basic(
    ...
    if file_ext not in DocubrainFileExtensions.ALL_ALLOWED_EXTENSIONS:
...
def _get_external_access_for_raw_gdrive_file(
    ...
    external_access_fn = cast(
        ...
        fetch_versioned_implementation_with_fallback(
            "docubrain.external_permissions.google_drive.doc_sync",
...
def _convert_drive_item_to_document(
    ...
    try:
        ...
        doc_id = docubrain_document_id_from_drive_file(file)
        external_access = (
...
def build_slim_document(
    ...
    return SlimDocument(
        id=docubrain_document_id_from_drive_file(file),
        external_access=external_access,
        parent_hierarchy_raw_node_id=(file.get("parents") or [None])[0],
    )
