...
from docubrain.configs.app_configs import INDEXING_EMBEDDING_MODEL_NUM_THREADS
from docubrain.configs.app_configs import LARGE_CHUNK_RATIO
from docubrain.configs.model_configs import BATCH_SIZE_ENCODE_CHUNKS
...
from docubrain.natural_language_processing.constants import DEFAULT_COHERE_MODEL
from docubrain.natural_language_processing.constants import DEFAULT_OPENAI_MODEL
from docubrain.natural_language_processing.constants import DEFAULT_VERTEX_MODEL
from docubrain.natural_language_processing.constants import DEFAULT_VOYAGE_MODEL
from docubrain.natural_language_processing.constants import EmbeddingModelTextType
from docubrain.natural_language_processing.exceptions import CohereBillingLimitError
from docubrain.natural_language_processing.exceptions import ModelServerRateLimitError
from docubrain.natural_language_processing.utils import get_tokenizer
from docubrain.natural_language_processing.utils import tokenizer_trim_content
from docubrain.utils.logger import setup_logger
from docubrain.utils.search_nlp_models_utils import pass_aws_key
...
WARM_UP_STRINGS = [
    "DocuBrain is amazing!",
    "Check out our easy deployment guide at",
    "https://docs.docubrain.app/deployment/getting_started/quickstart",
]
...
class CloudEmbedding:
    ...
    async def _embed_cohere(
        ...
        for text_batch in batch_list(texts, _COHERE_MAX_INPUT_LEN):
            # Does not use the same tokenizer as the DocuBrain API server but it's approximately the same
...
class EmbeddingModel:
    ...
    def _make_model_server_request(
        ...
        def _make_request() -> Response:
            headers = {}
            if tenant_id:
                headers["X-DocuBrain-Tenant-ID"] = tenant_id

            if request_id:
                headers["X-DocuBrain-Request-ID"] = request_id
