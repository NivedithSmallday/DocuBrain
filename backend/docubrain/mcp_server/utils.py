"""Utility helpers for the DocuBrain MCP server."""
...
def require_access_token() -> AccessToken:
    ...
    if not access_token:
        raise ValueError(
            "MCP Server requires a DocuBrain access token to authenticate your request"
        )
    return access_token
...
async def get_indexed_sources(
    ...
    try:
        ...
        logger.error(
            "DocuBrain MCP Server: Failed to fetch indexed sources",
            exc_info=True,
        )
        raise
        ...
        logger.error(
            "DocuBrain MCP Server: Unexpected error fetching indexed sources",
            exc_info=True,
        )
