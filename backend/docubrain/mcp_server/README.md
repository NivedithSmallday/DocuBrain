# DocuBrain MCP Server

## Overview

The DocuBrain MCP server allows LLMs to connect to your DocuBrain instance and access its knowledge base and search capabilities through the [Model Context Protocol (MCP)](https://modelcontextprotocol.io/).

With the DocuBrain MCP Server, you can search your knowledgebase,
give your LLMs web search, and upload and manage documents in DocuBrain.

All access controls are managed within the main DocuBrain application.

### Authentication

Provide a DocuBrain Personal Access Token or API Key in the `Authorization` header as a Bearer token.
...
The MCP server is built on [FastMCP](https://github.com/jlowin/fastmcp) and runs alongside the main DocuBrain API server:
...
```json
{
  "mcpServers": {
    "docubrain": {
      "url": "https://[YOUR_DOCUBRAIN_DOMAIN]:8090/",
      "transport": "http",
      "headers": {
        "Authorization": "Bearer YOUR_DOCUBRAIN_TOKEN_HERE"
      }
    }
  }
}
...
Search the user's private knowledge base indexed in DocuBrain. Returns ranked documents with content snippets, scores, and metadata.
...
The MCP Server automatically launches with the `Run All DocuBrain Services` task from the default launch.json.
...
4. Paste your DocuBrain bearer token
5. Click **Connect**
...
- `API_SERVER_URL_OVERRIDE_FOR_HTTP_REQUESTS`: Optional override URL. If set, takes precedence over the protocol/host variables. Used for self-hosting the MCP server with DocuBrain Cloud as the backend.