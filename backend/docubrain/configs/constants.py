...
DOCUBRAIN_DEFAULT_APPLICATION_NAME = "DocuBrain"
DOCUBRAIN_DISCORD_URL = "https://discord.gg/4NA5SbzrWb"
DOCUBRAIN_UTM_SOURCE = "docubrain_app"
SLACK_USER_TOKEN_PREFIX = "xoxp-"
SLACK_BOT_TOKEN_PREFIX = "xoxb-"
DOCUBRAIN_EMAILABLE_LOGO_MAX_DIM = 512
...
TENANT_ID_COOKIE_NAME = "docubrain_tid"  # tenant id - for workaround cases
ANONYMOUS_USER_COOKIE_NAME = "docubrain_anonymous_user"
...
DOCUBRAIN_METADATA_FILENAME = ".docubrain_metadata.json"

# Messages
DISABLED_GEN_AI_MSG = (
    "Your System Admin has disabled the Generative AI functionalities of DocuBrain.\n"
    "Please contact them if you wish to have this enabled.\n"
    "You can still use DocuBrain as a search engine."
)
...
DANSWER_API_KEY_DUMMY_EMAIL_DOMAIN = "docubrainapikey.ai"
...
KV_SETTINGS_KEY = "docubrain_settings"
KV_CUSTOMER_UUID_KEY = "customer_uuid"
KV_INSTANCE_DOMAIN_KEY = "instance_domain"
KV_ENTERPRISE_SETTINGS_KEY = "docubrain_enterprise_settings"
...
class DocumentSource(str, Enum):
    # Special case, document passed in via DocuBrain APIs without specifying a source type
...
class MessageType(str, Enum):
    ...
    USER_REMINDER = "user_reminder"  # Custom DocuBrain message type which is translated into a USER message when passed to the LLM
...
class MilestoneRecordType(str, Enum):
    ...
    CREATED_DOCUBRAIN_BOT = "created_docubrain_bot"
    REQUESTED_CONNECTOR = "requested_connector"


class DocubrainCeleryQueues:
...
class DocubrainRedisConstants:
    ACTIVE_FENCES = "active_fences"


class DocubrainCeleryPriority(int, Enum):
    HIGHEST = 0
    HIGH = auto()
    MEDIUM = auto()
    LOW = auto()
...
DOCUBRAIN_CLOUD_CELERY_TASK_PREFIX = "cloud"

# the tenant id we use for system level redis operations
DOCUBRAIN_CLOUD_TENANT_ID = "cloud"

# the redis namespace for runtime variables
DOCUBRAIN_CLOUD_REDIS_RUNTIME = "runtime"
CLOUD_BUILD_FENCE_LOOKUP_TABLE_INTERVAL_DEFAULT = 600


class DocubrainCeleryTask:
    DEFAULT = "celery"

    CLOUD_BEAT_TASK_GENERATOR = f"{DOCUBRAIN_CLOUD_CELERY_TASK_PREFIX}_generate_beat_tasks"
    CLOUD_MONITOR_ALEMBIC = f"{DOCUBRAIN_CLOUD_CELERY_TASK_PREFIX}_monitor_alembic"
    CLOUD_MONITOR_CELERY_QUEUES = (
        f"{DOCUBRAIN_CLOUD_CELERY_TASK_PREFIX}_monitor_celery_queues"
    )
    CLOUD_CHECK_AVAILABLE_TENANTS = (
        f"{DOCUBRAIN_CLOUD_CELERY_TASK_PREFIX}_check_available_tenants"
    )
    CLOUD_MONITOR_CELERY_PIDBOX = (
        f"{DOCUBRAIN_CLOUD_CELERY_TASK_PREFIX}_monitor_celery_pidbox"
    )
...
DOCUBRAIN_CELERY_BEAT_HEARTBEAT_KEY = "docubrain:celery:beat:heartbeat"
...
class DocubrainCallTypes(str, Enum):
    FIREFLIES = "FIREFLIES"
    GONG = "GONG"
