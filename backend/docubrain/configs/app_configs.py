...
from docubrain.configs.constants import QueryHistoryType
from docubrain.file_processing.enums import HtmlBasedConnectorTransformLinksStrategy
from docubrain.prompts.image_analysis import DEFAULT_IMAGE_SUMMARIZATION_SYSTEM_PROMPT
from docubrain.prompts.image_analysis import DEFAULT_IMAGE_SUMMARIZATION_USER_PROMPT
...
# This override allows self-hosting the MCP server with DocuBrain Cloud backend.
...
# 3. no queries
DOCUBRAIN_QUERY_HISTORY_TYPE = QueryHistoryType(
    (os.environ.get("DOCUBRAIN_QUERY_HISTORY_TYPE") or QueryHistoryType.NORMAL.value).lower()
)
...
# NOTE: if you are having problems accessing the DocuBrain web UI locally (especially
...
# and is available in DocuBrain EE
...
# restrict access to DocuBrain to only users with emails from those domains.
# E.g. `VALID_EMAIL_DOMAINS=example.com,example.org` will restrict DocuBrain
...
SAML_CONF_DIR = os.environ.get("SAML_CONF_DIR") or "/app/docubrain/configs/saml_config"
...
# If set, DocuBrain will listen to the `expires_at` returned by the identity
...
# of OpenSearch running for the relevant DocuBrain instance.
# NOTE: Now enabled on by default, unless the env indicates otherwise.
ENABLE_OPENSEARCH_INDEXING_FOR_DOCUBRAIN = (
    os.environ.get("ENABLE_OPENSEARCH_INDEXING_FOR_DOCUBRAIN", "true").lower() == "true"
)
...
ENABLE_OPENSEARCH_RETRIEVAL_FOR_DOCUBRAIN = (
    ENABLE_OPENSEARCH_INDEXING_FOR_DOCUBRAIN
    and os.environ.get("ENABLE_OPENSEARCH_RETRIEVAL_FOR_DOCUBRAIN", "").lower() == "true"
)
...
DOCUBRAIN_SEARCH_UI_USES_OPENSEARCH_KEYWORD_SEARCH = (
    os.environ.get("DOCUBRAIN_SEARCH_UI_USES_OPENSEARCH_KEYWORD_SEARCH", "").lower()
    == "true"
)
...
# View the list here:
# https://github.com/docubrain-dot-app/docubrain/blob/main/backend/docubrain/connectors/factory.py
...
CONFLUENCE_USE_DOCUBRAIN_USERS_FOR_GROUP_SYNC = (
    os.environ.get("CONFLUENCE_USE_DOCUBRAIN_USERS_FOR_GROUP_SYNC", "").lower() == "true"
)
...
# LLM used to generate chunk/document summaries when ENABLE_CONTEXTUAL_RAG is on.
# Overridable via env so production deployments can point at a real provider
# instead of the dev preset; the per-tenant SearchSettings value still takes
# precedence over these defaults (see indexing_pipeline.py).
DEFAULT_CONTEXTUAL_RAG_LLM_NAME = (
    os.environ.get("CONTEXTUAL_RAG_LLM_NAME") or "gpt-4o-mini"
)
DEFAULT_CONTEXTUAL_RAG_LLM_PROVIDER = (
    os.environ.get("CONTEXTUAL_RAG_LLM_PROVIDER") or "DevEnvPresetOpenAI"
)
...
# backend/docubrain/document_index/vespa/app_config/schemas/danswer_chunk.sd.jinja.
...
# Logs DocuBrain only model interactions like prompts, responses, messages etc.
LOG_DOCUBRAIN_MODEL_INTERACTIONS = (
    os.environ.get("LOG_DOCUBRAIN_MODEL_INTERACTIONS", "").lower() == "true"
)
...
BRAINTRUST_PROJECT = os.environ.get("BRAINTRUST_PROJECT", "DocuBrain")
...
SCHEDULED_EVAL_PERMISSIONS_EMAIL = os.environ.get(
    "SCHEDULED_EVAL_PERMISSIONS_EMAIL", "roshan@docubrain.app"
)
...
AUTO_LLM_CONFIG_URL = os.environ.get(
    "AUTO_LLM_CONFIG_URL",
    "https://raw.githubusercontent.com/docubrain-dot-app/docubrain/main/backend/docubrain/llm/well_known_providers/recommended-models.json",
)
...
#####
# Enterprise layer has been removed from this codebase.
ENTERPRISE_EDITION_ENABLED = False
...
S3_FILE_STORE_BUCKET_NAME = (
    os.environ.get("S3_FILE_STORE_BUCKET_NAME") or "docubrain-file-store-bucket"
)
S3_FILE_STORE_PREFIX = os.environ.get("S3_FILE_STORE_PREFIX") or "docubrain-files"
...
# These are DocuBrain-managed API keys provided to tenants by default
#####
...
STRIPE_PUBLISHABLE_KEY_URL = (
    "https://docubrain-stripe-public.s3.amazonaws.com/publishable-key.txt"
)
