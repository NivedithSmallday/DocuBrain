...
def run_remote(
    ...
    Args:
        base_url: Base URL of the remote server (e.g., "https://test.docubrain.app")
...
def main() -> None:
    ...
    parser.add_argument(
        "--base-url",
        type=str,
        default="https://test.docubrain.app",
        help="Base URL of the remote server (default: https://test.docubrain.app)",
    )
    ...
    if args.remote:
        if not args.api_key:
            print("Using API Key from DOCUBRAIN_EVAL_API_KEY")
        api_key: str = (
            args.api_key if args.api_key else os.environ.get("DOCUBRAIN_EVAL_API_KEY", "")
        )
