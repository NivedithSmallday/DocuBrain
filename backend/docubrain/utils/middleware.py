...
from shared_configs.contextvars import DOCUBRAIN_REQUEST_ID_CONTEXTVAR


def add_docubrain_tenant_id_middleware(
    app: FastAPI,
    ...
    async def set_tenant_id(
        ...
        docubrain_tenant_id = request.headers.get("X-DocuBrain-Tenant-ID")
        if docubrain_tenant_id:
            CURRENT_TENANT_ID_CONTEXTVAR.set(docubrain_tenant_id)
        return await call_next(request)


def add_docubrain_request_id_middleware(
    app: FastAPI,
    prefix: str,
    logger: logging.LoggerAdapter,  # noqa: ARG001
) -> None:
    ...
    async def set_request_id(
        ...
        docubrain_request_id = request.headers.get("X-DocuBrain-Request-ID")
        if not docubrain_request_id:
            docubrain_request_id = make_randomized_docubrain_request_id(prefix)

        DOCUBRAIN_REQUEST_ID_CONTEXTVAR.set(docubrain_request_id)
        return await call_next(request)


def make_randomized_docubrain_request_id(prefix: str) -> str:
    """generates a randomized request id"""

    hash_input = str(uuid.uuid4())
    return _make_docubrain_request_id(prefix, hash_input)


def make_structured_docubrain_request_id(prefix: str, request_url: str) -> str:
    """Not used yet, but could be in the future!"""
    hash_input = f"{request_url}:{datetime.now(timezone.utc)}"
    return _make_docubrain_request_id(prefix, hash_input)


def _make_docubrain_request_id(prefix: str, hash_input: str) -> str:
    ...
    docubrain_request_id = f"{prefix}:{hash_str}"
    return docubrain_request_id
...
def add_endpoint_context_middleware(app: FastAPI) -> None:
    ...
    Used by ``docubrain_db_connections_held_by_endpoint`` and
    ``docubrain_db_connection_hold_seconds`` in the pool event listeners.
