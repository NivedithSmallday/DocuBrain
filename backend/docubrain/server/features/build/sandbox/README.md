# DocuBrain Sandbox System

This directory contains the implementation of DocuBrain's sandbox system for running OpenCode agents in isolated environments.
...
cd backend
docker build -f Dockerfile.sandbox-templates -t docubraindotapp/backend:latest .

# Build sandbox container (lightweight runner)
cd docubrain/server/features/build/sandbox/kubernetes/docker
docker build -t docubraindotapp/sandbox:latest .
...
**Only needed if you're running the DocuBrain backend outside of Docker.** Most developers use Docker and can skip this section.

If you're running the backend Python process directly on your machine, you need templates at `/templates/`:

#### Web Template

The web template is a lightweight Next.js app (Next.js 16, React 19, shadcn/ui, Recharts) checked into the codebase at `backend/docubrain/server/features/build/templates/outputs/web/`.

For local development, create a symlink to this template:

```bash
sudo mkdir -p /templates/outputs
sudo ln -s $(pwd)/backend/docubrain/server/features/build/templates/outputs/web /templates/outputs/web
```
...
cd backend
python -m docubrain.server.features.build.sandbox.util.build_venv_template

# Or manually
python3 -m venv /templates/venv
/templates/venv/bin/pip install -r backend/docubrain/server/features/build/sandbox/kubernetes/docker/initial-requirements.txt
```
...
SANDBOX_BASE_PATH=/tmp/docubrain-sandboxes     # Default: /tmp/docubrain-sandboxes
...
SANDBOX_NAMESPACE=docubrain-sandboxes          # Default: docubrain-sandboxes

# Container image
SANDBOX_CONTAINER_IMAGE=docubraindotapp/sandbox:latest

# S3 bucket for snapshots and files
SANDBOX_S3_BUCKET=docubrain-sandbox-files      # Default: docubrain-sandbox-files
...
- Check pod logs: `kubectl logs -n docubrain-sandboxes sandbox-{sandbox-id}`
- Check init container: `kubectl logs -n docubrain-sandboxes sandbox-{sandbox-id} -c file-sync`
- Verify init container completed: `kubectl describe pod -n docubrain-sandboxes sandbox-{sandbox-id}`
...
- **Docker/K8s**: Check container logs: `kubectl logs -n docubrain-sandboxes sandbox-{sandbox-id}`
- Verify npm install succeeded (check entrypoint.sh logs)
- Check that web template was copied: `kubectl exec -n docubrain-sandboxes sandbox-{sandbox-id} -- ls /workspace/outputs/web`
...
```bash
export OUTPUTS_TEMPLATE_PATH=$HOME/.docubrain/templates/outputs
export VENV_TEMPLATE_PATH=$HOME/.docubrain/templates/venv

# Then symlink to your home directory
mkdir -p $HOME/.docubrain/templates/outputs
ln -s $(pwd)/backend/docubrain/server/features/build/templates/outputs/web $HOME/.docubrain/templates/outputs/web
```
...
The lightweight Next.js template (`backend/docubrain/server/features/build/templates/outputs/web/`) includes:
...
This template provides a modern development environment without the complexity of the full DocuBrain application, allowing agents to build custom UIs quickly.
