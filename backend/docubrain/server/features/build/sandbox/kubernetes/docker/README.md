...
This directory contains the Dockerfile and resources for building the DocuBrain Craft sandbox container image.
...
```bash
cd backend/docubrain/server/features/build/sandbox/kubernetes/docker
docker build --platform linux/amd64 -t docubraindotapp/sandbox:v0.1.x .
docker push docubraindotapp/sandbox:v0.1.x
```
...
docker buildx build --platform linux/amd64,linux/arm64 \
  -t docubraindotapp/sandbox:v0.1.x \
  --push .
```
...
```bash
docker tag docubraindotapp/sandbox:v0.1.x docubraindotapp/sandbox:latest
docker push docubraindotapp/sandbox:latest
```
...
docker buildx build --platform linux/amd64,linux/arm64 \
  -t docubraindotapp/sandbox:v0.1.x \
  -t docubraindotapp/sandbox:latest \
  --push .
```
...
2. **Update the ConfigMap** in in the internal repo
   ```yaml
   SANDBOX_CONTAINER_IMAGE: "docubraindotapp/sandbox:v0.1.x"
   ```
...
5. **Delete existing sandbox pods** (they will be recreated with the new image):
   ```bash
   kubectl delete pods -n docubrain-sandboxes -l app.kubernetes.io/component=sandbox
   ```
...
```bash
curl -s "https://hub.docker.com/v2/repositories/docubraindotapp/sandbox/tags" | jq '.results[].name'
```

### Check what image a pod is using

```bash
kubectl get pod <pod-name> -n docubrain-sandboxes -o jsonpath='{.spec.containers[?(@.name=="sandbox")].image}'
```
