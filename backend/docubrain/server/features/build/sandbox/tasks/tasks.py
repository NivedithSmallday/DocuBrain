...
from docubrain.configs.constants import CELERY_SANDBOX_FILE_SYNC_LOCK_TIMEOUT
...
from docubrain.server.features.build.configs import SANDBOX_IDLE_TIMEOUT_SECONDS
from docubrain.server.features.build.configs import SandboxBackend
from docubrain.server.features.build.configs import USER_LIBRARY_SOURCE_DIR
from docubrain.server.features.build.db.build_session import clear_nextjs_ports_for_user
...
from docubrain.server.features.build.sandbox.kubernetes.kubernetes_sandbox_manager import (
...
@shared_task(
    name=DocubrainCeleryTask.CLEANUP_IDLE_SANDBOXES,
...
def cleanup_idle_sandboxes_task(self: Task, *, tenant_id: str) -> None:  # noqa: ARG001
    ...
    lock: RedisLock = redis_client.lock(
        DocubrainRedisLocks.CLEANUP_IDLE_SANDBOXES_BEAT_LOCK,
    ...
    try:
        ...
        from docubrain.db.enums import SandboxStatus
        from docubrain.server.features.build.db.sandbox import create_snapshot__no_commit
        from docubrain.server.features.build.db.sandbox import get_idle_sandboxes
...
def _get_disabled_user_library_paths(db_session: "Session", user_id: str) -> list[str]:
    ...
    from docubrain.db.document import get_documents_by_source
...
@shared_task(
    name=DocubrainCeleryTask.SANDBOX_FILE_SYNC,
    soft_time_limit=TIMEOUT_SECONDS,
    bind=True,
    ignore_result=True,
)
def sync_sandbox_files(
    ...
    *,
    ...
    lock = redis_client.lock(
        f"{DocubrainRedisLocks.SANDBOX_FILE_SYNC_LOCK_PREFIX}:{user_id}",
...
# @shared_task(
#     name=DocubrainCeleryTask.CLEANUP_OLD_SNAPSHOTS,
#     soft_time_limit=300,
#     bind=True,
#     ignore_result=True,
# )
...
#     redis_client = get_redis_client(tenant_id=tenant_id)
#     lock: RedisLock = redis_client.lock(
#         DocubrainRedisLocks.CLEANUP_OLD_SNAPSHOTS_BEAT_LOCK,
...
#     try:
#         from docubrain.server.features.build.db.sandbox import delete_old_snapshots
