...
from docubrain.db.enums import SharingScope
...
from docubrain.server.features.build.api.messages_api import router as messages_router
from docubrain.server.features.build.api.models import BuildConnectorInfo
from docubrain.server.features.build.api.models import BuildConnectorListResponse
from docubrain.server.features.build.api.models import BuildConnectorStatus
from docubrain.server.features.build.api.models import RateLimitResponse
from docubrain.server.features.build.api.rate_limit import get_user_rate_limit_status
from docubrain.server.features.build.api.sessions_api import router as sessions_router
from docubrain.server.features.build.api.user_library import router as user_library_router
...
from docubrain.server.features.build.utils import is_docubrain_craft_enabled
...
def require_docubrain_craft_enabled(
    user: User = Depends(require_permission(Permission.BASIC_ACCESS)),
) -> User:
    """
    Dependency that checks if DocuBrain Craft is enabled for the user.
    Raises HTTP 403 if DocuBrain Craft is disabled via feature flag.
    """
    if not is_docubrain_craft_enabled(user):
        raise HTTPException(
            status_code=403,
            detail="DocuBrain Craft is not available",
        )
    return user


router = APIRouter(prefix="/build", dependencies=[Depends(require_docubrain_craft_enabled)])
...
# prevent LLM-generated apps from setting cookies on the parent DocuBrain domain.
EXCLUDED_HEADERS = {
    "content-encoding",
    "content-length",
    "transfer-encoding",
