...
from docubrain.feature_flags.factory import get_default_feature_flag_provider
from docubrain.feature_flags.interface import NoOpFeatureFlagProvider
...
from docubrain.server.features.build.configs import MAX_UPLOAD_FILE_SIZE_BYTES
...
# Combine base DocuBrain extensions with code file extensions
ALLOWED_EXTENSIONS: set[str] = (
    DocubrainFileExtensions.ALL_ALLOWED_EXTENSIONS | CODE_FILE_EXTENSIONS
)

# Combine base DocuBrain MIME types with code MIME types
ALLOWED_MIME_TYPES: set[str] = DocubrainMimeTypes.ALLOWED_MIME_TYPES | CODE_MIME_TYPES
...
# PostHog feature flag key for enabling DocuBrain Craft (cloud rollout control)
# Flag logic: True = enabled, False/null/not found = disabled
DOCUBRAIN_CRAFT_ENABLED_FLAG = "docubrain-craft-enabled"
...
def is_docubrain_craft_enabled(user: User) -> bool:
    """
    Check if DocuBrain Craft (Build Mode) is enabled for the user.

    Flag logic for "docubrain-craft-enabled":
    - Flag = True → enabled (DocuBrain Craft is available)
    - Flag = False → disabled (DocuBrain Craft is not available)
    - Flag = null/not found → disabled (DocuBrain Craft is not available)
    ...
    is_enabled = feature_flag_provider.feature_enabled(
        DOCUBRAIN_CRAFT_ENABLED_FLAG,
        user.id,
    )

    if is_enabled:
        logger.debug("DocuBrain Craft enabled via PostHog feature flag")
        return True
    else:
        logger.debug("DocuBrain Craft disabled via PostHog feature flag")
        return False
...
def ensure_build_mode_intro_notification(user: User, db_session: Session) -> None:
    ...
    """
    # PostHog feature flag check - only show notification if DocuBrain Craft is enabled
    ...
    create_notification(
        ...
        title="Introducing DocuBrain Craft",
        description="Unleash DocuBrain to create dashboards, slides, documents, and more with your connected data.",
