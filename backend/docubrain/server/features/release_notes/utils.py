...
from docubrain.cache.factory import get_shared_cache_backend
from docubrain.configs.app_configs import INSTANCE_TYPE
from docubrain.configs.constants import DocubrainRedisLocks
from docubrain.db.release_notes import create_release_notifications_for_versions
from docubrain.server.features.release_notes.constants import AUTO_REFRESH_THRESHOLD_SECONDS
from docubrain.server.features.release_notes.constants import FETCH_TIMEOUT
from docubrain.server.features.release_notes.constants import GITHUB_CHANGELOG_RAW_URL
from docubrain.server.features.release_notes.constants import REDIS_CACHE_TTL
from docubrain.server.features.release_notes.constants import REDIS_KEY_ETAG
from docubrain.server.features.release_notes.constants import REDIS_KEY_FETCHED_AT
from docubrain.server.features.release_notes.models import ReleaseNoteEntry
        ...
        if is_valid_version(version):
            all_entries.append(
                ReleaseNoteEntry(
                    version=version,
                    date=date,
                    title=f"DocuBrain {version} is available!",
                )
            )
...
def ensure_release_notes_fresh_and_notify(db_session: Session) -> None:
    ...
    cache = get_shared_cache_backend()
    lock = cache.lock(
        DocubrainRedisLocks.RELEASE_NOTES_FETCH_LOCK,
        timeout=90,
    )
