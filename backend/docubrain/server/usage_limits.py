...
from docubrain.configs.app_configs import ANTHROPIC_DEFAULT_API_KEY
from docubrain.configs.app_configs import COHERE_DEFAULT_API_KEY
from docubrain.configs.app_configs import OPENAI_DEFAULT_API_KEY
from docubrain.configs.app_configs import OPENROUTER_DEFAULT_API_KEY
from docubrain.db.usage import check_usage_limit
from docubrain.db.usage import UsageLimitExceededError
from docubrain.db.usage import UsageType
from docubrain.server.tenant_usage_limits import TenantUsageLimitKeys
from docubrain.server.tenant_usage_limits import TenantUsageLimitOverrides
...
# Collect all DocuBrain-managed default API keys for comparison
_DOCUBRAIN_MANAGED_API_KEYS: set[str] = set()
for key in [
    ...
    if key:
        _DOCUBRAIN_MANAGED_API_KEYS.add(key)


def is_docubrain_managed_api_key(api_key: str | None) -> bool:
    """Check if the given API key is one of DocuBrain's managed default keys."""
    return bool(api_key) and api_key in _DOCUBRAIN_MANAGED_API_KEYS
...
def is_tenant_on_trial_fn(tenant_id: str) -> bool:
    ...
    fn: Callable[[str], bool] = fetch_versioned_implementation(
        "docubrain.server.usage_limits", "is_tenant_on_trial"
    )
    return fn(tenant_id)
...
def _get_tenant_override(tenant_id: str, field_name: str) -> int | None:
    ...
    try:
        ...
        get_overrides_fn = fetch_versioned_implementation(
            "docubrain.server.tenant_usage_limits", "get_tenant_usage_limit_overrides"
        )
...
def check_llm_cost_limit_for_provider(
    ...
    """
    Check if the LLM cost limit would be exceeded for a provider using DocuBrain-managed keys.

    Only enforces limits when the provider uses DocuBrain-managed API keys.
    ...
    # Only enforce limits for DocuBrain-managed API keys
    if not is_docubrain_managed_api_key(llm_provider_api_key):
        return
