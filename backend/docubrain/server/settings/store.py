...
from docubrain.configs.app_configs import DISABLE_USER_KNOWLEDGE
...
from docubrain.configs.app_configs import DOCUBRAIN_QUERY_HISTORY_TYPE
from docubrain.configs.app_configs import SHOW_EXTRA_CONNECTORS
from docubrain.configs.constants import KV_SETTINGS_KEY
...
def load_settings() -> Settings:
    ...
    try:
        ...
        if value is not None:
            ...
            cache.set(DocubrainRedisLocks.ANONYMOUS_USER_ENABLED, "0", ex=SETTINGS_TTL)
    except Exception as e:
        logger.error(f"Error loading anonymous user setting from cache: {str(e)}")
        anonymous_user_enabled = False

    settings.anonymous_user_enabled = anonymous_user_enabled
    settings.query_history_type = DOCUBRAIN_QUERY_HISTORY_TYPE

    if DISABLE_USER_KNOWLEDGE:
        settings.user_knowledge_enabled = False

    settings.show_extra_connectors = SHOW_EXTRA_CONNECTORS
    settings.opensearch_indexing_enabled = ENABLE_OPENSEARCH_INDEXING_FOR_DOCUBRAIN
...
def store_settings(settings: Settings) -> None:
    ...
    if settings.anonymous_user_enabled is not None:
        cache.set(
            DocubrainRedisLocks.ANONYMOUS_USER_ENABLED,
