...
from docubrain import __version__ as docubrain_version
from docubrain.auth.permissions import require_permission
from docubrain.auth.users import is_user_admin
from docubrain.configs.app_configs import DEFAULT_USER_FILE_MAX_UPLOAD_SIZE_MB
from docubrain.configs.app_configs import DISABLE_VECTOR_DB
from docubrain.configs.app_configs import MAX_ALLOWED_UPLOAD_SIZE_MB
...
from docubrain.db.notification import dismiss_all_notifications
from docubrain.db.notification import get_notifications
from docubrain.db.notification import update_notification_last_shown
...
from docubrain.server.settings.models import (
    DEFAULT_FILE_TOKEN_COUNT_THRESHOLD_K_NO_VECTOR_DB,
)
from docubrain.server.settings.models import DEFAULT_FILE_TOKEN_COUNT_THRESHOLD_K_VECTOR_DB
from docubrain.server.settings.models import Notification
from docubrain.server.settings.models import Settings
from docubrain.server.settings.models import UserSettings
...
def fetch_settings(
    ...
    apply_fn = fetch_versioned_implementation_with_fallback(
        "docubrain.server.settings.api",
        "apply_license_status_to_settings",
        apply_license_status_to_settings,
    )
    general_settings = apply_fn(general_settings)

    # Check if DocuBrain Craft is enabled for this user (used for server-side redirects)
    docubrain_craft_enabled_for_user = is_docubrain_craft_enabled(user) if user else False

    return UserSettings(
        **general_settings.model_dump(),
        notifications=settings_notifications,
        needs_reindexing=needs_reindexing,
        docubrain_craft_enabled=docubrain_craft_enabled_for_user,
        vector_db_enabled=not DISABLE_VECTOR_DB,
        hooks_enabled=not MULTI_TENANT,
        version=docubrain_version,
