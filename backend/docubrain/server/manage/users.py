...
from docubrain.auth.anonymous_user import fetch_anonymous_user_info
from docubrain.auth.email_utils import send_user_email_invite
from docubrain.auth.invited_users import get_invited_users
from docubrain.auth.invited_users import remove_user_from_invited_users
from docubrain.auth.invited_users import write_invited_users
...
from docubrain.auth.users import anonymous_user_enabled
from docubrain.auth.users import current_curator_or_admin_user
from docubrain.auth.users import enforce_seat_limit
from docubrain.auth.users import optional_user
from docubrain.configs.app_configs import AUTH_BACKEND
from docubrain.configs.app_configs import AUTH_TYPE
from docubrain.configs.app_configs import AuthBackend
from docubrain.configs.app_configs import DEV_MODE
from docubrain.configs.app_configs import EMAIL_CONFIGURED
from docubrain.configs.app_configs import ENABLE_EMAIL_INVITES
from docubrain.configs.app_configs import NUM_FREE_TRIAL_USER_INVITES
from docubrain.configs.app_configs import REDIS_AUTH_KEY_PREFIX
from docubrain.configs.app_configs import SESSION_EXPIRE_TIME_SECONDS
from docubrain.configs.app_configs import USER_AUTH_SECRET
from docubrain.configs.app_configs import VALID_EMAIL_DOMAINS
from docubrain.configs.constants import FASTAPI_USERS_AUTH_COOKIE_NAME
from docubrain.configs.constants import PUBLIC_API_TAGS
from docubrain.db.api_key import is_api_key_email_address
from docubrain.db.auth import get_live_users_count
from docubrain.db.engine.sql_engine import get_session
from docubrain.db.enums import AccountType
from docubrain.db.enums import Permission
from docubrain.db.enums import UserFileStatus
from docubrain.db.models import User
from docubrain.db.models import UserFile
from docubrain.db.user_preferences import activate_user
from docubrain.db.user_preferences import deactivate_user
from docubrain.db.user_preferences import get_all_user_assistant_specific_configs
from docubrain.db.user_preferences import get_latest_access_token_for_user
from docubrain.db.user_preferences import get_memories_for_user
from docubrain.db.user_preferences import update_assistant_preferences
from docubrain.db.user_preferences import update_user_assistant_visibility
from docubrain.db.user_preferences import update_user_auto_scroll
from docubrain.db.user_preferences import update_user_chat_background
from docubrain.db.user_preferences import update_user_default_app_mode
from docubrain.db.user_preferences import update_user_default_model
from docubrain.db.user_preferences import update_user_personalization
from docubrain.db.user_preferences import update_user_pinned_assistants
from docubrain.db.user_preferences import update_user_role
from docubrain.db.user_preferences import update_user_shortcut_enabled
from docubrain.db.user_preferences import update_user_temperature_override_enabled
from docubrain.db.user_preferences import update_user_theme_preference
from docubrain.db.users import batch_get_user_groups
from docubrain.db.users import delete_user_from_db
from docubrain.db.users import get_all_accepted_users
from docubrain.db.users import get_all_users
from docubrain.db.users import get_page_of_filtered_users
from docubrain.db.users import get_total_filtered_users_count
from docubrain.db.users import get_user_by_email
from docubrain.db.users import get_user_counts_by_role_and_status
from docubrain.db.users import validate_user_role_update
from docubrain.key_value_store.factory import get_kv_store
from docubrain.redis.redis_pool import get_raw_redis_client
from docubrain.server.documents.models import PaginatedReturn
from docubrain.server.features.projects.models import UserFileSnapshot
from docubrain.server.manage.models import AllUsersResponse
from docubrain.server.manage.models import AutoScrollRequest
from docubrain.server.manage.models import BulkInviteResponse
from docubrain.server.manage.models import ChatBackgroundRequest
from docubrain.server.manage.models import DefaultAppModeRequest
from docubrain.server.manage.models import EmailInviteStatus
from docubrain.server.manage.models import MemoryItem
from docubrain.server.manage.models import PersonalizationUpdateRequest
from docubrain.server.manage.models import TenantInfo
from docubrain.server.manage.models import TenantSnapshot
from docubrain.server.manage.models import ThemePreferenceRequest
from docubrain.server.manage.models import UserByEmail
from docubrain.server.manage.models import UserInfo
from docubrain.server.manage.models import UserPreferences
from docubrain.server.manage.models import UserRoleResponse
from docubrain.server.manage.models import UserRoleUpdateRequest
from docubrain.server.manage.models import UserSpecificAssistantPreference
from docubrain.server.manage.models import UserSpecificAssistantPreferences
from docubrain.server.models import FullUserSnapshot
from docubrain.server.models import InvitedUserSnapshot
from docubrain.server.models import MinimalUserSnapshot
from docubrain.server.models import UserGroupInfo
from docubrain.server.usage_limits import is_tenant_on_trial_fn
...
async def test_upsert_user(
    ...
    user = await fetch_ee_implementation_or_noop(
        "docubrain.server.saml", "upsert_saml_user", None
    )(email=request.email)
...
def list_accepted_users(
    ...
    try:
        from docubrain.db.models import ScimUserMapping
...
def bulk_invite_users(
    ...
    if MULTI_TENANT:
        try:
            fetch_ee_implementation_or_noop(
                "docubrain.server.tenants.provisioning", "add_users_to_tenant", None
    ...
    if MULTI_TENANT and not DEV_MODE:
        ...
        try:
            ...
            fetch_ee_implementation_or_noop(
                "docubrain.server.tenants.billing", "register_tenant_users", None
            ...
            fetch_ee_implementation_or_noop(
                "docubrain.server.tenants.user_mapping", "remove_users_from_tenant", None
            )(new_invited_emails, tenant_id)
            raise e
...
def deactivate_user_api(
    ...
    if not MULTI_TENANT:
        fetch_ee_implementation_or_noop(
            "docubrain.db.license", "invalidate_license_cache", None
        )()
...
def verify_user_logged_in(
    ...
    team_name = fetch_ee_implementation_or_noop(
        "docubrain.server.tenants.user_mapping", "get_tenant_id_for_email", None
    )(user.email)
    ...
    if MULTI_TENANT:
        if team_name != get_current_tenant_id():
            user_count = fetch_ee_implementation_or_noop(
                "docubrain.server.tenants.user_mapping", "get_tenant_count", None
            )(team_name)
        ...
        tenant_invitation = fetch_ee_implementation_or_noop(
            "docubrain.server.tenants.user_mapping", "get_tenant_invitation", None
        )(user.email)

    super_users_list = cast(
        list[str],
        fetch_versioned_implementation_with_fallback(
            "docubrain.configs.app_configs",
            "SUPER_USERS",
            [],
        ),
    )
