...
from docubrain.auth.users import current_chat_accessible_user
from docubrain.auth.users import current_curator_or_admin_user
from docubrain.background.celery.tasks.pruning.tasks import (
    try_creating_prune_generator_task,
)
from docubrain.background.celery.versioned_apps.client import app as client_app
from docubrain.configs.app_configs import EMAIL_CONFIGURED
from docubrain.configs.app_configs import ENABLED_CONNECTOR_TYPES
from docubrain.configs.app_configs import MOCK_CONNECTOR_FILE_PATH
from docubrain.configs.constants import DocumentSource
from docubrain.configs.constants import FileOrigin
from docubrain.configs.constants import MilestoneRecordType
...
from docubrain.configs.constants import PUBLIC_API_TAGS
from docubrain.connectors.exceptions import ConnectorValidationError
from docubrain.connectors.factory import validate_ccpair_for_user
from docubrain.connectors.google_utils.google_auth import (
    get_google_oauth_creds,
)
from docubrain.connectors.google_utils.google_kv import (
...
from docubrain.connectors.google_utils.google_kv import get_auth_url
from docubrain.connectors.google_utils.google_kv import (
    get_google_app_cred,
)
from docubrain.connectors.google_utils.google_kv import (
...
from docubrain.connectors.google_utils.google_kv import verify_csrf
from docubrain.connectors.google_utils.shared_constants import DB_CREDENTIALS_DICT_TOKEN_KEY
from docubrain.connectors.google_utils.shared_constants import (
    GoogleOAuthAuthenticationMethod,
)
from docubrain.db.connector import create_connector
from docubrain.db.connector import delete_connector
from docubrain.db.connector import fetch_connector_by_id
from docubrain.db.connector import fetch_connectors
from docubrain.db.connector import fetch_unique_document_sources
from docubrain.db.connector import get_connector_credential_ids
from docubrain.db.connector import mark_ccpair_with_indexing_trigger
from docubrain.db.connector import update_connector
from docubrain.db.connector_credential_pair import add_credential_to_connector
from docubrain.db.connector_credential_pair import (
    fetch_connector_credential_pair_for_connector,
)
from docubrain.db.connector_credential_pair import get_cc_pair_groups_for_ids
from docubrain.db.connector_credential_pair import get_connector_credential_pair
from docubrain.db.connector_credential_pair import get_connector_credential_pairs_for_user
from docubrain.db.connector_credential_pair import (
    get_connector_credential_pairs_for_user_parallel,
)
from docubrain.db.connector_credential_pair import verify_user_has_access_to_cc_pair
from docubrain.db.credentials import cleanup_gmail_credentials
from docubrain.db.credentials import cleanup_google_drive_credentials
from docubrain.db.credentials import create_credential
from docubrain.db.credentials import delete_service_account_credentials
from docubrain.db.credentials import fetch_credential_by_id_for_user
from docubrain.db.deletion_attempt import check_deletion_attempt_is_allowed
from docubrain.db.document import get_document_counts_for_all_cc_pairs
from docubrain.db.engine.sql_engine import get_session
from docubrain.db.enums import AccessType
from docubrain.db.enums import ConnectorCredentialPairStatus
from docubrain.db.enums import IndexingMode
from docubrain.db.enums import Permission
from docubrain.db.enums import ProcessingMode
from docubrain.db.federated import fetch_all_federated_connectors_parallel
from docubrain.db.index_attempt import get_index_attempts_for_cc_pair
from docubrain.db.index_attempt import get_latest_index_attempts_by_status
from docubrain.db.index_attempt import get_latest_index_attempts_parallel
from docubrain.db.index_attempt import (
    get_latest_successful_index_attempts_parallel,
)
from docubrain.db.models import ConnectorCredentialPair
from docubrain.db.models import FederatedConnector
from docubrain.db.models import IndexAttempt
from docubrain.db.models import IndexingStatus
from docubrain.db.models import User
from docubrain.db.models import UserRole
from docubrain.file_processing.file_types import PLAIN_TEXT_MIME_TYPE
from docubrain.file_processing.file_types import WORD_PROCESSING_MIME_TYPE
from docubrain.file_store.file_store import FileStore
from docubrain.file_store.file_store import get_default_file_store
from docubrain.key_value_store.interface import KvKeyNotFoundError
from docubrain.redis.redis_pool import get_redis_client
from docubrain.server.documents.models import AuthStatus
from docubrain.server.documents.models import AuthUrl
from docubrain.server.documents.models import ConnectorBase
from docubrain.server.documents.models import ConnectorCredentialPairIdentifier
from docubrain.server.documents.models import ConnectorFileInfo
from docubrain.server.documents.models import ConnectorFilesResponse
from docubrain.server.documents.models import ConnectorIndexingStatusLite
from docubrain.server.documents.models import ConnectorIndexingStatusLiteResponse
from docubrain.server.documents.models import ConnectorRequestSubmission
from docubrain.server.documents.models import ConnectorSnapshot
from docubrain.server.documents.models import ConnectorStatus
from docubrain.server.documents.models import ConnectorUpdateRequest
from docubrain.server.documents.models import CredentialBase
from docubrain.server.documents.models import CredentialSnapshot
from docubrain.server.documents.models import DocsCountOperator
from docubrain.server.documents.models import FailedConnectorIndexingStatus
from docubrain.server.documents.models import FileUploadResponse
from docubrain.server.documents.models import GDriveCallback
from docubrain.server.documents.models import GmailCallback
from docubrain.server.documents.models import GoogleAppCredentials
from docubrain.server.documents.models import GoogleServiceAccountCredentialRequest
from docubrain.server.documents.models import GoogleServiceAccountKey
from docubrain.server.documents.models import IndexedSourcesResponse
from docubrain.server.documents.models import IndexingStatusRequest
from docubrain.server.documents.models import ObjectCreationIdResponse
from docubrain.server.documents.models import RunConnectorRequest
from docubrain.server.documents.models import SourceSummary
from docubrain.server.federated.models import FederatedConnectorStatus
from docubrain.server.models import StatusResponse
from docubrain.server.utils_vector_db import require_vector_db
from docubrain.utils.logger import setup_logger
from docubrain.utils.telemetry import mt_cloud_telemetry
from docubrain.utils.threadpool_concurrency import CallableProtocol
from docubrain.utils.threadpool_concurrency import run_functions_tuples_in_parallel
from docubrain.utils.variable_functionality import fetch_ee_implementation_or_noop
...
def save_zip_metadata_to_file_store(
    ...
    """
    Extract .docubrain_metadata.json from zip and save to file store.
    Returns the file_id or None if no metadata file exists.
    """
    try:
        metadata_file_info = zf.getinfo(DOCUBRAIN_METADATA_FILENAME)
        with zf.open(metadata_file_info, "r") as metadata_file:
            ...
            try:
                ...
                logger.warning(f"Unable to load {DOCUBRAIN_METADATA_FILENAME}: {e}")
                raise HTTPException(
                    status_code=400,
                    detail=f"Unable to load {DOCUBRAIN_METADATA_FILENAME}: {e}",
                )

            # Save to file store
            file_id = file_store.save_file(
                content=BytesIO(metadata_bytes),
                display_name=DOCUBRAIN_METADATA_FILENAME,
                file_origin=FileOrigin.CONNECTOR_METADATA,
                file_type="application/json",
            )
            return file_id
    except KeyError:
        logger.info(f"No {DOCUBRAIN_METADATA_FILENAME} file")
        return None
    ...
    try:
        ...
        if new_file_paths:
            ...
            client_app.send_task(
                DocubrainCeleryTask.CHECK_FOR_INDEXING,
...
def create_connector_from_model(
    ...
    try:
        ...
        fetch_ee_implementation_or_noop(
            "docubrain.db.user_group", "validate_object_creation_for_user", None
        )(
            db_session=db_session,
            user=user,
...
def submit_connector_request(
    ...
    Tracks via PostHog telemetry and sends email to hello@docubrain.app.
    """
    tenant_id = get_current_tenant_id()
    ...
    if EMAIL_CONFIGURED:
        try:
            subject = "DocuBrain Craft Connector Request"
...
<li><strong>Tenant ID:</strong> {tenant_id}</li>
            ...
            send_email(
                user_email="hello@docubrain.app",
                subject=subject,
                html_body=email_body_html,
                text_body=email_body_text,
            )
            logger.info(
                f"Connector request email sent to hello@docubrain.app for connector: {connector_name}"
            )
        except Exception as e:
