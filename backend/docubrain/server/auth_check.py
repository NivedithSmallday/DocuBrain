...
from docubrain.auth.users import current_user_with_expired_token
...
PUBLIC_ENDPOINT_SPECS = [
    ...
    ("/auth/type", {"GET"}),
    # just gets the version of DocuBrain (e.g. 0.3.11)
    ("/version", {"GET"}),
    # Gets stable and beta versions for DocuBrain docker images
    ("/versions", {"GET"}),
...
def check_router_auth(
    ...
    control_plane_dep = fetch_ee_implementation_or_noop(
        "docubrain.server.tenants.access", "control_plane_dep"
    )
    current_cloud_superuser = fetch_ee_implementation_or_noop(
        "docubrain.auth.users", "current_cloud_superuser"
    )
    verify_scim_token = fetch_ee_implementation_or_noop(
        "docubrain.server.scim.auth", "verify_scim_token"
    )
