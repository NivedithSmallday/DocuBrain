...
from docubrain.background.celery.tasks.beat_schedule import (
    CLOUD_DOC_PERMISSION_SYNC_MULTIPLIER_DEFAULT,
)
from docubrain.configs.constants import CLOUD_BUILD_FENCE_LOOKUP_TABLE_INTERVAL_DEFAULT
from docubrain.configs.constants import DOCUBRAIN_CLOUD_REDIS_RUNTIME
from docubrain.configs.constants import DOCUBRAIN_CLOUD_TENANT_ID
from docubrain.configs.constants import DOCUBRAIN_EMAILABLE_LOGO_MAX_DIM
...
from docubrain.utils.file import FileWithMimeType
from docubrain.utils.file import DocubrainStaticFileManager
from docubrain.utils.variable_functionality import (
    fetch_ee_implementation_or_noop,
)


class DocubrainRuntime:
    ...
    def _get_with_static_fallback(
        ...
        docubrain_file: FileWithMimeType | None = None

        if db_filename:
            file_store = get_default_file_store()
            docubrain_file = file_store.get_file_with_mime_type(db_filename)

        if not docubrain_file:
            docubrain_file = DocubrainStaticFileManager.get_static(static_filename)
    ...
    def get_logo() -> FileWithMimeType:
        ...
        db_filename: str | None = fetch_ee_implementation_or_noop(
            "docubrain.server.enterprise_settings.store", "get_logo_filename", None
        )

        return DocubrainRuntime._get_with_static_fallback(db_filename, STATIC_FILENAME)

    @staticmethod
    def get_emailable_logo() -> FileWithMimeType:
        docubrain_file = DocubrainRuntime.get_logo()

        # check dimensions and resize downwards if necessary or if not PNG
        image = Image.open(io.BytesIO(docubrain_file.data))
        if (
            image.size[0] > DOCUBRAIN_EMAILABLE_LOGO_MAX_DIM
            or image.size[1] > DOCUBRAIN_EMAILABLE_LOGO_MAX_DIM
            or image.format != "PNG"
        ):
            image.thumbnail(
                (DOCUBRAIN_EMAILABLE_LOGO_MAX_DIM, DOCUBRAIN_EMAILABLE_LOGO_MAX_DIM),
                Image.LANCZOS,
            )  # maintains aspect ratio
            output_buffer = io.BytesIO()
            image.save(output_buffer, format="PNG")
            docubrain_file = FileWithMimeType(
    ...
    def get_logotype() -> FileWithMimeType:
        ...
        db_filename: str | None = fetch_ee_implementation_or_noop(
            "docubrain.server.enterprise_settings.store", "get_logotype_filename", None
        )
    ...
    def get_beat_multiplier() -> float:
        ...
        r = get_redis_replica_client(tenant_id=DOCUBRAIN_CLOUD_TENANT_ID)

        beat_multiplier_raw = r.get(f"{DOCUBRAIN_CLOUD_REDIS_RUNTIME}:beat_multiplier")
    ...
    def get_doc_permission_sync_multiplier() -> float:
        ...
        value_raw = r.get(f"{DOCUBRAIN_CLOUD_REDIS_RUNTIME}:doc_permission_sync_multiplier")
    ...
    def get_build_fence_lookup_table_interval() -> int:
        ...
        interval_raw = r.get(
            f"{DOCUBRAIN_CLOUD_REDIS_RUNTIME}:build_fence_lookup_table_interval"
        )
