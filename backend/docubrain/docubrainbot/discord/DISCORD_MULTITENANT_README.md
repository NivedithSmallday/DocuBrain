...
The Discord bot uses a **single-client, multi-tenant** architecture where one `DocubrainDiscordClient` instance serves multiple tenants (organizations) simultaneously. Tenant isolation is achieved through:
...
│                                                                     │
│  ┌─────────────────────────┐    ┌─────────────────────────────┐    │
│  │   DiscordCacheManager   │    │      DocubrainAPIClient          │    │
│  │                         │    │                             │    │
...
### 1. Cache Manager (`backend/docubrain/docubrainbot/discord/cache.py`)
...
### 2. API Client (`backend/docubrain/docubrainbot/discord/api_client.py`)

The `DocubrainAPIClient` is a **stateless async HTTP client** that communicates with DocuBrain API pods.

#### Key Design: Per-Request API Key Injection

```python
class DocubrainAPIClient:
...
```python
# backend/docubrain/db/discord_bot.py
...
1. **Single Client, Multiple Tenants**: One `DocubrainAPIClient` and one `DiscordCacheManager` instance serves all tenants via dynamic API key injection.
...
| Component | Path |
|-----------|------|
| Cache Manager | `backend/docubrain/docubrainbot/discord/cache.py` |
| API Client | `backend/docubrain/docubrainbot/discord/api_client.py` |
| Discord Client | `backend/docubrain/docubrainbot/discord/client.py` |
| API Key DB Operations | `backend/docubrain/db/discord_bot.py` |
| Cache Manager Tests | `backend/tests/unit/docubrain/docubrainbot/discord/test_cache_manager.py` |
| API Client Tests | `backend/tests/unit/docubrain/docubrainbot/discord/test_api_client.py` |