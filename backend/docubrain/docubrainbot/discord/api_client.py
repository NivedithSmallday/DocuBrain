"""Async HTTP client for communicating with DocuBrain API pods."""

import aiohttp

from docubrain.chat.models import ChatFullResponse
from docubrain.docubrainbot.discord.constants import API_REQUEST_TIMEOUT
from docubrain.docubrainbot.discord.exceptions import APIConnectionError
from docubrain.docubrainbot.discord.exceptions import APIResponseError
from docubrain.docubrainbot.discord.exceptions import APITimeoutError
...
class DocubrainAPIClient:
    """Async HTTP client for sending chat requests to DocuBrain API pods.

    This client manages an aiohttp session for making non-blocking HTTP
    requests to the DocuBrain API server. It handles authentication with per-tenant
    API keys and multi-tenant routing.

    Usage:
        client = DocubrainAPIClient()
    ...
    def __init__(
        ...
        # TODO: Ideally, this override is only used when someone is launching a DocuBrain service independently
    ...
    async def send_chat_message(
        ...
        """Send a chat message to the DocuBrain API server and get a response.
