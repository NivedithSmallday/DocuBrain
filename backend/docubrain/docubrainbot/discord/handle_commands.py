...
from docubrain.configs.app_configs import DISCORD_BOT_INVOKE_CHAR
from docubrain.configs.constants import DOCUBRAIN_DISCORD_URL
from docubrain.db.discord_bot import bulk_create_channel_configs
from docubrain.db.discord_bot import get_guild_config_by_discord_id
from docubrain.db.discord_bot import get_guild_config_by_internal_id
from docubrain.db.discord_bot import get_guild_config_by_registration_key
from docubrain.db.discord_bot import sync_channel_configs
from docubrain.db.engine.sql_engine import get_session_with_tenant
from docubrain.db.utils import DiscordChannelView
from docubrain.docubrainbot.discord.cache import DiscordCacheManager
from docubrain.docubrainbot.discord.constants import REGISTER_COMMAND
from docubrain.docubrainbot.discord.constants import SYNC_CHANNELS_COMMAND
from docubrain.docubrainbot.discord.exceptions import RegistrationError
from docubrain.docubrainbot.discord.exceptions import SyncChannelsError
from docubrain.server.manage.discord_bot.utils import parse_discord_registration_key
...
async def handle_dm(message: discord.Message) -> None:
    ...
    dm_response = (
        ...
        f"[DocuBrain Discord]({DOCUBRAIN_DISCORD_URL}) for help!"
    )
...
async def handle_registration_command(
    ...
    try:
        ...
        await message.reply(
            ...
            "This server is now connected to DocuBrain. "
            "I'll respond to messages based on your server and channel settings set in DocuBrain."
        )
...
async def _register_guild(
    ...
    if existing_tenant is not None:
        ...
        raise RegistrationError(
            "This server is already registered.\n\nDocubrainBot can only connect one Discord server to one DocuBrain workspace."
        )
            ...
            with get_session_with_tenant(tenant_id=tenant_id) as db:
                ...
                if not config:
                    raise RegistrationError(
                        ...
                        "Please generate a new one from the DocuBrain admin panel."
                    )
                ...
                if config.guild_id is not None:
                    raise RegistrationError(
                        ...
                        "Please generate a new key from the DocuBrain admin panel."
                    )
    ...
    try:
        ...
        def _get_guild_config_id() -> int | None:
            with get_session_with_tenant(tenant_id=tenant_id) as db:
                if not message.guild:
                    raise SyncChannelsError(
                        "Server not found. This shouldn't happen. Please contact DocuBrain support."
                    )
        ...
        if not guild_config_id:
            raise SyncChannelsError(
                "Server config not found. This shouldn't happen. Please contact DocuBrain support."
            )
        ...
        await message.reply(
            ...
            "New channels are disabled by default. Enable them in the DocuBrain admin panel."
        )
