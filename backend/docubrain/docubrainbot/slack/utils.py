...
from docubrain.configs.app_configs import DISABLE_TELEMETRY
from docubrain.configs.constants import ID_SEPARATOR
from docubrain.configs.constants import MessageType
from docubrain.configs.docubrainbot_configs import DOCUBRAIN_BOT_FEEDBACK_VISIBILITY
from docubrain.configs.docubrainbot_configs import DOCUBRAIN_BOT_MAX_QPM
from docubrain.configs.docubrainbot_configs import DOCUBRAIN_BOT_MAX_WAIT_TIME
from docubrain.configs.docubrainbot_configs import DOCUBRAIN_BOT_NUM_RETRIES
from docubrain.configs.docubrainbot_configs import (
    DOCUBRAIN_BOT_RESPONSE_LIMIT_PER_TIME_PERIOD,
)
from docubrain.configs.docubrainbot_configs import (
    DOCUBRAIN_BOT_RESPONSE_LIMIT_TIME_PERIOD_SECONDS,
)
from docubrain.connectors.slack.utils import SlackTextCleaner
...
from docubrain.docubrainbot.slack.constants import FeedbackVisibility
from docubrain.docubrainbot.slack.models import ChannelType
...
from docubrain.utils.text_processing import replace_whitespaces_w_space
...
_DOCUBRAIN_BOT_MESSAGE_COUNT: int = 0
_DOCUBRAIN_BOT_COUNT_START_TIME: float = time.time()


def get_docubrain_bot_auth_ids(
...
def check_message_limit() -> bool:
    ...
    """
    if DOCUBRAIN_BOT_RESPONSE_LIMIT_PER_TIME_PERIOD <= 0:
        return True
    global _DOCUBRAIN_BOT_MESSAGE_COUNT
    global _DOCUBRAIN_BOT_COUNT_START_TIME
    time_since_start = time.time() - _DOCUBRAIN_BOT_COUNT_START_TIME
    if time_since_start > DOCUBRAIN_BOT_RESPONSE_LIMIT_TIME_PERIOD_SECONDS:
        _DOCUBRAIN_BOT_MESSAGE_COUNT = 0
        _DOCUBRAIN_BOT_COUNT_START_TIME = time.time()
    if (_DOCUBRAIN_BOT_MESSAGE_COUNT + 1) > DOCUBRAIN_BOT_RESPONSE_LIMIT_PER_TIME_PERIOD:
        logger.error(
            f"DocubrainBot has reached the message limit {DOCUBRAIN_BOT_RESPONSE_LIMIT_PER_TIME_PERIOD}"
            f" for the time period {DOCUBRAIN_BOT_RESPONSE_LIMIT_TIME_PERIOD_SECONDS} seconds."
            " These limits are configurable in backend/docubrain/configs/docubrainbot_configs.py"
        )
        return False
    _DOCUBRAIN_BOT_MESSAGE_COUNT += 1
    return True
...
def remove_docubrain_bot_tag(tenant_id: str, message_str: str, client: WebClient) -> str:
    bot_token_user_id, _ = get_docubrain_bot_auth_ids(tenant_id, web_client=client)
...
def _build_error_block(error_message: str) -> Block:
    ...
    """
    display_text = (
        "There was an error displaying all of the DocuBrain answers."
        f" Please let an admin or an docubrain developer know. Error: {error_message}"
    )
    return SectionBlock(text=display_text)


@retry(
    tries=DOCUBRAIN_BOT_NUM_RETRIES,
    delay=0.25,
    backoff=2,
...
def read_slack_thread(
    ...
    for reply in replies:
        if "user" in reply and "bot_id" not in reply:
            ...
            blocks: Any
            is_docubrain_bot_response = False

            reply_user = reply.get("user")
            reply_bot_id = reply.get("bot_id")

            self_slack_bot_user_id, self_slack_bot_bot_id = get_docubrain_bot_auth_ids(
                tenant_id, client
            )
            if reply_user is not None and reply_user == self_slack_bot_user_id:
                is_docubrain_bot_response = True

            if reply_bot_id is not None and reply_bot_id == self_slack_bot_bot_id:
                is_docubrain_bot_response = True

            if is_docubrain_bot_response:
                # DocubrainBot response
                message_type = MessageType.ASSISTANT
                user_sem_id = "Assistant"

                # DocubrainBot responses have both text and blocks
                ...
                if not blocks:
                    logger.warning(f"DocubrainBot response has no blocks: {reply}")
                    continue
                ...
                if message is not None and message.startswith("_Filters"):
                    if len(blocks) < 2:
                        ...
                        continue
                    # This is the DocubrainBot answer format, if there is a change to how we respond,
                    # this will need to be updated to get the correct "answer" portion
                    message = reply["blocks"][1].get("text", {}).get("text")
            else:
                # Other bots are not counted as the LLM response which only comes from DocuBrain
                message_type = MessageType.USER
        ...
        message = remove_docubrain_bot_tag(tenant_id, message, client=client)
...
def slack_usage_report(action: str, sender_id: str | None, client: WebClient) -> None:
    ...
    if sender_email is not None:
        with get_session_with_current_tenant() as db_session:
            docubrain_user = get_user_by_email(email=sender_email, db_session=db_session)

    optional_telemetry(
        record_type=RecordType.USAGE,
        data={"action": action},
        user_id=str(docubrain_user.id) if docubrain_user else "Non-DocuBrain-Or-No-Auth-User",
    )


class SlackRateLimiter:
    def __init__(self) -> None:
        self.max_qpm: int | None = DOCUBRAIN_BOT_MAX_QPM
        self.max_wait_time = DOCUBRAIN_BOT_MAX_WAIT_TIME
...
def get_feedback_visibility() -> FeedbackVisibility:
    try:
        return FeedbackVisibility(DOCUBRAIN_BOT_FEEDBACK_VISIBILITY.lower())
    except ValueError:
