...
from docubrain.chat.process_message import remove_answer_citations
from docubrain.configs.constants import MessageType
from docubrain.configs.constants import SearchFeedbackType
from docubrain.configs.docubrainbot_configs import DOCUBRAIN_BOT_FOLLOWUP_EMOJI
...
from docubrain.db.feedback import create_doc_retrieval_feedback
from docubrain.db.users import get_user_by_email
from docubrain.docubrainbot.slack.blocks import build_follow_up_resolved_blocks
from docubrain.docubrainbot.slack.blocks import build_slack_response_blocks
from docubrain.docubrainbot.slack.blocks import get_document_feedback_blocks
...
from docubrain.docubrainbot.slack.handlers.handle_regular_answer import (
    handle_regular_answer,
)
from docubrain.docubrainbot.slack.models import SlackMessageInfo
from docubrain.docubrainbot.slack.utils import build_feedback_id
from docubrain.docubrainbot.slack.utils import decompose_action_id
from docubrain.docubrainbot.slack.utils import fetch_group_ids_from_names
from docubrain.docubrainbot.slack.utils import fetch_slack_user_ids_from_emails
from docubrain.docubrainbot.slack.utils import get_channel_name_from_id
from docubrain.docubrainbot.slack.utils import get_feedback_visibility
...
from docubrain.server.query_and_chat.models import ChatMessageDetail
...
def handle_publish_ephemeral_message_button(
    ...
    # Obtain docubrain_user and chat_message information
    if not chat_message_id:
        raise ValueError("Missing chat_message_id in the payload")

    with get_session_with_current_tenant() as db_session:
        docubrain_user = get_user_by_email(user_email, db_session)
        if not docubrain_user:
            raise ValueError("Cannot determine docubrain_user_id from email in payload")
        try:
            chat_message = get_chat_message(chat_message_id, docubrain_user.id, db_session)
        except ValueError:
        ...
        docubrain_bot_answer = ChatBasicResponse(
...
def handle_slack_feedback(
    ...
    # Get DocuBrain user from Slack ID
    ...
    with get_session_with_current_tenant() as db_session:
        docubrain_user = get_user_by_email(email, db_session) if email else None
        if feedback_type in [LIKE_BLOCK_ACTION_ID, DISLIKE_BLOCK_ACTION_ID]:
            create_chat_message_feedback(
                is_positive=feedback_type == LIKE_BLOCK_ACTION_ID,
                feedback_text="",
                chat_message_id=message_id,
                user_id=docubrain_user.id if docubrain_user else None,
                db_session=db_session,
            )
            remove_scheduled_feedback_reminder(
                client=client,
...
def handle_followup_button(
    ...
    update_emote_react(
        emoji=DOCUBRAIN_BOT_FOLLOWUP_EMOJI,
        channel=channel_id,
