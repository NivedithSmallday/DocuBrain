...
from docubrain.configs.docubrainbot_configs import DOCUBRAIN_BOT_FEEDBACK_REMINDER
...
from docubrain.db.users import add_slack_user_if_not_exists
from docubrain.db.users import get_user_by_email
from docubrain.docubrainbot.slack.blocks import get_feedback_reminder_blocks
from docubrain.docubrainbot.slack.handlers.handle_regular_answer import (
    handle_regular_answer,
)
from docubrain.docubrainbot.slack.handlers.handle_standard_answers import (
...
from docubrain.docubrainbot.slack.utils import fetch_user_ids_from_groups
from docubrain.docubrainbot.slack.utils import respond_in_thread_or_channel
from docubrain.docubrainbot.slack.utils import slack_usage_report
...
def schedule_feedback_reminder(
    ...
    if not DOCUBRAIN_BOT_FEEDBACK_REMINDER:
    ...
    future = now + datetime.timedelta(minutes=DOCUBRAIN_BOT_FEEDBACK_REMINDER)
...
def handle_message(
    ...
    DocuBrain failing to answer/retrieve docs does count and should be notified
    """
    ...
    if slack_channel_config.channel_config.get("disabled"):
        logger.info("Skipping message: DocubrainBot is disabled for this channel")
        return False
    ...
    if respond_tag_only and not bypass_filters and not is_bot_dm:
        logger.info("Skipping message: DocubrainBot only responds to tags in this channel")
        return False
    ...
    # If configured to respond to team members only, then cannot be used with a /DocubrainBot command
    ...
    if send_to and is_slash_command:
        if sender_id:
            respond_in_thread_or_channel(
                ...
                text="The DocubrainBot slash command is not enabled for this channel",
                thread_ts=None,
            )
                ...
                if seat_result is not None and not seat_result.available:
                    ...
                    respond_in_thread_or_channel(
                        ...
                        text=(
                            ...
                            "DocuBrain administrator to add more seats."
                        ),
                    )
                    return False
