...
from docubrain.configs.app_configs import POD_NAME
from docubrain.configs.app_configs import POD_NAMESPACE
from docubrain.configs.constants import MessageType
from docubrain.configs.constants import DocubrainRedisLocks
from docubrain.configs.docubrainbot_configs import NOTIFY_SLACKBOT_NO_ANSWER
from docubrain.connectors.slack.utils import expert_info_from_slack_id
...
from docubrain.db.models import SlackBot
from docubrain.db.search_settings import get_current_search_settings
from docubrain.db.slack_bot import fetch_slack_bot
from docubrain.db.slack_bot import fetch_slack_bots
...
from docubrain.docubrainbot.slack.config import get_slack_channel_config_for_bot_and_channel
from docubrain.docubrainbot.slack.config import MAX_TENANTS_PER_POD
from docubrain.docubrainbot.slack.config import TENANT_ACQUISITION_INTERVAL
from docubrain.docubrainbot.slack.config import TENANT_HEARTBEAT_EXPIRATION
from docubrain.docubrainbot.slack.config import TENANT_HEARTBEAT_INTERVAL
from docubrain.docubrainbot.slack.config import TENANT_LOCK_EXPIRATION
from docubrain.docubrainbot.slack.constants import DISLIKE_BLOCK_ACTION_ID
from docubrain.docubrainbot.slack.constants import FEEDBACK_DOC_BUTTON_BLOCK_ACTION_ID
from docubrain.docubrainbot.slack.constants import FOLLOWUP_BUTTON_ACTION_ID
from docubrain.docubrainbot.slack.constants import FOLLOWUP_BUTTON_RESOLVED_ACTION_ID
from docubrain.docubrainbot.slack.constants import GENERATE_ANSWER_BUTTON_ACTION_ID
from docubrain.docubrainbot.slack.constants import IMMEDIATE_RESOLVED_BUTTON_ACTION_ID
from docubrain.docubrainbot.slack.constants import KEEP_TO_YOURSELF_ACTION_ID
from docubrain.docubrainbot.slack.constants import LIKE_BLOCK_ACTION_ID
from docubrain.docubrainbot.slack.constants import SHOW_EVERYONE_ACTION_ID
from docubrain.docubrainbot.slack.constants import VIEW_DOC_FEEDBACK_ID
from docubrain.docubrainbot.slack.handlers.handle_buttons import handle_doc_feedback_button
from docubrain.docubrainbot.slack.handlers.handle_buttons import handle_followup_button
from docubrain.docubrainbot.slack.handlers.handle_buttons import (
...
from docubrain.docubrainbot.slack.handlers.handle_buttons import handle_slack_feedback
from docubrain.docubrainbot.slack.handlers.handle_message import handle_message
from docubrain.docubrainbot.slack.handlers.handle_message import (
    remove_scheduled_feedback_reminder,
)
from docubrain.docubrainbot.slack.handlers.handle_message import schedule_feedback_reminder
from docubrain.docubrainbot.slack.models import SlackContext
...
from docubrain.docubrainbot.slack.utils import check_message_limit
from docubrain.docubrainbot.slack.utils import decompose_action_id
from docubrain.docubrainbot.slack.utils import get_channel_name_from_id
from docubrain.docubrainbot.slack.utils import get_channel_type_from_id
from docubrain.docubrainbot.slack.utils import get_docubrain_bot_auth_ids
from docubrain.docubrainbot.slack.utils import read_slack_thread
from docubrain.docubrainbot.slack.utils import remove_docubrain_bot_tag
from docubrain.docubrainbot.slack.utils import respond_in_thread_or_channel
from docubrain.docubrainbot.slack.utils import TenantSocketModeClient
from docubrain.redis.redis_pool import get_redis_client
from docubrain.server.manage.models import SlackBotTokens
...
class SlackbotHandler:
    ...
    def acquire_tenants(self) -> None:
        ...
        gated_tenants = fetch_ee_implementation_or_noop(
            "docubrain.server.tenants.product_gating",
            "get_gated_tenants",
            set(),
        )()
        all_active_tenants = [
        ...
        for tenant_id in all_active_tenants:
            ...
            rlock: RedisLock = redis_client.lock(
                DocubrainRedisLocks.SLACK_BOT_LOCK,
    ...
    def send_heartbeats(pod_id: str, tenant_ids: set[str]) -> None:
        ...
        for tenant_id in tenant_ids:
            redis_client = get_redis_client(tenant_id=tenant_id)
            heartbeat_key = f"{DocubrainRedisLocks.SLACK_BOT_HEARTBEAT_PREFIX}:{pod_id}"
            redis_client.set(
...
def prefilter_requests(req: SocketModeRequest, client: TenantSocketModeClient) -> bool:
    ...
    bot_token_user_id, bot_token_bot_id = get_docubrain_bot_auth_ids(
    ...
    if req.type == "events_api":
        ...
        if (
            ...
            or remove_docubrain_bot_tag(tenant_id, msg, client=client.web_client)
        ...
        if event_type == "message":
            ...
            if bot_token_user_id and bot_token_user_id in event_user:
                is_docubrain_bot_msg = True

            if bot_token_bot_id and bot_token_bot_id in event_bot_id:
                is_docubrain_bot_msg = True

            # DocubrainBot should never respond to itself
            if is_docubrain_bot_msg:
                logger.info("Ignoring message from DocubrainBot (self-message)")
                return False

            # DMs with the bot don't pick up the @DocubrainBot so we have to keep the
            # caught events_api
        ...
        if is_bot_message:
            ...
            # If DocubrainBot is not specifically tagged and the channel is not set to respond to bots, ignore the message
        ...
        # Can respond in thread if it's an "im" directly to DocuBrain or @DocubrainBot is tagged
        if (
            thread_ts
    ...
    if req.type == "slash_commands":
        ...
        if not channel:
            channel_specific_logger.error(
                "Received DocubrainBot command without channel - skipping"
            )
            return False
        ...
        if not sender:
            channel_specific_logger.error(
                "Cannot respond to DocubrainBot command without sender to respond to."
            )
            return False
...
def build_request_details(
    ...
    if req.type == "events_api":
        ...
        bot_token_user_id, _ = get_docubrain_bot_auth_ids(tenant_id, client.web_client)
        message_ts = event.get("ts")
        thread_ts = event.get("thread_ts")
        ...
        msg = remove_docubrain_bot_tag(tenant_id, msg, client=client.web_client)
        ...
        if tagged:
            logger.debug("User tagged DocubrainBot")
...
def _check_tenant_gated(client: TenantSocketModeClient, req: SocketModeRequest) -> bool:
    ...
    """
    from docubrain.server.settings.models import ApplicationStatus
    ...
    if not is_bot_event and not is_duplicate_mention:
        ...
        if channel:
            respond_in_thread_or_channel(
                ...
                thread_ts=thread_ts,
                text=(
                    "Your organization's subscription has expired. Please contact your DocuBrain administrator to restore access."
                ),
            )
...
def _get_socket_client(
    ...
    # https://docs.docubrain.app/admins/getting_started/slack_bot_setup
