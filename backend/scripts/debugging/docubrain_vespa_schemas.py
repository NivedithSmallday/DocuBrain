"""Tool to generate all supported schema variations for DocuBrain Cloud's Vespa database.

Usage:

```
PYTHONPATH=. python scripts/debugging/docubrain_vespa_schemas.py
```
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        default="docubrain/document_index/vespa/app_config/schemas/danswer_chunk.sd.jinja",
    )
    parser.add_argument(
        "--cloud-services-template",
        help="The cloud-services.xml.jinja template path",
        default="ee/docubrain/document_index/vespa/app_config/cloud-services.xml.jinja",
    )
    parser.add_argument(
        "--output-path",
