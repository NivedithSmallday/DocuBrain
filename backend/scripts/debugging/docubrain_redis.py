...
from docubrain.configs.app_configs import REDIS_DB_NUMBER
from docubrain.configs.app_configs import REDIS_HOST
from docubrain.configs.app_configs import REDIS_PASSWORD
from docubrain.configs.app_configs import REDIS_PORT
from docubrain.configs.app_configs import REDIS_SSL
...
from docubrain.redis.redis_pool import RedisPool
...
class DocubrainRedisCommand(Enum):
...
def get_user_id(user_email: str) -> tuple[UUID, str]:
    if MULTI_TENANT:
        raise RuntimeError(
            "Multi-tenant email-to-tenant mapping requires enterprise components. "
            "Run this script with --tenant-id when MULTI_TENANT is enabled."
        )
...
def docubrain_redis(
    ...
    if command == DocubrainRedisCommand.purge_connectorsync_taskset:
    ...
    elif command == DocubrainRedisCommand.purge_documentset_taskset:
        return purge_by_match_and_type(
            "*documentset_taskset*", "set", batch, dry_run, r
        )
    elif command == DocubrainRedisCommand.purge_usergroup_taskset:
        return purge_by_match_and_type("*usergroup_taskset*", "set", batch, dry_run, r)
    elif command == DocubrainRedisCommand.purge_locks_blocking_deletion:
        ...
        return 0
    elif command == DocubrainRedisCommand.purge_vespa_syncing:
        return purge_by_match_and_type(
            "*connectorsync:vespa_syncing*", "string", batch, dry_run, r
        )
    elif command == DocubrainRedisCommand.purge_pidbox:
        return purge_by_match_and_type(
            "*reply.celery.pidbox", "list", batch, dry_run, r
        )
    elif command == DocubrainRedisCommand.get_list_element:
        ...
        print(f"{result}")
        return 0
    elif command == DocubrainRedisCommand.get_user_token:
        if not user_email:
        ...
        if token_key:
            ...
            return 2
    elif command == DocubrainRedisCommand.delete_user_token:
        if not user_email:
        ...
        if delete_user_token_from_redis(r, user_email, dry_run):
            ...
            return 2
    elif command == DocubrainRedisCommand.add_invited_user:
        if not user_email:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="DocuBrain Redis Manager")
    parser.add_argument(
        "--command",
        type=DocubrainRedisCommand,
        help="The command to run",
        choices=list(DocubrainRedisCommand),
        required=True,
    )
