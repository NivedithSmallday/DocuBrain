...
def create_new_chat_session(docubrain_url: str, api_key: str | None) -> int:
    headers = {"Authorization": f"Bearer {api_key}"} if api_key else None
    session_endpoint = docubrain_url + "/api/chat/create-chat-session"
...
def manage_users(docubrain_url: str, headers: dict[str, str] | None) -> AllUsersResponse:
    endpoint = docubrain_url + "/manage/users"
...
def get_chat_sessions(
    docubrain_url: str, headers: dict[str, str] | None, user_id: UUID
) -> ChatSessionsResponse:
    endpoint = docubrain_url + "/admin/chat-sessions"
...
def get_session_history(
    docubrain_url: str, headers: dict[str, str] | None, session_id: UUID
) -> dict[str, Any]:
    endpoint = docubrain_url + f"/admin/chat-session-history/{session_id}"
...
def process_all_chat_feedback(docubrain_url: str, api_key: str | None) -> None:
    headers = {"Authorization": f"Bearer {api_key}"} if api_key else None

    all_users = manage_users(docubrain_url, headers)
    if not all_users:
    ...
    for user_id in user_ids:
        r_sessions = get_chat_sessions(docubrain_url, headers, user_id)
        logger.info(f"user={user_id} num_sessions={len(r_sessions.sessions)}")
        for session in r_sessions.sessions:
            try:
                s = get_session_history(docubrain_url, headers, session.id)
            ...
            for m in s.get("messages", []):
                logger.info(
                    ...
                    f"feedback_type={m.get('feedback_type')} "
                    f"feedback_text={m.get('feedback_text')}"
                )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        help="DocuBrain URL, should point to DocuBrain nginx.",
    )

    # Not needed if Auth is disabled?
    # Or for DocuBrain MIT Edition API key must be replaced with session cookie
    parser.add_argument(
        "--api-key",
        type=str,
        help="DocuBrain Admin Level API key",
    )

    args = parser.parse_args()
    process_all_chat_feedback(docubrain_url=args.url, api_key=args.api_key)
