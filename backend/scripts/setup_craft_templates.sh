#!/bin/sh
# Setup DocuBrain Craft templates
...
if [ "$ENABLE_CRAFT" = "false" ] || [ "$ENABLE_CRAFT" = "False" ]; then
    echo "DocuBrain Craft is disabled (ENABLE_CRAFT=false), skipping template setup"
    exit 0
fi
...
CRAFT_BASE="/app/docubrain/server/features/build/sandbox/kubernetes/docker"
...
echo "Setting up DocuBrain Craft templates..."
