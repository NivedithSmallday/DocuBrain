...
def extract_jira_project(project_url: str) -> tuple[str, str]:
    parsed = urlparse(project_url)
    path_parts = [part for part in parsed.path.split("/") if part]

    try:
        projects_index = path_parts.index("projects")
        project_key = path_parts[projects_index + 1]
    except (ValueError, IndexError) as e:
        raise ValueError(f"Invalid Jira project URL: {project_url}") from e

    jira_base = f"{parsed.scheme}://{parsed.netloc}"
    return jira_base, project_key
